...
**Idempotency key must be scoped to the client.** The idempotency store must use `(client_id, idempotency_key)` as the composite unique key — not `idempotency_key` alone. A key-only scope means a key generated by one client could collide with a key generated by another, causing one client's request to return another's result. The client generates the key, but the server scopes it. This is not an edge case in banking: two branches processing deposits simultaneously with UUIDs generated by different client implementations could accidentally converge on the same short key space if the scoping is absent.
