...
**Authoring skills** (`plugins/babelstone-engine/skills/`, see its README — bundled in the `babelstone-engine` plugin) encode the conformant procedures: `new-adr` (scaffold with the right ADR-PC-000 shape + dual number-check + Verifiable-commitments seed), `amend-adr` / `supersede-adr` (the §P9 one-command drift acknowledgment), `pack-author` (a `pt.YYYY.N` regulatory pack). Non-Claude-Code agents can follow each `SKILL.md` as a procedure. Prefer them over hand-rolling these artefacts.
