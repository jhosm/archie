...
| [`review`](./review/SKILL.md) | Run the §P3 domain reviewers over the current diff before a PR — **discovers** the agent roster from disk (drop a new `agents/*.md` and it's picked up; no edit here), routes the diff to the agents in scope, spawns them in parallel, and synthesizes one advisory `PASS \| CHANGES REQUESTED` roll-up; read-only, never gates. Paired with the `surface-review-reminder` PreToolUse hook that nudges before `gh pr create` / `git push -u` | **built** |
