...
The surface has three load-bearing properties. It must be *declarative* — no engine code change to ship a new variant within an existing family. Validation must be *synchronous at commit time* — the product team learns at commit time that a configuration is well-formed and pack-compliant. Deployment must be *safe-by-default* — a new configuration cannot break configurations already running in production. The depth question (templates only, DSL only, or both) is open and tracked in [04 §4](./04-open-questions.md); whichever depth is chosen has to satisfy all three properties or the wedge fails.
