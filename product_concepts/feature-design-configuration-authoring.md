...
- "DSL-only" mis-describes the variant layer as **unbounded**. A genuine DSL — variants composing engine primitives directly — costs the wedge on two mechanisms at once:

  1. **Cadence separation breaks.** The variant cadence is fast (weekly, engineer + risk review, no compliance step) specifically because the typed schema and pack pre-vet what a variant author can express; compliance review sits upstream once at schema time, not per variant. A DSL lets the author compose primitives in combinations the schema never permitted and compliance never saw, which the variant-cadence gate is not sized to evaluate. Either the gate slows down (and the weekly cadence dies) or it stays fast and ships regulatory misuses — the §4 risk that "the DSL can be used to build products that violate regulatory or commercial constraints the engine is meant to enforce."

  2. **The wedge's falsifiability test dies.** §7.1 commits to "a variant PR that touches engine `src/` has failed the wedge" — the test that catches schema gaps and primitive gaps by forcing them to surface as engine-file edits, which then visibly route to the slow-cadence track. With a DSL the same author writes more DSL instead, no engine code touched, and the test passes vacuously even when the variant is engine-level work. The diagnostic that distinguishes "legitimate variant" from "schema gap dressed up as a variant" disappears.
