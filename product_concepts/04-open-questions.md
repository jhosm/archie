...
**Context.** The strangler-fig motion in [01-product-architecture §6](./01-product-architecture.md) and [02-v1-scope §3](./02-v1-scope-term-deposits.md) requires first-class coexistence with the operating bank's legacy core. The legacy estate has several integration surfaces that may be relevant:
...
**Context.** The agility wedge ([00-product-vision §2](./00-product-vision.md), [01-product-architecture §3](./01-product-architecture.md)) depends on new products being configuration changes. The open question is the **depth** of the configuration surface — three credible models:
...
## Lettered Questions from Design-Notes Companions

Each design-notes companion opens its own questions in a continuing letter sequence; they accumulate here so the brief carries the full register, with each entry pointing at its source document for the full context. Skim by letter range; drill into the source for the trade-off space.

### Q-I through Q-Q — from [feature-design-configuration-surface](./feature-design-configuration-surface.md)
...
### Q-R through Q-W — from [feature-design-configuration-authoring](./feature-design-configuration-authoring.md)
...
### Q-X through Q-AC — from [feature-design-event-store-projections](./feature-design-event-store-projections.md)
...
- **Q-AC. Event-store technology selection.** Kurrent / EventStoreDB, Postgres-based, or Redpanda-as-event-store. A small spike per candidate against the synthetic v4-scale load test (see Q-AK) is the proposed path. Refined by Q-AK and §6 of [feature-design-two-modes-asymmetry](./feature-design-two-modes-asymmetry.md).

### Q-AD through Q-AJ — from [feature-design-strangler-fig-coexistence](./feature-design-strangler-fig-coexistence.md)
...
### Q-AK through Q-AO — from [feature-design-two-modes-asymmetry](./feature-design-two-modes-asymmetry.md)
