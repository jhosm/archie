...
| File | What It Covers |
|---|---|
| [feature-design-configuration-surface](./feature-design-configuration-surface.md) | Deepens §01 §3 and §01 §5: rate sheets as a price layer separate from product structure, and the pack vocabulary (T1) as a typed jurisdiction-scoped vocabulary. Resolves two sub-questions of Open Question 8 and opens nine new ones. |
...
| [feature-design-event-store-projections](./feature-design-event-store-projections.md) | Reframes §00 §3 and §02 §2.3: drops "subledger" as a primary concept, replaces with event store + bitemporal projections. Specifies the four time-dimensional capabilities, strict engine-vs-family separation, the cross-cutting vs family-specific event taxonomy (5 new generic events + 3 new family-specific deposit events), handler discipline, replay reconciliation, snapshot strategy, and GL coupling. Resolves Open Question 3 (time-travel) and opens six new questions (Q-X through Q-AC). |
| [feature-design-strangler-fig-coexistence](./feature-design-strangler-fig-coexistence.md) | Deepens §01 §6 and §02 §3: frames coexistence as a multi-year period with start/middle/end phases. Specifies the seven dimensions of dual operation (SoR map, settlement plumbing, legacy emission, unified read surface, reconciliation, regulatory reporting, SoR transitions), the cutover mechanics, and the end state. Commits to three brainstorm-decided shapes: legacy emits a daily batch file (24h staleness asymmetry); regulatory reporting aggregates downstream via a named reporting application; renewal of a legacy in-flight deposit creates a new engine-native instance linked by causation_id. Narrows Open Question 5 to the operational SLA-calibration scope and opens seven new questions (Q-AD through Q-AJ). |
| [feature-design-two-modes-asymmetry](./feature-design-two-modes-asymmetry.md) | Deepens §01 §4 and §03 §v4: operationalises the brief's warning that the engine has to be built with the irregular profile (v4) as the upper-bound design point. Quantifies the with-a-plan vs irregular asymmetry across seven dimensions and commits to **Approach C — interfaces for v4, implementations for v1**, specifying six non-negotiable v1 commitments (event store with scale path; no batch-only assumptions; reserved `partition_key`; per-projection sync/async; snapshot infrastructure in v1; synthetic v4-scale load tests as v1 acceptance). Refines event-store selection criteria for Q-AC and replay budgets for Q-Z; opens five new questions (Q-AK through Q-AO). |
