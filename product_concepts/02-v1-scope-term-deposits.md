...
| Event | When | Carries |
|---|---|---|
| `DepositConstituted` | A new deposit is created and funded | `deposit_id`, `customer_id`, `current_account_id` (debited for principal), `principal_cents`, `currency`, `tan_basis_points`, `term_days`, `start_date`, `maturity_date`, `interest_variant` (`AT_MATURITY` / `PERIODIC` / `ADVANCE`), `payment_period_months` if periodic, `auto_renewal_policy` |
...
- **Early-withdrawal penalty schedules beyond the simplest case.** v1 supports a single early-termination policy per product configuration (e.g. "lose all accrued interest" or "lose accrued interest above a floor"). **Banded penalty schedules** ("first 30 days: 100% penalty; 30–90 days: 50%; ...") are a likely v1 stretch goal, not a true v1+ deferral — most PT incumbent banks already operate banded schedules on at least some deposit products, so a v1 that cannot express them risks not being adoptable on any product line the bank actually sells. Final v1 inclusion is a customer-development call against the first design-partner bank.
