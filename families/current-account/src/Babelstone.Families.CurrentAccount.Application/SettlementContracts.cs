...
// Every field is a structural value or a stable code / opaque reference — NO PII (ADR-PC-004 §P2): the
// account is the opaque path id, the intent reference is a process-derived token, never a name / NIF / IBAN.
//
// The exactly-once key on this surface is INVERTED from every other CA endpoint (ADR-PC-043 §Idempotency,
...
/// <summary>The bounded credit-ADMISSION rejection taxonomy (ADR-PC-043 §The credit-admission gate): the
/// reasons a credit-receive is refused by construction, each a stable machine code the caller / reconciler
/// honours so an undeliverable credit is ATTRIBUTED, never silently dropped (ADR-PC-043 §Undeliverable
...
public static class CreditRejectedReason
{
    ...
    /// §Undeliverable credit — hold at source).</summary>
    public const string AccountClosed = "ACCOUNT_CLOSED";

    /// <summary>The account is ERASED (GDPR Article 17 terminal) — a credit cannot land (ADR-PC-043 §The
...
/// <summary>The intent behind one settlement CREDIT-receive attempt (ADR-PC-043 §2 ConfirmCredit): land
/// <see cref="AmountCents"/> as a Credit into account <see cref="AccountId"/>, if the account can receive it.
/// STRUCTURAL only — no PII (ADR-PC-004 §P2). The pure <see cref="CurrentAccountCreditAdmissionDecider"/>
...
/// (the in-band WRONG-AMOUNT guard, ADR-PC-043 §1); the decider rejects a non-positive amount.</param>
...
/// <summary>The intent behind one settlement CAPTURE attempt (ADR-PC-043 §2 ConfirmDebit): turn an authorize
...
/// only — no PII (ADR-PC-004 §P2).</summary>
...
/// one intent (SETTLEMENT_CA_CAPTURE_HOLD_MATCH); the spine captures WHERE the hold state is ACTIVE.</param>
...
/// <param name="TargetHoldId">The reservation to capture — the authorize's hold id (SETTLEMENT_CA_CAPTURE_HOLD_MATCH).</param>
...
/// over-capture, ADR-PC-037 §D4), or null when the capture landed cleanly.</summary>
