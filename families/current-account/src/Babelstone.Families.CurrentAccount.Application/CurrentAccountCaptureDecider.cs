...
/// <summary>
/// The pure decision core of the settlement CAPTURE (ADR-PC-043 / ADR-PC-037): given the account's active-hold set and a capture command, it enforces the
...
/// </summary>
/// <remarks>
/// <b>Enforce authorize.hold_id == capture.target_hold_id for one intent (ADR-PC-043; pinned by
/// CurrentAccountCaptureTests).</b> The target MUST name an ACTIVE authorization hold on this account —
...
/// command-time half of the ADR-PC-043 double guard; the projection-time half is the store's
...
public static class CurrentAccountCaptureDecider
{
    ...
    /// ADR-PC-037), and return the atomic <see cref="HoldCaptured"/> + <see cref="AccountDebited"/>
        ...
        string accountRef, IReadOnlyList<Hold> activeHolds, CaptureAccountCommand command)
    {
        ...
        // Hold-match (ADR-PC-043; pinned by CurrentAccountCaptureTests): the target must name an ACTIVE authorization hold on
        ...
        // Classify the capture amount against the held amount (ADR-PC-037): under releases the remainder,
...
/// followed by the family <see cref="AccountDebited"/>) and the ADR-PC-037 reconciliation signal (a
