...
public sealed class CurrentAccountFamilyModule : IFamilyModule
{
    ...
    [
        ...
        // The authorize refusal fact (ADR-PC-037): a DECLINED synchronous authorize appends this
        ...
        // The overdraft-interest accrual fact (ADR-PC-037): a day's descoberto interest charged against
        ...
        // family state — ADR-PC-033). The fee math is command-side, never in this fold (ADR-PC-037).
        ...
        // The settlement CREDIT-receive fact (ADR-PC-043): the family's
        // FIRST money-IN event. Store-only for now (its .avsc/EventCatalog entry is a later change) and
        ...
        // the credit-receive command, never in this fold (ADR-PC-043).
        ...
        // The settlement CAPTURE-debit fact (ADR-PC-043): a
        ...
        // .avsc is a later change) and folded as a no-op on the family position, but IMovementBearing,
        ...
        // family — ADR-PC-021); the family supplies only its TState here, splicing in every
