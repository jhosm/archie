...
public static class SagaCommandPayloadFactory
{
        ...
        SagaBusinessReference reference)
    {
        ...
        return commandType switch
        {
            ConstitutionProcess.ReserveAccountBalance => new ReserveAccountBalanceCommand
            {
                ...
                // (a GUID string the engine-CA authorize writer reads as the destination, ADR-PC-043);
            ...
            ConstitutionProcess.ConfirmDebit => new ConfirmDebitCommand
            {
                ...
                // The engine-CA capture leg. AccountRef is the promoted destination
                ...
                // authorize placed and targets it (target_hold_id = f(intent_reference)); AmountCents
    ...
    /// <summary>
    /// The settlement-counterparty discriminator for a constitution funding leg:
    ...
    /// </summary>
    /// <remarks>
    /// PURE and deterministic (ADR-PC-010) — a classification of the pinned <c>SourceAccountRef</c>
    ...
    // GUID). Deterministic string classification — no clock, no I/O (ADR-PC-010).
