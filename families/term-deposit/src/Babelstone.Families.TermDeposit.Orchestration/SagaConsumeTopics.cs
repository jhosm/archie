...
/// subscribes to this topic"). The orchestrator keys its <see cref="ConstitutionProcess"/>
...
///   <see cref="ConstitutionProcess.ProcessConstituted"/>; bd babelstone-3klm) arrives HERE, with
...
/// matter of registering its <see cref="ISagaStateMachine"/> /
/// <c>ISagaCommandRouter</c> / <see cref="IResultEventBridge"/> alongside the
...
public static class SagaConsumeTopics
{
    ...
    /// the engine's integration facts arrive — those land on <see cref="TermDepositIntegrationTopic"/>.
    /// This is the orchestrator's OWN process topic, not a catalogued integration channel, so it stays an
    /// EXPLICIT constant — it has no AsyncAPI catalogue entry to derive it from (ADR-IC-003 §A7).</summary>
    ...
    /// topic; the engine adds no routing).
    /// <para>
    /// DERIVED from the AsyncAPI catalogue, not hand-written: the generated
    /// <see cref="FamilyIntegrationTopics.All"/> is the family's catalogue channels (topic == channel ==
    /// <c>aggregate_type</c>), regenerated by <c>scripts/gen-saga-topics.py</c> and CI-gated against the
    /// catalogue (a build failure on drift, never a runtime stall — ADR-IC-003). The term-deposit family
    /// publishes every event on a SINGLE channel (<c>term_deposit</c>); this exposes that one topic for the
    /// single-topic call sites (the renewal saga, the test fixtures). A second integration channel would
    /// be a build break here, surfacing the new subscription explicitly rather than silently.</para></summary>
    public static string TermDepositIntegrationTopic { get; } = FamilyIntegrationTopics.All.Count == 1
        ? FamilyIntegrationTopics.All[0]
        : throw new InvalidOperationException(
            "The term-deposit family now publishes on more than one catalogue channel "
            + $"([{string.Join(", ", FamilyIntegrationTopics.All)}]); update the saga subscription to "
            + "consume all of them (use FamilyIntegrationTopics.All directly) before relying on the "
            + "single-topic TermDepositIntegrationTopic accessor.");

    /// <summary>The topic set a constitution-saga consumer subscribes to: the orchestrator-produced
    /// process topic (explicit) AND the engine's term-deposit integration topic(s) DERIVED from the
    /// catalogue (<see cref="FamilyIntegrationTopics.All"/>; bd babelstone-9w2k.4). The internal process
    /// topic is prepended explicitly so an over-broad pattern can never pull it (or an unrelated topic)
    /// into the consumer group (ADR-IC-001 one-group-per-consumer); the integration topics come from the
    /// catalogue so a missed channel is a CI failure, not a saga that silently never advances.</summary>
    public static IReadOnlyList<string> ConstitutionProcessTopics { get; } =
        [ConstitutionProcessTopic, .. FamilyIntegrationTopics.All];
}
