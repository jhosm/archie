...
/// <c>aggregate_type</c>; ADR-IC-003 A6 2026-06-15) via <c>SagaConsumeLoop</c> —
...
public static class ConstitutionResultEvents
{
    ...
    public sealed class Bridge : IResultEventBridge
    {
        ...
        /// <summary>
        /// [REVIEW-FLAG A] The in-aggregate ValidateProductLimits AUTO-PASS (bd babelstone-t7o3.8,
        /// ADR-IC-018 §P6). ValidateProductLimits has NO HTTP route (the router returns null), but at v1
        /// it AUTO-PASSES so the parallel-validation join can complete and the happy path reach COMPLETED.
        /// The substrate treats this no-route command as a SYNTHETIC Applied (flip PUBLISHED + self-advance
        /// to LimitsValidated). Every OTHER no-route command STILL becomes terminal FAILED — the bridge
        /// names only its own carve-out, so the substrate carries no family branch. The real product-limits
        /// verdict (incl. LimitsRejected) is H.2 / babelstone-n55u.
        /// </summary>
        public bool IsNoRouteAutoPass(string commandType) =>
            commandType == ConstitutionProcess.ValidateProductLimits;

        /// <summary>
        /// Scenario C (bd babelstone-t7o3.10, ADR-IC-018 §P6): an HTTP 202 Accepted on the irreversible
        /// ConfirmDebit is an EXPLICIT INDETERMINATE settlement signal (the ACL accepted the debit but
        /// cannot yet confirm whether the Core executed it — the network dropped after the debit was sent).
        /// 202 is a 2xx, so the substrate would otherwise classify it Applied; this reinterpretation makes
        /// the dispatcher flip the row terminal and self-advance with CoreDebitIndeterminate, parking the
        /// saga in AWAIT_CORE_CLEARANCE (ADR-IC-003 §P4). ONLY for ConfirmDebit; every other command/status
        /// returns null (the substrate's default classification applies). A ConfirmDebit *timeout* is NOT
        /// this — it stays transient (the substrate leaves the row PENDING for an idempotent retry).
        /// </summary>
        public CommandDeliveryKind? ClassifyResponse(string commandType, int httpStatusCode) =>
            ...
            && commandType == ConstitutionProcess.ConfirmDebit
