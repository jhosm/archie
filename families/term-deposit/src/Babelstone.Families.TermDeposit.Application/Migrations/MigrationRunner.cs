...
/// <summary>
/// Applies the term-deposit family's forward-only <see cref="MigrationSet"/> against a
/// PostgreSQL database (lifted from the engine's
/// <c>Babelstone.EventStore.Migrations.MigrationRunner</c>). Thin and hand-rolled
/// (ADR-PC-010): no migration framework owns the schema. It connects as the migration role
/// (ADR-PC-001 §P3) — the role that holds the DDL privileges the engine's runtime role is
/// denied.
///
...
/// (<c>schema_migrations_term_deposit</c>) and serialises on a DISTINCT advisory-lock key, so
...
/// the engine migrations must run first — this set's <c>0001_read_model.sql</c> grants on the
/// <c>babelstone_engine</c> role that engine migration <c>0002_append_only_role.sql</c> creates,
/// and fails loud if that role is absent.
/// </summary>
public sealed class MigrationRunner(string connectionString)
{
    // DISTINCT ledger from the engine's `schema_migrations`: the family read model shares the
    // engine's Postgres tier (ADR-IC-005 §S1), so a shared ledger would interleave two
    // independently-versioned sets (engine 0001.. and family 0001..) in one table — duplicate
    // version PKs and a cross-set version collision. A per-set ledger keeps each set's applied
    // history its own.
    private const string LedgerDdl = """
        CREATE TABLE IF NOT EXISTS schema_migrations_term_deposit (
    ...
    // A stable, arbitrary 64-bit key naming THIS runner's session advisory lock. Two runners
    // that start concurrently (overlapping deploys, an app boot racing CI) would otherwise both
    // read the ledger, both apply migration N, and collide. The lock serialises them: the second
    // waits. A DIFFERENT constant from the engine runner's (3937070637541916881) and the
    // orchestrator runner's (6044827715031892057), so a family deploy and an engine deploy against
    // the same cluster do not block on each other's unrelated migration set.
    private const long MigrationLockKey = 5821934760118327449;

    /// <summary>
    /// Applies every migration not yet recorded in <c>schema_migrations_term_deposit</c>, in
        ...
        try
        {
            ...
            foreach (var migration in MigrationSet.All)
            {
                ...
                await using (var record = new NpgsqlCommand(
                    "INSERT INTO schema_migrations_term_deposit (version, name) VALUES (@version, @name);", connection, tx))
                {
        ...
        NpgsqlConnection connection, CancellationToken ct)
    {
        ...
        await using var command = new NpgsqlCommand("SELECT version FROM schema_migrations_term_deposit;", connection);
