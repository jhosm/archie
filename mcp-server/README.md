...
The walking-skeleton slice (auth deferred): a `constitute_deposit` **tool** (write) and a
`get_deposit` **tool** (on-demand read), over Streamable HTTP, that hit the engine command/query
host (`Babelstone.Engine.Api`, [ADR-PC-021](../docs/product-management/product_concepts/adrs/ADR-PC-021-application-layer-family-owned-deciders.md) §D5) directly. The
