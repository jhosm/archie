...
/// (lifted from the engine's <c>Babelstone.EventStore.Migrations.MigrationRunner</c>).
/// Thin and hand-rolled (ADR-PC-010): no migration framework owns the schema. It connects
/// as the migration role (ADR-PC-001 §P3) — the role that holds the DDL privileges the
/// orchestrator's runtime role (<c>babelstone_orchestrator</c>) is denied.
/// </summary>
public sealed class MigrationRunner(string connectionString)
{
    ...
    // otherwise both read the ledger, both apply migration N, and collide. The lock
    // serialises them: the second waits. A DIFFERENT constant from the engine runner's, so
    // an orchestrator deploy and an engine deploy against the same cluster do not block on
    // each other's unrelated migration set.
    private const long MigrationLockKey = 6044827715031892057;

    /// <summary>
    /// Applies every migration not yet recorded in <c>schema_migrations</c>, in ascending
    /// version order. Each migration runs in its own transaction together with its ledger
    /// insert, so a failure leaves the database at the last fully applied version.
