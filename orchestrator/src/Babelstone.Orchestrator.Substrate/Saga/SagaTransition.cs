...
/// </summary>
/// <param name="Next">The state the saga moves to. An OPAQUE string to the substrate
/// (ADR-IC-018 §D3/§P3): each saga owns its own state vocabulary as string constants in its
/// module; the substrate persists and compares it as a string, never a central enum.</param>
...
public readonly record struct TransitionOutcome(string Next, ImmutableArray<string> Commands)
{
    /// <summary>A transition to <paramref name="next"/> that emits no commands.</summary>
    public static TransitionOutcome To(string next) => new(next, ImmutableArray<string>.Empty);

    /// <summary>A transition to <paramref name="next"/> that emits the given command names.</summary>
    public static TransitionOutcome To(string next, params string[] commands) =>
        new(next, [.. commands]);
}
...
public interface ISagaStateMachine
{
    ...
    /// state (Document 05 step 0); the machine treats it as the only legal start state. An
    /// opaque string the saga module owns (ADR-IC-018 §D3).</summary>
    ...
    /// </summary>
    bool TryAdvance(string current, string eventType, out TransitionOutcome outcome);
    ...
    /// terminal set (ADR-IC-018 §D3), so the multi-saga substrate asks the routed machine —
    /// never a central per-saga predicate — once it has keyed the machine by <c>saga_type</c>.
    /// A late event for a terminal saga is a no-op advance.
    /// </summary>
...
/// specification (ADR-IC-003 §P2). A concrete saga (e.g. the term-deposit <c>ConstitutionProcess</c>
/// in its family module) is just this base populated with its <c>(state, event) → (next, commands)</c> rows.
/// </summary>
public abstract class TableStateMachine : ISagaStateMachine
{
    private readonly ImmutableDictionary<(string, string), TransitionOutcome> _table;

    /// <param name="sagaType">The persisted <c>saga_type</c> discriminator.</param>
    /// <param name="initialState">The only legal start state (an opaque string the saga owns).</param>
    ...
    protected TableStateMachine(
        string sagaType,
        string initialState,
        IEnumerable<((string From, string Event) Key, TransitionOutcome Outcome)> table)
    {
        SagaType = sagaType;
        InitialState = initialState;

        var builder = ImmutableDictionary.CreateBuilder<(string, string), TransitionOutcome>();
    ...
    /// <para>
    /// <b>A concrete saga MAY override this</b> precisely for such a state: e.g. the term-deposit
    /// <c>ConstitutionProcess</c> overrides it because HUMAN_INTERVENTION_REQUIRED appears in its
    /// table only as a <c>To()</c> target, never as a <c>From</c>-key, so this default would report
    /// it terminal — but it is a production-reachable escalation state an operator resolves out of.
    /// Its override keeps HIR NON-terminal and the refactor behaviour-preserving, so the substrate
    /// default and that machine's answer DIVERGE on HIR today, by design. The override lives in the
    /// family module (ADR-IC-018 §D3) — the substrate names no family state here.
    /// </para></summary>
    public virtual bool IsTerminal(string state) => !_table.Keys.Any(k => k.Item1 == state);
    ...
    public ImmutableDictionary<(string From, string Event), TransitionOutcome> Transitions => _table;
}
