...
| [**implementation_guidelines/**](./docs/product-management/implementation_guidelines/code-comments.md) | How we write the code, not just what it does | Cross-cutting authoring conventions — the code-comment guideline (comment the *why*, cite only durable/verifiable anchors) and the [telemetry guideline](./docs/product-management/implementation_guidelines/telemetry.md) (what to instrument, no PII, emit through the shared `babelstone.*` contract) |
