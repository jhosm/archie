...
/// <see cref="TermDepositProjectionReader"/> read access is wired — it does NOT yet schedule
...
/// </para>
/// <para>
/// The reader is injected (not constructed here) so the timing child can drive it without touching
/// the host shell, and so a test can substitute a fake projection store. No clock reads, no
/// emission, no I/O beyond the idle wait.
/// </para>
/// </remarks>
public sealed class NotificationWorker(
    TermDepositProjectionReader projectionReader,
    ILogger<NotificationWorker> logger) : BackgroundService
{
    // Held so the timing/emission children resolve the read window through the host, and so the DI
    // composition fails loud at startup if the reader was not registered. Not exercised on a clock
    // here — this skeleton introduces no scheduling (bd babelstone-60n8.1 acceptance criteria).
    private readonly TermDepositProjectionReader _projectionReader =
        projectionReader ?? throw new ArgumentNullException(nameof(projectionReader));
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        _logger.LogInformation(
            "Notification worker started (ADR-IC-011). Projection read access wired over the " +
            "ADR-IC-005 surface; no scheduler timing or emission in this skeleton.");
