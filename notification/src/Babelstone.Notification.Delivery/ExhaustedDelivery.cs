...
/// <summary>
/// One retry-exhaustion fact awaiting (or having completed) its backbone announcement — the CLR face
/// of a <c>notification_delivery_exhausted</c> outbox row (ADR-IC-011). The row is written by
/// <see cref="PostgresDeliveryOutbox.MarkDeadLetteredAsync"/> in the SAME transaction as the
/// DEAD_LETTERED flip (ADR-IC-004: a crash between "give up" and "announce it" never loses the
...
/// Never the name/NIF/contact itself (ADR-PC-004).</param>
...
/// <param name="TriggerKind">EVENT_DRIVEN | SCHEDULED | PRE_CONTRACTUAL (ADR-PC-025).</param>
/// <param name="CausationId">The causing domain event the undelivered signal traced to — carried
/// through unchanged so an operator can walk from the exhaustion back to the fact that implied the
/// notice (ADR-PC-023). Null for SCHEDULED.</param>
/// <param name="Attempts">Delivery attempts made before dead-lettering (the ADR-IC-011 exhaustion
/// count).</param>
...
/// <summary>
/// The relay's read side of the exhaustion outbox (ADR-IC-011 / ADR-IC-004): claim the PENDING
/// announcements, publish each to the backbone, flip it PUBLISHED. A produce failure leaves the row
/// PENDING for the next pass (backpressure, never FAILED — the same posture as the engine relay).
/// Implemented by <see cref="PostgresDeliveryOutbox"/> alongside <see cref="IDeliveryOutbox"/>: the
/// two faces share one store because the exhausted row is BORN in the delivery store's own
