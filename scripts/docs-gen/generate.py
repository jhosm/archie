...
It also renders one file OUTSIDE the reference/ tree — the DocFX home page
(ADR-PC-026 §P3):

  docfx/index.md     <- docs/product-docs/README.md            (corpus-relative links absolutised to main GitHub URLs)
...
# DocFX home page (ADR-PC-026 §P3): the site's landing page IS the product-docs
# README, but the corpus it links into is NOT stitched into the site, so its
# relative links 404 there. We render the README into docfx/index.md with every
# relative link rewritten to an absolute `main` GitHub URL — the same main-pin the
# navbar's "Docs on GitHub" entry uses (the Pages artifact is built only from
# main). The README on GitHub keeps its version-correct relative links untouched.
PRODUCT_DOCS = ROOT / "docs" / "product-docs"
DOCFX_INDEX = ROOT / "docfx" / "index.md"
GH_BLOB = "https://github.com/jhosm/babelstone/blob/main/"  # files
GH_TREE = "https://github.com/jhosm/babelstone/tree/main/"  # directories
_README_LINK_RE = re.compile(r"(\]\()([^)]+)(\))")
_README_SKIP = ("http://", "https://", "mailto:", "tel:", "#", "/", "data:")
...
# --------------------------------------------------------------------------
# DocFX home page — the product-docs README with corpus links absolutised
# (ADR-PC-026 §P3). Lives outside the REF tree; written/checked separately.
# --------------------------------------------------------------------------
def _absolutise_readme_link(target: str) -> str:
    """Rewrite a README-relative link target to an absolute `main` GitHub URL.
    External (http/mailto/…), in-page anchor, and root-absolute links pass through
    unchanged. Directories map to /tree/main/, files to /blob/main/; any #anchor is
    preserved. Targets are resolved relative to docs/product-docs/."""
    if target.startswith(_README_SKIP):
        return target
    path, sep, frag = target.partition("#")
    if not path:
        return target
    resolved = (PRODUCT_DOCS / path).resolve()
    try:
        repo_rel = resolved.relative_to(ROOT).as_posix()
    except ValueError:
        return target  # escapes the repo tree — leave it alone (shouldn't happen)
    base = GH_TREE if (path.endswith("/") or resolved.is_dir()) else GH_BLOB
    return f"{base}{repo_rel}{sep}{frag}"


def render_docfx_home() -> str:
    src = (PRODUCT_DOCS / "README.md").read_text()
    body = _README_LINK_RE.sub(
        lambda m: m.group(1) + _absolutise_readme_link(m.group(2)) + m.group(3), src
    )
    banner_line = (
        "<!-- DO NOT EDIT — generated by scripts/docs-gen/generate.py from "
        "docs/product-docs/README.md (ADR-PC-026 §P3). This is the README rendered "
        "as the DocFX home page, with its corpus-relative links rewritten to "
        "absolute main GitHub URLs (the corpus is not stitched into this site, so "
        "relative links would 404). The README on GitHub keeps its relative links. "
        "Run: make docs-gen -->\n"
    )
    return banner_line + "\n" + body.lstrip("\n")
...
def main() -> int:
    ...
    home = render_docfx_home()  # docfx/index.md (ADR-PC-026 §P3), checked/written alongside
    ...
    if check:
        ...
        if not DOCFX_INDEX.exists():
            drift.append("missing:  docfx/index.md")
        elif DOCFX_INDEX.read_text() != home:
            drift.append("changed:  docfx/index.md")
        if drift:
            print("docs-verify: generated docs are out of date — run `make docs-gen`:")
            print("\n".join("  " + d for d in drift))
            return 1
        print(
            f"docs-verify: generated docs are up to date "
            f"({len(pages)} reference pages + docfx/index.md)."
        )
        return 0
    ...
    for f in sorted(REF.rglob("*.md")):
        if f not in generated:
            f.unlink()
    DOCFX_INDEX.write_text(home)
    print(
        f"docs-gen: wrote {len(pages)} reference pages to {rel(REF)}/ "
        f"and the DocFX home page to {rel(DOCFX_INDEX)}."
    )
    return 0
