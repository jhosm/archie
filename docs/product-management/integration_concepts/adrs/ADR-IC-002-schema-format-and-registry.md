...
**S2 · Ecosystem coherence:** Avro and the Confluent Schema Registry were designed together for the Kafka ecosystem. Every Kafka / Redpanda documentation example, every Kafka Connect converter, and every stream processing framework uses Avro as the default format. The Redpanda built-in SR has first-class Avro support. Compatibility mode enforcement (BACKWARD, FORWARD, FULL, NONE) is deeply documented and battle-tested for Avro specifically. The integration is seamless.
