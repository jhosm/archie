...
## Amendment — 2026-07-08: the Mission Control demo BFF is an acknowledged §P2/§P4 exception — it same-origin-proxies to the internal services and attests `X-Client-Id` from the OIDC session `sub`, not from Kong

In plain English: this ADR says every caller reaches the backend through Kong, which validates the caller's token and stamps a trustworthy caller id on the request. The little demo server that fronts the Mission Control UI on the staging host (`app.babelstone.dev`) does **not** do that — it proxies straight to the internal engine/orchestrator without Kong in front. It used to forge a *static* caller id (`CLI-DEMO-0001`) on that path, which silently contradicted this ADR. Now that the demo has a real OpenID-Connect login, it instead sets the caller id from the **logged-in operator's own identity** (the OIDC session's `sub`), which is exactly the value Kong itself would propagate — so the caller id is now genuinely *attested*, not invented. But the demo still isn't Kong: it has no edge rate-limiting, request validation, or SCA step-up on that path. This amendment records that the Kong-bypass is an accepted exception for this demo host, so the divergence is explicit rather than silent ([ADR-PC-020 §D3](../../product_concepts/adrs/ADR-PC-020-llm-toolchain-and-conformance-governance.md)).

The Mission Control demo BFF (staging, `app.babelstone.dev`) is an acknowledged exception to §P2/§P4 for this Traefik-fronted demo host: it same-origin-proxies to the internal engine/orchestrator rather than routing through Kong, and attests `X-Client-Id` from the OIDC-validated session `sub` (mirroring Kong's attest-from-`sub` algorithm) rather than from a Kong-validated token. It does NOT provide edge rate-limiting, request validation, or SCA step-up on that path. Full Kong-fronted conformance for this path is tracked as bd babelstone-zla1.10.9. The production `api.babelstone.dev` product surface remains Kong-fronted and unchanged.

This is additive: it changes no §P2/§P4 obligation for the Kong-fronted product surface — it only records that the demo host is a scoped exception. The attest-from-`sub` mechanism the BFF now uses is deliberately the *same* algorithm §P4 mandates for Kong (overwrite `X-Client-Id` from the validated principal, never trust a client-supplied value), so the exception narrows to "no Kong edge policies," not "no attestation."

---

## Amendment — 2026-07-12: §P1's edge convergence has a second, boot-time path on staging — a Kong initContainer splices real mcp-mtls-ca edge certs from a mounted Secret when deck-sync is unavailable

In plain English: §P1 says Kong's whole edge config is one git-tracked file, converged by `deck sync` (never an Admin-API mutation). On staging that `deck sync` is deliberately skipped — the box runs OpenBao in `-dev` mode with no real edge material (real provisioning is deferred to M.2, bd babelstone-puu3) and the Admin API is bound to the pod loopback. But since bd babelstone-zla1.12.25 the engine + orchestrator REQUIRE a client cert that chains to the cluster-generated internal CA (`mcp-mtls-ca`), and `mcp-server` always has — so the committed kong.yml's THROWAWAY POC edge client certs are rejected at the TLS handshake, and with no `deck sync` there was no path to replace them. This amendment records the fix (bd babelstone-fhw2.1): a Kong **initContainer** mounts cert-manager Secrets holding real `mcp-mtls-ca` material and, at pod start, splices it over the POC placeholders **by entity id** into a copy of kong.yml that Kong then loads. It is a **render-and-load of the declarative config, not an Admin-API mutation** — so it honours §P1's letter — but it is a *second* convergence path alongside `deck sync`, which this records so the divergence is explicit, not silent (the [ADR-PC-020 §D3](../../product_concepts/adrs/ADR-PC-020-llm-toolchain-and-conformance-governance.md) drift gate).

The staging edge may be converged by a boot-time initContainer splice (`infra/kong/splice-edge-certs.sh`, run from `overlays/staging/kong-edge-mtls.patch.yaml`) in addition to `deck sync`. Both paths obey §P1's binding constraints: **DB-less** (`database = "off"`, unchanged), **config-in-git** (the committed kong.yml is the source; the splice replaces only the POC cert *material*, which — exactly like the OpenBao material `deck sync` injects — is cluster-generated and never committed), and **no Admin-API mutation** (Kong loads the rendered file via `KONG_DECLARATIVE_CONFIG`; nothing is `PATCH`ed on a running instance). The splice is the pod-start mirror of `scripts/deck-sync.sh`'s `splice_pem`: the SAME id-anchored, surgical block-scalar replace over the SAME entities (`7e9b6f1a-…` engine/orchestrator client cert, `a1b2c3d4-…` mcp-server client cert, `f0e1d2c3-…` the `mcp-mtls-ca` bundle the `mcp-server` upstream verifies against), touching no other byte — so the embedded Lua pre-functions, routes, and plugins are unchanged.

This is additive and scoped to staging: it changes no §P1 obligation for the deck-sync path (which remains the mechanism on any environment where OpenBao holds real edge material) and no §P5 trust model. It deliberately does **not** flip `tls_verify` on the engine/orchestrator upstreams (the §P5 reverse half — Kong verifying the *upstream server* cert): those services carry no `ca_certificates` reference and there is no `KONG_LUA_SSL_TRUSTED_CERTIFICATE`, so flipping them would make Kong verify an `mcp-mtls-ca` server cert against the container's system store and break the hop. The engine's own `RequireCertificate` + CA-pin validation (Boundary 2, the enforcement that matters) is unaffected; the client-cert splice is exactly what lets a valid Kong-presented cert through it. The boot-time path retires once M.2/bd babelstone-puu3 provisions real edge material into staging OpenBao and `deck sync` is un-skipped.
