...
## Amendment — 2026-06-14: at v1, the saga-side not-executed clearance fails closed instead of RETRY_PERMITTED (no reissue producer yet)

**In plain English:** this ADR says that when a clearance check finds a debit did *not* execute, the operation should be marked "retry-permitted" so the saga can decide whether to send it again. At v1 the part of the system that would actually re-send it — the ACL's in-flight state machine — does not exist yet (the ACL is still a stub). So for now, when the orchestrator's v1 clearance query comes back "not executed", the saga simply fails cleanly: no money moved, so there is nothing to undo, and there is nothing to retry against. This records that deliberate gap on the record and names exactly when it closes (the real ACL, DEF-1).

Wiring the constitution saga's indeterminate-clearance leg (bd `babelstone-t7o3.10`, on [ADR-IC-003](./ADR-IC-003-saga-orchestrator.md)) put §D5 step 5 / §P5's not-executed disposition into a v1 context this ADR did not anticipate: a saga running **before** the ACL service exists. The orchestrator stands in for the absent ACL clearance task at v1 ([ADR-IC-003](./ADR-IC-003-saga-orchestrator.md) Amendment A4), and its not-executed branch cannot honour `RETRY_PERMITTED` because nothing yet implements the reissue path.

*Revised 2026-06-14: §D5 step 5 and §P5 (`RETRY_PERMITTED` → "let the saga's compensation logic decide whether to reissue with the same `idempotency_key`") are the **steady-state** disposition and remain binding the moment the real ACL exists. At v1 there is **no reissue producer**: the ACL in-flight state machine (`IN_FLIGHT → CONFIRMED | INDETERMINATE | REJECTED`; `INDETERMINATE → CONFIRMED | RETRY_PERMITTED`) and the outbound client that refuses to re-send except from `RETRY_PERMITTED` are not built. So the v1 orchestrator-side clearance (a transitional stand-in for the ACL clearance task — [ADR-IC-003](./ADR-IC-003-saga-orchestrator.md) Amendment A4/A6) fails **closed** on the not-executed verdict (`DebitNotExecuted → DEPOSIT_CONSTITUTION_FAILED`, no reversal — a not-executed debit committed nothing, and §P4 of ADR-IC-003 forbids a busy retry) instead of transitioning to `RETRY_PERMITTED`. This is conservative and money-safe: it never reverses a debit that never happened and never blind-reissues. **Back-out:** DEF-1 (bd `babelstone-ub9s`) lands the §D5 state machine and restores the `RETRY_PERMITTED` path, after which the saga's compensation logic owns the reissue decision exactly as §D5/§P5 specify. The mirror of this divergence is recorded in [ADR-IC-003](./ADR-IC-003-saga-orchestrator.md) Amendment A6 so the two records agree. This is a divergence under the explicit-drift gate (ADR-PC-020 §D3), not a supersession — §Decision and §P1–§P8 are otherwise binding as written.*
