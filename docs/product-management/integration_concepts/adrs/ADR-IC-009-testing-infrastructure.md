...
*Correction 2026-07-03: the official `pactfoundation/pact-broker` image does NOT embed PostgreSQL — it expects an external database via `PACT_BROKER_DATABASE_URL`. The dev-stack composition (`infra/compose.yaml`, profile `pact`) therefore pairs the broker with a companion `postgres:16-alpine` service, the same service-scoped-second-Postgres shape as the Backstage catalog's. Narrative correction only; the S1 conclusion (one acceptable extra database container at POC scale) and the Decision are unchanged.*
