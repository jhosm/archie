...
**Third, the Outbox Event Router SMT's value is reduced for a small schema.** The SMT provides routing logic for multi-aggregate outbox tables (routing to different Redpanda topics based on `aggregate_type`). The same logic is a handful of lines in the polling publisher's topic-selection code. The SMT's value is proportional to the number of aggregate types sharing an outbox table; at POC scale, this is not a differentiating advantage.
...
- **Upgrade path coordination:** when the team upgrades from polling to Debezium CDC, the actual risk is **duplicate publication during the cutover window** — both mechanisms reading the same `PENDING` rows and producing them, with consumer inbox dedup absorbing the duplicates but at the cost of doubled relay traffic and inverted ordering risk if Debezium produces a later row before polling acks an earlier one. (There is no SQL-level conflict: Debezium reads WAL offsets and does not depend on the `status` column.) The migration sequence avoids the window entirely: deploy Debezium with a fresh consumer group in parallel, drain the polling publisher (wait until all `PENDING` rows are `PUBLISHED`), disable the polling publisher, verify CDC delivery via consumer lag metrics, then optionally repurpose or drop the `status` column.
...
## Amendment — 2026-06-16: an optional `integration_headers` column the relay promotes to `ce_<key>` headers

**In plain English:** the relay used to publish a fixed set of CloudEvents headers, all built from a closed list of outbox columns. We now let a domain event declare a few extra routing labels (CloudEvents *extension attributes*) that the relay copies onto the message as `ce_<key>` headers — so a downstream consumer can filter on them without decoding the payload. To carry those labels safely, the outbox row gets one new optional column. Nothing about the existing columns or the publisher's order/idempotency changes.

§P1 fixes the outbox column set and states "the publisher reads only the above columns; additional columns are invisible to the relay." Realising the generic, family-agnostic CloudEvents-extension promotion seam [ADR-IC-018 §P5](./ADR-IC-018-family-owned-saga-modules.md) anticipates requires the relay to read **one** more column, so this amendment extends the §P1 contract — narrowly and additively:

- **The outbox row gains `integration_headers`** (`JSONB`, **nullable**), added to the §P1 column set after `published_at`. It stores the CloudEvents extension attributes the event declared (`DomainEvent.IntegrationHeaders`): a `{ key → string-value }` map, written in the same append transaction as the row (§P6 / `ES_ATOMIC_APPEND_OUTBOX` unchanged). `NULL` means the event declared none — the case for every pre-amendment row and every event with no header-only routing discriminator.
- **The relay now reads `integration_headers` too**, and for each entry emits a `ce_<key>` CloudEvents header (e.g. `autorenewalpolicy` → `ce_autorenewalpolicy`) **in addition to** the standard CE header set. The relay names no key and knows no family — it copies whatever map the row carries, so the seam stays generic (the §P1 "publisher reads only the listed columns" clause is widened by exactly this one column; no other column becomes relay-visible). Every emitted header therefore remains **derivable from the outbox row alone** — the new column *is* on the row.

This is additive and BACKWARD-compatible: `ADD COLUMN … JSONB NULL` leaves existing rows `NULL`, for which the relay emits no extension header (its prior behaviour exactly). The Decision (a custom polling publisher on a PostgreSQL outbox) is unchanged, so this does not supersede the ADR (ADR-PC-020 §D3/§D5). The values are structural routing tokens, never PII (ADR-PC-004 §P2). The engine realises it in migration `0016_outbox_integration_headers.sql`, `Babelstone.EventStore.OutboxRow.IntegrationHeaders`, `PostgresEventStore.InsertOutboxAsync`, and `OutboxDrainer` (the drain `SELECT` + `BuildHeaders`). Its first user is `term_deposit`'s `DepositMatured` (`auto_renewal_policy` → `ce_autorenewalpolicy`), but nothing in the relay or the column is family-specific.
