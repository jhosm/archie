...
- `MCP_SCA_GATE_CANNOT_BYPASS` — **the step-up-SCA gate on the agent money-movers cannot be bypassed client-side**: an irreversible money-mover settles only on the bank's own AS-signed `acr`/`auth_time` (which a courier agent cannot forge), never on the agent's report; a money-mover with no SCA proof or a stale `auth_time` is `422 SCA_REQUIRED` **before any side effect** and does not settle, and a fabricated elicitation "accept" without a genuinely refreshed token is `422`'d again on the retry (§P8, the governing source, as the 2026-06-20 §A7–§A9 amendment realises it on the engine-direct path). Realised in the engine `ScaPrecondition` gate — **`Live`** per the catalogue (the single source of truth for status).
...
This ADR's per-tool scope discipline (one tool maps to exactly one scope, no god scope, §P4) is realised by the [ADR-IC-006](./ADR-IC-006-edge-api-gateway.md) gateway and governed there, not as this ADR's own catalogue row. The §P8 elicitation-URL-mode rule for irreversible operations (the SCA-bound action transitions on the bank's signal, not the agent's report) was originally a saga-orchestrator concern, but the 2026-06-20 §A7–§A9 amendment moved the enforced gate onto the **engine-direct** money-mover path — so its no-client-side-bypass invariant is now this ADR's own catalogue row `MCP_SCA_GATE_CANNOT_BYPASS` above.
...
## Amendment — 2026-06-24: §A10's "same engine gate" assumption is retired — on the full-saga path the SCA freshness re-check relocates to the irreversible settlement leg

In plain English: the 2026-06-20 amendment (§A7–§A10) wired the real step-up-SCA gate for an AI agent's irreversible money-movers and noted that any *saga-routed* money-mover would later thread the same bank-signed SCA proof "to the same engine gate." That note assumed the engine is where the money moves. [ADR-PC-032](../../product_concepts/adrs/ADR-PC-032-money-movement-primitive.md) (Accepted 2026-06-23) changes that: money movement becomes **append-first**, and the irreversible cash leg leaves the engine for a downstream, confirmation-gated settlement saga (bd `babelstone-t7o3.13` / bd `babelstone-t7o3.15`). Once the engine no longer *effects* the cash, an SCA re-check sitting on the engine's append route gates a *record*, not the money — and on the saga's own ordering the cash has already moved by the time the engine is reached. So the freshness authority must follow the cash to the settlement leg. This amendment retires §A10's "same engine gate" assumption for the full-saga path and relocates the enforcement point. It is **additive**: §P8's load-bearing principle — *the irreversible action transitions on the bank's own out-of-band signal, before any side effect, never on the agent's report* — is unchanged; relocating the gate is precisely what keeps it true once the cash leg leaves the engine. (Surfaced when bd `babelstone-ls44` / PR #309 — which carries the gateway-attested claims through the saga substrate on the assumption the engine re-checks them — met ADR-PC-032, which moves the cash leg off the engine and is silent on SCA. Tracked by bd `babelstone-t7o3.19`.)

### A11 · On the full-saga path the SCA freshness re-check moves to the irreversible settlement leg (refines §A10)

Once [ADR-PC-032](../../product_concepts/adrs/ADR-PC-032-money-movement-primitive.md) makes every `Originated` money-mover append-first — the engine records the `Movement`; its cash leg is a downstream, confirmation-gated `SettlementProcess` step ([ADR-PC-032 slot 5](../../product_concepts/adrs/ADR-PC-032-money-movement-primitive.md)) — the engine's append route is no longer "before any side effect" *for the money*. The §P8 freshness re-check therefore moves to the **settlement-dispatch instant**: the gate sits on the `SettlementProcess` cash leg that effects the movement (`ConfirmDebit` / `ConfirmCredit` against the Core ACL, [ADR-PC-016 slot 5](../../product_concepts/adrs/ADR-PC-016-legacy-current-account-adapter.md)), and refuses fail-closed on absent/stale SCA **before** the irreversible debit/credit.

The attestation model is unchanged from the engine-direct path (§A8): the orchestrator substrate **attests** — it re-emits the gateway-attested `X-SCA-Acr` / `X-SCA-Auth-Time` claims (carried on `saga_outbox` by bd `babelstone-ls44`) onto the cash leg's delivery — and the **receiver denies**. The substrate itself never denies: that is the attest-not-deny posture of [ADR-IC-006 §P2](./ADR-IC-006-edge-api-gateway.md), and the orchestrator stays an extraction-ready, family-agnostic substrate ([ADR-PC-019 §P2](../../product_concepts/adrs/ADR-PC-019-repository-strategy-monorepo.md) / [ADR-IC-018 §D2](./ADR-IC-018-family-owned-saga-modules.md)) that carries operational columns, not domain policy. The engine remains the freshness authority **only** for money-movers that stay engine-direct; it is not the authority for a leg whose cash it no longer effects.

Freshness is re-checked **at the dispatch instant, not inherited from saga entry**: the saga duration ceiling (~1800 s, [ADR-IC-006 §P4](./ADR-IC-006-edge-api-gateway.md)) is 6× the `SCA_MAX_AGE` window (300 s), so a proof fresh when the saga started can be badly stale when the cash leg fires. The Kong edge SCA gate on saga *entry* (the constitution route, [ADR-IC-006 §P2](./ADR-IC-006-edge-api-gateway.md)) guards admission, not the cash instant, and does not substitute for the at-dispatch re-check.

The populate point relocates with the gate: ADR-PC-032's settlement design companion ([§4.2](../../product_concepts/feature-design-money-movement-settlement.md)) makes the standalone money-movers (maturity / coupon) **event-auto-started**, so there is no edge-originated saga-command write to carry the OIDC claims in (the seam bd `babelstone-ls44` fills via the command sink). On an auto-started leg the attested `acr` / `auth_time` instead propagate forward on the **Movement-bearing event's CloudEvents headers** from the original step-up, so the settlement leg can re-check them — specified by ADR-PC-032's companion amendment (its §A7–§A8) and built by bd `babelstone-t7o3.15`, governed by bd `babelstone-t7o3.19`.

### A12 · This amends §A10's realisation; it does not supersede this ADR

The Decision (Python SDK, Streamable HTTP, behind Kong, reuse the IAM) and §P1–§P9 remain binding as written. §P8's principle — the irreversible action transitions on the bank's own AS-signed signal a courier cannot forge, before any side effect — is **unchanged**; this amendment is what keeps it true once the cash leg leaves the engine. It refines only §A10's realisation note (that the saga path threads "to the same engine gate"), whose assumption that the engine stays the money-mover is now retired for the full-saga path. The engine-direct realisation (A7–A9) is **unaffected**: where a money-mover is still engine-direct, the `ScaPrecondition` gate is still the correct, `Live` enforcement point.

Catalogue posture: `MCP_SCA_GATE_CANNOT_BYPASS` stays `Live` for the engine-direct path (its realisation in the engine `ScaPrecondition` gate is unchanged). The **saga-path** realisation — the same no-client-side-bypass invariant enforced at the `SettlementProcess` cash leg — is `Planned`, tracked by bd `babelstone-t7o3.19` (depends on bd `babelstone-t7o3.15`), and registers as its own catalogue Test ID when that gate lands, the same "Planned until the implementing issue lands its gate" posture ADR-PC-032's Verifiable-commitments take. The 2026-05-31 (A1–A4), 2026-06-15 (A5–A6), and 2026-06-20 (A7–A10) amendments are otherwise unaffected.

---

## Amendment — 2026-06-24: §A8's "production hardening" note is realised — the refreshed step-up token is now RFC 8705 mTLS-bound (sender-constrained)

In plain English: when an AI agent completes a step-up strong-authentication challenge and retries an irreversible money-mover, the bank now hands it a token that is *tied to the agent's own connection*, not a plain ticket anyone could replay. If that token is stolen and presented from a different machine, the gateway notices the mismatch and rejects it — the stolen ticket is worthless to the thief. This realises the production-hardening note §A8 already anticipated ("Sender-constraining the refreshed token (RFC 8705 mTLS-bound, or RFC 9449 DPoP) is the production hardening the MCP edge's existing mTLS posture (§P5, `CERT_REQUIRED`) already anticipates"). It is **additive**: §A8's principle — the refreshed proof re-enters as a bank-signed `acr`/`auth_time` Kong attests to the engine — is **unchanged**; this amendment names the concrete sender constraint (RFC 8705 mTLS-bound, **not** RFC 9449 DPoP) that the §P5 `CERT_REQUIRED` mTLS posture made the natural choice, and the attestation that carries the confirmed binding through to the engine. Realised in bd `babelstone-26rb`.

### A13 · The refreshed step-up token is sender-constrained via RFC 8705 mTLS-binding (chosen over DPoP)

The refreshed step-up access token carries an RFC 7800 `cnf` (confirmation) claim with an `x5t#S256` member — the base64url SHA-256 thumbprint of the client certificate the holder presents on the mutually-authenticated connection (RFC 8705 §3, *Mutual-TLS Client Certificate-Bound Access Tokens*). This binds the token to that certificate: a token replayed from a different sender presents a different certificate, whose thumbprint does not match the token's `cnf`, and is rejected.

**Why mTLS-binding, not DPoP (§A8's other option).** The MCP edge already runs the Kong→MCP hop over mutual TLS — uvicorn requires a client certificate (`ssl_cert_reqs=CERT_REQUIRED`) and Kong presents one with `tls_verify: true` (§P5, ADR-IC-006 §P5 Boundary 2, completed bd `babelstone-29ic`). RFC 8705 reuses exactly that already-deployed certificate as the sender constraint — no new proof-of-possession header, no per-request DPoP signature for the agent to compute, no new key-management surface. RFC 9449 (DPoP) is the right choice when the channel is *not* already mutually authenticated (a bearer-over-TLS public client); here the channel **is** mutually authenticated, so mTLS-binding is the lower-surface realisation of the same anti-replay property. DPoP remains the documented alternative if a future deployment fronts the MCP server without client-certificate mTLS.

### A14 · Kong validates the binding and attests it; the engine accepts the attested binding (attest-not-deny)

The binding is **enforced at the gateway** and **accepted at the engine**, the same Boundary-2 attest-not-deny split (ADR-IC-006 §P2) that governs `X-Client-Id` (§P3) and `X-SCA-Acr`/`X-SCA-Auth-Time` (§A8). On the `/mcp` route Kong validates the token's `cnf.x5t#S256` against the presented client certificate's thumbprint; a token whose `cnf` is present but does **not** match is `401`'d before the request reaches the MCP server (a stolen token replayed from a different sender never arrives) — the replay defence. On a match (or a `cnf`-less POC-legacy plain Bearer, accepted as *unbound*) Kong **attests** the confirmed thumbprint to the upstream as the `X-SCA-Cnf-X5t` header, overwriting any client-supplied value (the same anti-spoof `set_header` pattern as `X-Client-Id`). The MCP app surfaces that attested binding on its `AuthContext` (`sender_bound` / `is_sender_constrained`) and the engine's `ScaPrecondition` recognises the `X-SCA-Cnf-X5t` header as the gateway-confirmed binding — it **accepts** the attested binding as additive context and never re-derives it from the token. The freshness gate is **unchanged** by the binding: a fresh, gateway-attested SCA proof passes whether the token was sender-constrained or a plain Bearer.

### A15 · This amends §A8's realisation; it does not supersede this ADR

The Decision (Python SDK, Streamable HTTP, behind Kong, reuse the IAM) and §P1–§P9 remain binding as written. §A8's principle — the refreshed step-up proof re-enters as a bank-signed `acr`/`auth_time` Kong validates and attests to the engine, which settles only on it — is **unchanged**; this amendment realises §A8's own "production hardening" note by naming the concrete sender constraint (RFC 8705 mTLS-binding) and the `X-SCA-Cnf-X5t` attestation that carries the confirmed binding through the chain. The POC realisation uses the same throwaway MCP client certificate the §P5 mTLS posture already commits (`infra/kong/kong.yml`'s `a1b2c3d4-…` cert, CN=`babelstone-mcp-client-poc`); the stub authorization server (`infra/stub-as/mint-stepup-token.sh`) mints the `cnf` from that certificate's live-computed thumbprint, so the binding tracks the committed cert and a deploy-time `deck sync` that swaps the cert tracks both. The 2026-05-31 (A1–A4), 2026-06-15 (A5–A6), 2026-06-20 (A7–A10), and earlier 2026-06-24 (A11–A12) amendments are unaffected. No new catalogue Test ID is introduced: the sender constraint hardens the existing `MCP_SCA_GATE_CANNOT_BYPASS` posture (a replayed token is now rejected at the gateway in addition to the engine's freshness gate) rather than asserting a new commitment; its end-to-end runtime lock rides the existing `make mcp-contract-test` harness (bd `babelstone-5ot0`) as that harness gains a sender-mismatch case.

---

## Amendment — 2026-07-08: §A8's "OIDC `acr`" is realised against the deployed Logto — no native `acr`; step-up *strength* is a synthesised non-`acr` claim, *freshness* rides Logto's native `auth_time`

In plain English: when we actually stood up the real authorization server (Logto) and looked, it turns out Logto does **not** emit the OIDC `acr` claim §A8 assumed it would — the "how strong was this login" marker. It *does* emit `auth_time` (the "how fresh is this login" marker), which is the part the step-up gate actually leans on. So the step-up story splits cleanly: **freshness** works out of the box on Logto's native `auth_time` (no change), while **strength** now has to be *manufactured* — Logto reads which factors (passkey/TOTP) were used during sign-in and stamps a custom claim, under a name that is **not** `acr` (a claim literally named `acr` is silently dropped by Logto's OIDC core as a built-in collision). This is the ADR-IC-021 §C4 "Logto `acr` maturity is the watch-item" resolving — negatively, but harmlessly, because the gate never depended on the strength marker to *move money*: it depends on freshness, which is native. Empirically surfaced by the IAM security verification (bd `babelstone-zla1.10.5`, slice 2).

### A16 · The deployed AS (Logto) emits no native OIDC `acr`; §A8's step-up proof is realised as native `auth_time` (freshness) + a synthesised non-`acr` claim (strength)

The live Logto discovery document (`https://auth.babelstone.dev/oidc/.well-known/openid-configuration`, verified 2026-07-08) reports `acr_values_supported` **absent**, `acr` **absent** from `claims_supported`, and both `request_parameter_supported` and `claims_parameter_supported` `false` — so neither an `acr_values` request nor an essential-`acr` claim request is available on this deployment. `auth_time` **is** in `claims_supported`. This is a direct property of Logto's engine (`panva/node-oidc-provider`, the OpenID-certified core Logto runs on per [ADR-IC-021](./ADR-IC-021-iam-oauth-authorization-server.md)): its `acrValues` config defaults to `[]`, which disables the `acr` claim and removes `acr_values_supported` from discovery, and Logto exposes no configuration that maps "MFA completed" onto an `acr` string. §A8's "refreshed access token carrying an OIDC `acr`" is therefore **not natively obtainable** from the real AS. It is realised as two separable legs:

- **Freshness (load-bearing, unchanged).** Logto's native `auth_time` is emitted and is genuinely refreshed by a real re-authentication (`prompt=login` / `max_age`, honoured by the node-oidc-provider default interaction policy). Kong's `/mcp` SCA `pre-function` and the engine's `ScaPrecondition` already gate on `X-SCA-Auth-Time` against `SCA_MAX_AGE` (300 s); this leg needs **no change** and is the load-bearing SCA signal. `MCP_SCA_GATE_CANNOT_BYPASS` (the freshness gate) stays `Live` and AS-agnostic.
- **Strength (synthesised, non-`acr` name).** The `acr`-equivalent strength marker is produced by Logto's custom-JWT-claims feature (`getCustomJwtClaims`), whose access-token `context` exposes the current sign-in's `interactionEvent.verificationRecords` (`Totp` / `WebAuthn`). The script reads those and emits a **custom claim under a name that is not `acr`** — Logto's built-in claims "cannot be overridden or modified; conflicting custom claims are ignored", and `acr` is a node-oidc-provider built-in, so a claim literally named `acr` is dropped. Kong's SCA `pre-function`, which today reads a claim named `acr`, is repointed to read that custom name (a `kong.yml` pre-function edit only — no edge-contract or route change, the same class of deploy-time repoint as the [ADR-IC-006 §P7](./ADR-IC-006-edge-api-gateway.md) issuer/JWKS swap). The synthesised claim lands on the **access token only** (not the id_token) and only on a **fresh** interactive grant (not a silent `refresh_token`), which is exactly what the §A7 engine-`422`→re-auth→refreshed-token loop already forces.

### A17 · This amends §A8's realisation; it does not supersede this ADR

The Decision (Python SDK, Streamable HTTP, behind Kong, reuse the IAM) and §P1–§P9 remain binding as written. §A8's principle — the irreversible action transitions **only** on a bank-signed, Kong-attested, un-forgeable step-up proof, **before any side effect** — is **unchanged**; a courier agent still cannot forge it. What this amendment refines is the *concrete claim* that carries §A8's proof against the **real** AS: step-up **freshness** on Logto's native `auth_time` (mechanism unchanged), and step-up **strength** on a **synthesised non-`acr`-named claim** rather than a native OIDC `acr`, with the matching `kong.yml` pre-function read-name repoint. This resolves the ADR-IC-021 §C4 "Logto `acr` maturity" watch-item (`IAM_SCA_ACR_AUTH_TIME`): native `acr` is confirmed **absent**, and the commitment is honoured by the `auth_time`-freshness leg (Live via the existing engine/orchestrator gates) plus the synthesised-strength leg. The **currently wired** `acr` in the POC / CI path is minted by the stub authorization server (`infra/stub-as/mint-stepup-token.sh`) signing a literal `acr` with the committed POC key — so `MCP_SCA_GATE_CANNOT_BYPASS` is proven and `Live` **independent of Logto**; the synthesised-claim path (the `getCustomJwtClaims` script deployed to Logto + the Kong read-name repoint) is the real-Logto realisation, wired when Boundary-6/9 go fully live and tracked by bd `babelstone-zla1.10.5`. No new catalogue Test ID is introduced. The 2026-05-31 (A1–A4), 2026-06-15 (A5–A6), 2026-06-20 (A7–A10), and 2026-06-24 (A11–A12, A13–A15) amendments are unaffected.

## Amendment — 2026-07-12: a TEMPORARY, default-off demo-BFF SCA bypass (`MC_SCA_BYPASS`) lets the Mission Control gateway stand-in mint the step-up attestation in oidc/staging until the interactive step-up flow is built

In plain English: in the Mission Control demo, pressing "Mature deposit" (or paying a loan installment) triggers an irreversible money-mover the engine refuses — `422 SCA_REQUIRED` — unless it sees a fresh, bank-attested step-up proof. The demo's little proxy (`serve.py`) already stands in for the bank gateway and mints that proof on a laptop (dev mode). But in the deployed staging demo it deliberately did **not** — so every "Mature" button there fails, because the real in-browser step-up challenge that would mint a genuine proof **is not built yet**. This amendment records a stop-gap: an explicit, off-by-default `MC_SCA_BYPASS` flag lets that same demo proxy mint the proof in staging too, so the demo's money-movers work end-to-end. It is a **deliberate, self-documenting, time-boxed hole confined to the demo host** — the engine's real fail-closed gate is untouched, the flag must be opted into (never reached by accident), and it is to be removed the moment the interactive step-up flow ships. It is **additive**: §P8's principle — the irreversible action settles only on a bank-signed proof, before any side effect — is unchanged in the engine; what changes is only *which demo-host modes* the gateway *stand-in* is allowed to mint that proof in. Surfaced by bd `babelstone-13zf` (follow-up to the dev-mode injection, bd `babelstone-e4mq`).

### A18 · The demo BFF may mint the gateway SCA attestation in oidc mode behind an explicit, default-off `MC_SCA_BYPASS` flag — a demo-host-only, time-boxed scaffold

Mission Control's `serve.py` is the demo's gateway stand-in: on the engine `/v1/*` arm it mints a step-up token (`infra/stub-as/mint-stepup-token.sh`, the same POC key path §A13 commits) and attests `X-SCA-Acr` / `X-SCA-Auth-Time` to the engine exactly as Kong would, so the five gated money-movers do not `422`. Per bd `babelstone-e4mq` that minting was confined to **dev mode** (the `AUTH is None` branch); in **oidc mode** — the staging deployment fronting `auth.babelstone.dev` — it minted nothing, because forging a gateway attestation against a real deployment is a hole. That fail-safe default is **retained**. What this amendment adds is a single, explicit, off-by-default escape hatch: with `MC_SCA_BYPASS` set, `serve.py` mints the **same** demo attestation in oidc mode too. The staging overlay (`infra/k8s/overlays/staging/mission-control.yaml`) opts in.

This is a **scaffold, not a new posture**, and it is bounded on every axis:
- **Time-boxed.** It exists only because the interactive in-UI step-up flow (a browser SCA challenge that mints a *genuine* refreshed proof — the §A7→§A8 loop realised for the *human at the browser* rather than the *agent*) is **not built yet**. It is to be deleted when that flow ships; the flag name, the staging manifest env, and this amendment are the removal checklist, and its removal is tracked by bd `babelstone-aggc` (which builds the interactive step-up flow and retires this scaffold) so the time-box is enforced by a tracker entry, not this amendment's prose alone.
- **Route-bounded.** The bypass widens only the *mode* axis (dev-only → oidc too); it does **not** widen the *route* set. Injection stays gated by the unchanged five-money-mover regex — an ungated `/v1/*` write (settlement-ingress reserve/debit/credit, the current-account authorise) and every read still receive nothing.
- **Host-bounded.** The mint script signs over the throwaway `infra/kong/kong.yml` POC key (§A13), invalid the moment that key is replaced at deploy time, and the flag is wired ON only in the demo/staging overlay. It is not, and cannot silently become, a production posture.
- **Opt-in.** The safe state (no oidc injection) is the default; the convenience must be opted into — the same fail-safe inversion `MC_ALLOW_UNAUTHENTICATED` uses (ADR-IC-021 rollout / bd `babelstone-zla1.10.8.2`).

### A19 · This amends the demo-BFF realisation of §A8; it does not supersede this ADR

The Decision (Python SDK, Streamable HTTP, behind Kong, reuse the IAM) and §P1–§P9 remain binding as written. §P8's principle — the irreversible action transitions only on a bank-signed, un-forgeable step-up proof, before any side effect — is **unchanged in the engine**: the `ScaPrecondition` gate is untouched and still `422`s a money-mover with no fresh attestation. `MCP_SCA_GATE_CANNOT_BYPASS` stays `Live` — it governs the *engine* gate against the *agent-as-courier*, which this amendment does not touch. What this amendment records is a deliberate, explicit-drift-gated ([ADR-PC-020 §D3](../../product_concepts/adrs/ADR-PC-020-llm-toolchain-and-conformance-governance.md)) divergence confined to the **demo BFF**: the Mission Control gateway stand-in may, behind the default-off `MC_SCA_BYPASS`, mint the demo attestation in oidc/staging as a temporary scaffold until the interactive step-up flow lands (bd `babelstone-13zf`). No new catalogue Test ID is introduced — the demo-mint seam asserts no production commitment; its behaviour is locked by `docs/demo/mission-control/test_serve_auth.py` (the default-off fail-safe and the opt-in oidc widening each have a test). The 2026-05-31 (A1–A4), 2026-06-15 (A5–A6), 2026-06-20 (A7–A10), 2026-06-24 (A11–A12, A13–A15), and 2026-07-08 (A16–A17) amendments are unaffected.

## Amendment — 2026-07-12: the demo-BFF SCA attestation (§A18) is synthesised in-process — the Mission Control container has no mint script, so it sets the two headers directly

In plain English: the previous amendment (A18/A19) turned on the staging SCA bypass, but the demo still failed — "Mature failed (422): Strong Customer Authentication … is required" — when a deposit was matured against the deployed `app.babelstone.dev`. The reason: the way the demo proxy "mints" the proof is by running a shell script (`infra/stub-as/mint-stepup-token.sh`) that signs a throwaway token. That script works on a developer laptop, where the whole repo is on disk, but it is **not inside the Mission Control container image** — that image ships `serve.py` + `index.html` (plus a lone `psycopg` dependency) — no mint script, no signing key, no `bash`/`openssl`. So in staging the mint step failed, the proxy relayed the request without the proof, and the engine correctly `422`'d. This amendment records the fix: the demo proxy now **synthesises the two attestation headers directly in Python** (a non-empty `acr` + a fresh `auth_time`) when the mint script is unavailable, instead of shelling out. It is **additive**: §P8's principle and the engine's `ScaPrecondition` are still unchanged; only *how the demo BFF produces the two headers it already sends* changes. Surfaced and fixed by bd `babelstone-uob5`.

### A20 · The demo BFF synthesises `X-SCA-Acr` / `X-SCA-Auth-Time` in-process when the mint toolchain is absent (the container norm), which is security-equivalent to the minted path

A18 described the demo BFF minting the attestation via `infra/stub-as/mint-stepup-token.sh` (the §A13 throwaway-POC-key path). That subprocess is a **local-dev nicety, not a runtime dependency**: the Mission Control image (`docs/demo/mission-control/Dockerfile`, build context `docs/demo/mission-control`, base `python:3.12-slim`) copies `serve.py` + `index.html` (plus a lone `psycopg` dependency), so the mint script, the signing key, and `bash`/`openssl` are all absent in the deployed container — the mint always fails there. `serve.py` therefore now **prefers** the minted token when the toolchain is on disk (local dev, unchanged) and **falls back to an in-process synthetic attestation** otherwise: it sets `X-SCA-Acr` to the exact acr the wired step-up mint emits (`urn:bank:sca:psd2` — the strong PSD2 level `infra/stub-as/mint-stepup-token.sh` forces, so a synthesised proof is byte-identical to a real mint's on this path) and `X-SCA-Auth-Time` to the current unix time.

This is **security-equivalent** to the minted path, not a weakening, because on the engine-direct path (`ENGINE_URL=https://engine:8080`, no Kong between the BFF and the engine) the engine's `ScaPrecondition.Check` reads the two headers **directly** — it verifies only that `acr` is non-empty and that `auth_time` is a fresh unix timestamp inside the 300 s window, and it **never validates a token** (attest-not-deny, [ADR-IC-006 §P2](./ADR-IC-006-edge-api-gateway.md)). A synthesised header pair is thus indistinguishable to the engine from a decoded-real-token pair, and the "throwaway key ⇒ tokens invalid at deploy" defence-in-depth argument A18 invoked applied only to a path the engine never exercises on this arm. Both paths equally rest on the load-bearing guard that is **unchanged**: the injection happens only in dev mode or behind the explicit default-off `MC_SCA_BYPASS`, and only on the five money-mover routes.

### A21 · This amends §A18's realisation; it does not supersede this ADR

The Decision (Python SDK, Streamable HTTP, behind Kong, reuse the IAM) and §P1–§P9 remain binding as written. §P8's principle — the irreversible action transitions only on a bank-signed, un-forgeable step-up proof, before any side effect — is **unchanged in the engine**: `ScaPrecondition` is untouched and `MCP_SCA_GATE_CANNOT_BYPASS` stays `Live` for the engine-direct path against the agent-as-courier. This amendment refines only the **demo BFF's realisation** of §A18: *how* the Mission Control gateway stand-in produces the two headers it attests (in-process synthesis when the mint script is absent, which is always the case in the container), keeping the temporary `MC_SCA_BYPASS` scaffold actually functional in staging until the interactive step-up flow lands (bd `babelstone-aggc`). No new catalogue Test ID is introduced — the synthesis is locked by `docs/demo/mission-control/test_serve_auth.py` (the mint-unavailable fallback and the oidc-bypass-without-a-mint-script regression each have a test). The 2026-05-31 (A1–A4), 2026-06-15 (A5–A6), 2026-06-20 (A7–A10), 2026-06-24 (A11–A12, A13–A15), 2026-07-08 (A16–A17), and the earlier 2026-07-12 (A18–A19) amendments are unaffected.
