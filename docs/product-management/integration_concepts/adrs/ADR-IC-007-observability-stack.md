...
*Added 2026-06-14 (bd babelstone-2dex): Surfacing the trace id to the caller (synchronous HTTP boundary).* The engine's HTTP host (`Babelstone.Engine.Api`) wires `AddAspNetCoreInstrumentation()`, so each inbound request is a SERVER span: when the caller sends a W3C `traceparent` that span adopts its trace id (the Layer 1 join), and the manual `deposit.*` spans — children of `Activity.Current` — nest under it instead of starting as disconnected roots. The host then hands the active trace id back to the caller on every response (command and query) via an **explicit `X-Trace-Id` response header** carrying the bare 32-hex trace id. This is the chosen mechanism over the W3C `traceresponse` header (Trace Context Level 2): the immediate consumer is a browser (Mission Control's Telemetry tab, bd babelstone-f0ic.9) that queries Grafana Tempo by trace id and needs the bare id, not the `00-<trace-id>-<span-id>-<flags>` framing it would have to re-parse. The trace id is an opaque operational identifier, never a subject reference (§P4 / ADR-PC-004 §P2). Scope is the synchronous HTTP boundary only — orchestrator OTel wiring and `traceparent`-across-Redpanda propagation (`OBS_TRACEPARENT_PROPAGATION`, §P1) remain Planned.
...
*Revised 2026-06-24 (bd njt2.9–2.11, ADR-PC-020 §D3 explicit-drift): the classification + call-site-comment + code-review discipline above is a **design-time, cultural** control, and the OTel Collector (§P1) was the only runtime transform this ADR named. A **runtime emit-time enforcement** control now joins them. `AddBabelstonePiiGuard` (`Babelstone.Telemetry.Hosting`) registers a `BaseProcessor<Activity>` + a `BaseProcessor<LogRecord>` + a metric-`View` `TagKeys` allowlist on every Babelstone host's trace / log / metric provider, and strips any attribute whose key is outside the admitted `babelstone.*`/semantic-convention operational tier **before export** — emitting a corrective `telemetry_pii_attributes_stripped_total` counter so a strip is never silently fail-open. This is the **load-bearing** leg of `OBS_NO_PII_ATTRS` (catalogue OBS-3): every real attribute is runtime-valued, so it is invisible both to the cultural review and to the build-time BENG005 analyser (which therefore stays a secondary tripwire, never the gate). The classification discipline above remains in force — the runtime guard is defence-in-depth over it, not a replacement: an admitted key still carries the tier its author chose, and read-side Grafana RBAC (§P4/§P6) still governs who may query it. The guard classifies attribute KEYS, not values inside an admitted namespace; the auto-instrumentation is configured value-PII-safe by default (no `db.statement`, no `url.query`/headers — see `BabelstoneNpgsqlInstrumentation`).*
...
| `OBS_NO_PII_ATTRS` | No PII in any telemetry signal — only the `babelstone.*`/semantic-convention operational tier across span tags, log fields, and metric dimensions; money as integer cents. Enforced at emit by the runtime guard (`AddBabelstonePiiGuard`, §P4 Revised 2026-06-24) — the load-bearing leg — with the BENG005 analyser a secondary build-time tripwire. | Live |
