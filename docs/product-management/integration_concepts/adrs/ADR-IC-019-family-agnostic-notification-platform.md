...
*Revised 2026-07-02: the family-agnostic-core + composition-by-discovery covenant this Decision instantiates for the notification estate is now also stated once, cross-cuttingly, in [ADR-PC-040](../../product_concepts/adrs/ADR-PC-040-family-agnostic-substrate-covenant.md), which adds the default-deny universal backstops (`FAMILY_TO_CORE_DEFAULT_DENY` / `COMPOSITION_ROOT_NAMES_NO_FAMILY`) behind this ADR's NOTIF-1 gate — the §D4 host exemption becomes the explicit `<BabelstoneRole>CompositionRoot</BabelstoneRole>` marker on `Babelstone.Notification.Host` — and relocates `NotificationModuleLoader`'s scan/activation mechanics onto the shared `FamilyModuleScanner` (`Babelstone.Composition`; initially landed in `Babelstone.Cadence`, relocated 2026-07-03), the loader's public contract unchanged. D1–D5 remain binding as written.*
