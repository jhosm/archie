...
## Amendment — 2026-06-08: `last_updated` is event-derived for an event-sourced read model

Implementing the term-deposit CQRS read surface (D.4, bd babelstone-yfr2) on the in-house event-sourced engine revealed that §P3's `last_updated` definition — *"Wall-clock time of the most recent projection write"* — cannot hold for a read model that must rebuild byte-identically. The engine's read model is fed by a per-stream drainer re-folding pure domain events (no Redpanda offset; the per-stream `sequence_number` is the §P2 offset analog). A `clock_timestamp()`-at-write `last_updated` would make two truncate-and-refold rebuilds disagree, breaking the rebuild-determinism gate ([ADR-PC-002 §P4](../../product_concepts/adrs/ADR-PC-002-application-level-bitemporality.md) / [ADR-PC-010 §P5](../../product_concepts/adrs/ADR-PC-010-dotnet-hand-rolled-engine.md)). This amendment is additive: it refines what value the §P3 column carries on an event-sourced projection; the column itself, its mandatory presence, and its operational purpose (staleness display, read-after-write) remain as §P3 states.

### A1 · For an event-sourced read model, `last_updated` is the producing event's `transaction_time`, not the SQL write clock

When a read model is materialised by re-folding the durable event log (rather than by a broker-consumer projector with a wall-clock-at-write semantics), §P3's `last_updated` is **runtime-supplied from the producing event's `transaction_time`** — event-derived, deterministic, and stamped by the mapper as data, never read from `clock_timestamp()` or a column `DEFAULT`. A truncate-and-refold rebuild ([§P5](#p5--rebuild-is-a-first-class-operation-not-a-break-glass-procedure)) then reproduces every `last_updated` value byte-for-byte ([ADR-PC-002 §P4](../../product_concepts/adrs/ADR-PC-002-application-level-bitemporality.md) / [ADR-PC-010 §P5](../../product_concepts/adrs/ADR-PC-010-dotnet-hand-rolled-engine.md)). Consequently the staleness signal §P4 surfaces from `last_updated` on this engine reads the **age of the world-fact** (how old the most-recent event the row reflects is), not the age of the projector's last write — equivalent under a live, caught-up drainer, and the only definition that survives a rebuild. The broker-fed-projector reading of §P3 (wall-clock-at-write) stays correct for projectors that are *not* re-folding a deterministic log.

### A2 · This amends the decision; it does not supersede this ADR

[§D](#decision) (CQRS read model on the same Postgres tier), [§P1](#p1--projections-live-in-a-dedicated-schema), [§P2](#p2--upsert-with-event-offset-guard-is-the-canonical-projection-write) (UPSERT with the offset/sequence monotonicity guard), [§P4](#p4--projection-lag-is-a-first-class-sli), and [§P5](#p5--rebuild-is-a-first-class-operation-not-a-break-glass-procedure) remain binding as written. A1 is appended to — not a revision of — §P3's mandatory-columns table: `last_updated` is still mandatory and still TIMESTAMPTZ; only the *source* of its value is clarified for the event-sourced case.
