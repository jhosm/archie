...
Implementing the indeterminate-clearance wiring (bd `babelstone-t7o3.10`) revealed the prior amendment's A1 list of v1-synthesized settlement-result events and its two-outcome (2xx/4xx) delivery model are incomplete: the `ConfirmDebit` leg has a **third** terminal outcome — the ACL's explicit `INDETERMINATE` — and a new clearance-query leg (`QueryCoreDebitStatus`) whose own delivery outcome the orchestrator must likewise synthesize into a result event at v1. This is the §P4 first-class-waiting-state principle ("long waits are states, not blocked threads") being *exercised* (it is exactly what `AWAIT_CORE_CLEARANCE` exists for), not a new direction; it is appended to A1 on the identical terms. It also surfaces a genuine divergence from [ADR-IC-012](./ADR-IC-012-anti-corruption-layer-implementation.md) §D5/§P5 on the *not-executed disposition* — recorded below in **A6**.
...
- **`Refused` (4xx = not-executed)** → `DebitNotExecuted`, which fails the saga **closed** to `DEPOSIT_CONSTITUTION_FAILED` with **no reversal** — nothing was committed, so there is nothing to compensate (mirroring the `PreconditionRefused` no-op terminal).

The `4xx = not-executed` encoding is a **v1 stub convention**: the dispatcher's slot-5 model reads a 4xx as a terminal `Refused`, and the WireMock clearance stub reuses that status to mean "the queried debit was not found / not executed" because the v1 ACL has no event channel. This is *not* the slot-5 "the engine refused an illegal/invalid command" semantics; it is a clearance **verdict** piggybacked on the HTTP status. When DEF-1 lands, the real ACL emits **typed clearance events** (a `DebitConfirmed`/`DebitNotExecuted` off its own topic) and both the `INDETERMINATE`-status synthesis and the `4xx = not-executed` convention are removed — additive, not a permanent fork.
...
§Decision and every Implementation Principle (§P1–§P7) remain binding as written; in particular this **implements** §P4 (a long wait is a named first-class state, never a busy retry — "long waits are states, not blocked threads") and **honours** §P5 (reversibility ordering: the irreversible debit, and the late `DebitConfirmed`/`ActivateDeposit` the clearance accepts, are reachable only from `APPROVED`) and §P6 (a clearance that cannot resolve escalates to `HUMAN_INTERVENTION_REQUIRED`, never a swallowed exception). The §Consequences/§P4 bus-resume contract is the steady state and stays true the moment DEF-1's real ACL produces these clearance events; A4 records only the v1 transitional stand-in and its dated back-out (DEF-1 / bd `babelstone-ub9s`). It is appended to the 2026-06-14 amendment (A1) and to §P4/§P5/§P6, not a revision of them.

### A6 · The not-executed disposition diverges from ADR-IC-012's RETRY_PERMITTED at v1

**In plain English:** when the clearance answer is "the debit did *not* execute", the steady-state design (decided in the ACL ADR, ADR-IC-012) says to mark the operation *retry-permitted* and let the saga decide whether to send it again. At v1 nothing exists to send it again — the real ACL state machine that owns that "retry-permitted" status is not built yet — so instead the saga simply fails cleanly (no money moved, nothing to undo). This records that deliberate gap so it is on the record, and names exactly when it closes.

The not-executed clearance verdict (`DebitNotExecuted`) drives `AWAIT_CORE_CLEARANCE → DEPOSIT_CONSTITUTION_FAILED` with **no reissue path**. This **diverges** from [ADR-IC-012](./ADR-IC-012-anti-corruption-layer-implementation.md) §D5 step 5 and §P5 — the canonical indeterminate-clearance protocol, inherited verbatim by [ADR-PC-016](../../product_concepts/adrs/ADR-PC-016-legacy-current-account-adapter.md) §64 — which specify the not-executed disposition as: transition the in-flight operation to `RETRY_PERMITTED` and "let the saga's compensation logic decide whether to reissue (with the same `idempotency_key`)".

**Why the v1 divergence.** `RETRY_PERMITTED` presupposes the ACL-side in-flight state machine (`IN_FLIGHT → CONFIRMED | INDETERMINATE | REJECTED`, then `INDETERMINATE → CONFIRMED | RETRY_PERMITTED`) that ADR-IC-012 §D5 owns — and the outbound client that refuses to re-send except from `RETRY_PERMITTED`. At v1 that machine does **not** exist (the ACL is a WireMock shim), so there is **no reissue producer** to hand the operation to. A not-executed debit committed nothing, and §P4 forbids a busy retry, so the v1 saga-side clearance fails **closed** rather than parking in a `RETRY_PERMITTED` it cannot service. This is conservative and money-safe: it never reverses a debit that never happened, and never blind-reissues.

**Back-out.** DEF-1 (bd `babelstone-ub9s`) lands the real ACL with the ADR-IC-012 §D5 state machine; when it does, the not-executed verdict transitions the operation to `RETRY_PERMITTED` and the saga's compensation logic owns the reissue decision, exactly as ADR-IC-012 §D5/§P5 specify, and this fail-closed disposition is removed. The mirror of this record lives in [ADR-IC-012](./ADR-IC-012-anti-corruption-layer-implementation.md) (Amendment 2026-06-14) so the two ADRs agree. This is a divergence recorded under the explicit-drift gate (ADR-PC-020 §D3), not a supersession of either ADR-IC-003 or ADR-IC-012.
