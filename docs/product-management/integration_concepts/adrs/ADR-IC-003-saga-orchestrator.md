...
Implementing the indeterminate-clearance wiring (bd `babelstone-t7o3.10`) revealed the prior amendment's A1 list of v1-synthesized settlement-result events and its two-outcome (2xx/4xx) delivery model are incomplete: the `ConfirmDebit` leg has a **third** terminal outcome — the ACL's explicit `INDETERMINATE` — and a new clearance-query leg (`QueryCoreDebitStatus`) whose own delivery outcome the orchestrator must likewise synthesize into a result event at v1. This is the §P5 first-class-waiting-state principle being *exercised* (it is exactly what `AWAIT_CORE_CLEARANCE` exists for), not a new direction; it is appended to A1 on the identical terms.
...
§Decision and every Implementation Principle (§P1–§P7) remain binding as written; in particular this **implements** §P5 (a long wait is a named first-class state, never a busy retry) and §P6 (a clearance that cannot resolve escalates to `HUMAN_INTERVENTION_REQUIRED`, never a swallowed exception). The §Consequences/§P4 bus-resume contract is the steady state and stays true the moment DEF-1's real ACL produces these clearance events; A4 records only the v1 transitional stand-in and its dated back-out (DEF-1 / bd `babelstone-ub9s`). It is appended to the 2026-06-14 amendment (A1) and to §P5/§P6, not a revision of them.
