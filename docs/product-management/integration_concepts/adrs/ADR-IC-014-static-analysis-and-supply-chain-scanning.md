...
1. **SAST — CodeQL.** A [`.github/workflows/codeql.yml`](../../../../.github/workflows/codeql.yml) matrix over `csharp` (build-mode `none`) and `go` (build-mode `autobuild`), triggered on PRs and pushes to `main` that touch the relevant source paths, plus a weekly scheduled full scan. Results upload to the Security tab (free on a public repo). Python is listed in the matrix design and turned on when `mcp-server/` gains `.py` source. **Report-only at first** (no `fail-on`, not a required check yet) — see Consequences.
2. **SCA / dependency audit — Dependabot.** A [`.github/dependabot.yml`](../../../../.github/dependabot.yml) covering `nuget` (engine + estate services), `gomod` (`pack-validate`), and `github-actions` (the workflows' own pinned actions), weekly with PR grouping. Dependabot **alerts** are enabled at the repo level.
...
- **CodeQL C# `build-mode: none` coverage.** Buildless extraction trades a little dataflow precision for zero build-toolchain coupling. Accepted for the report-only baseline; if a motivation-relevant query needs a built database, that one language leg can switch to a `manual` build (mise-pinned .NET, as [`ci.yml`](../../../../.github/workflows/ci.yml) does) without changing the rest.
...
> No executable engine-runtime commitments — this decision is realised as CI/CD configuration (CodeQL, Dependabot) and a repository setting (secret scanning + push protection), not as engine code with a fitness function in the [commitment catalogue](../../product_concepts/adrs/commitment-catalogue.md). Its *gates are the workflows and settings themselves*: the presence and green status of the [`codeql.yml`](../../../../.github/workflows/codeql.yml) analysis, the [`dependabot.yml`](../../../../.github/dependabot.yml) ecosystems, and push protection. Making these **required checks** is the bd `archie-j72w` (Q.7) deliverable; when that lands, the security jobs join the [ADR-PC-019](../../product_concepts/adrs/ADR-PC-019-repository-strategy-monorepo.md) §P1 path-scoped required-check set.
...
## Amendment — 2026-06-27: the container-image grype scan runs PER-PLATFORM (amd64 + arm64), not amd64-only

When the container images went multi-arch (bd `babelstone-zla1.8` — a `linux/amd64` + `linux/arm64` manifest list, because the Hetzner CAX41 staging node is ARM64), the §A2 report-only grype SBOM scan in [`image-build.yml`](../../../../.github/workflows/image-build.yml) quietly lost half its coverage: syft/grype resolve a bare multi-arch **index** reference (`IMAGE@<index-digest>`) to the runner-host child manifest — `linux/amd64` on the `ubuntu-latest` runner — so a single scan inventoried **only the amd64 layers** and silently skipped the **arm64 child that actually runs in staging** (bd `babelstone-oq9z`). The SBOM/scan now run **once per platform**, pinning syft with `--platform linux/amd64` and `--platform linux/arm64` (the `anchore/sbom-action` `platform` input), each emitting its own CycloneDX SBOM artifact and uploading a per-arch SARIF category (`image-grype-<service>-amd64`, `image-grype-<service>-arm64`).

**This adds no gate — it restores scan visibility.** The blocking SCA layer (§A1 — NuGetAudit, govulncheck, dependency-review-action) is **source/graph-based and architecture-independent**: it gates on the resolved dependency graph, which is identical across the amd64 and arm64 image builds (same package versions, same module graph). The grype image scan remains **report-only** (`fail-build: false`, §A2) — DORA supply-chain evidence + Security-tab defense-in-depth over the *published runtime layers* — for both arches. So this amendment widens the §A2 scan's reach to the arm64 child without touching the §A1 gate, the §F1 public-repo cost premise, or the report-only posture. Inventorying the arm64 child is static layer inspection (syft pulls + unpacks the arm64 blobs from the registry), so it needs no QEMU/emulation on the amd64 runner. The thin pack-baked `engine-staging` derived image still omits its own scan by design (a FROM+COPY of static YAML over the already-per-arch-scanned base engine image — unchanged here).

---

## Amendment — 2026-07-08: the container images (and their grype scan) are amd64-only again — the arm64 leg is dropped

The staging cluster landed on a Hetzner **CPX42 (x86/amd64)** node, not the planned CAX41 ARM box — the CAX ARM instances were out of stock in all three locations at provision time (bd `babelstone-zla1.2`). Since nothing runs on ARM, [`image-build.yml`](../../../../.github/workflows/image-build.yml) now builds **`linux/amd64` only** (bd `babelstone-zla1.13`): the `linux/arm64` manifest leg, the QEMU/binfmt emulation step it required (the slowest CI step), and the per-platform SBOM/scan fan-out are all removed. The §A2 report-only grype SBOM scan therefore runs **once**, over the single `linux/amd64` child, under one SARIF category (`image-grype-<service>`) — the per-arch split introduced by the 2026-06-27 amendment above is collapsed back to single-arch.

**This reverses the 2026-06-27 amendment's per-platform scan, and nothing else.** That amendment existed only to keep the arm64 child (which then ran in staging) visible to the scan; with no arm64 artifact published, there is no arm64 child to lose coverage of, so the visibility concern it addressed no longer applies. Everything the amendment left untouched stays untouched here too: the §A1 blocking SCA layer is architecture-independent and unaffected; the §A2 scan stays **report-only** (`fail-build: false`); the §F1 public-repo cost premise is unchanged (a single-arch scan is strictly cheaper). **Tradeoff (bd `babelstone-zla1.13`):** dropping arm64 means staging cannot move back onto a CAX ARM box without re-adding the arm64 build+scan leg — accepted because the x86 pivot is the current reality, the emulated arm64 build was the slowest CI step, and re-adding is a small, well-understood revert of this amendment if ARM stock returns.

---

## Amendment — 2026-07-09: third-party Kubernetes image digests are maintained by a hand-rolled audit workflow (Dependabot's docker ecosystem cannot see them; Renovate stays rejected)

**In plain English:** we started pinning the third-party images the Kubernetes overlays deploy (postgres, kong, redpanda, logto, the k3s control-plane upgrader, …) to immutable digests so a redeploy pulls the exact bytes we validated (that pin is owned by [ADR-PC-007 §A3](../../product_concepts/adrs/ADR-PC-007-signed-yaml-oci-pack.md)). A pinned digest rots as upstream rebuilds a rolling tag, and — it turns out — the tool this ADR chose to keep dependencies fresh, **Dependabot, structurally cannot bump these**. This amendment records the small hand-rolled workflow that does, and confirms Renovate stays rejected.

The Decision named **Dependabot** as the SCA / dependency-audit leg and rejected **Renovate** on operational-simplicity grounds (§"Renovate — rejected (SCA)"). Dependabot's `docker` ecosystem in [`dependabot.yml`](../../../../.github/dependabot.yml) tracks the `FROM` tags in the per-service Dockerfiles. But the ADR-PC-007 §A3 third-party pins live as kustomize `images:` transformer entries (`name`/`newTag`/`digest`) and as a `system-upgrade` Plan's `image:` — **neither of which Dependabot's `docker` updater parses** (it handles Dockerfiles only). So the chosen tool cannot keep these digests fresh, and an unmaintained digest pin rots silently — exactly the staleness this ADR's supply-chain posture exists to prevent.

Renovate's `kustomize` manager *can* bump them — but re-introducing Renovate would reverse this ADR's §S1/§S2 rejection of it (a second dependency bot, the moving part the GitHub-native posture avoids) for a single niche. Instead the maintenance is a **hand-rolled** scheduled workflow, [`cd-thirdparty-digest-audit.yml`](../../../../.github/workflows/cd-thirdparty-digest-audit.yml) + [`scripts/cd-thirdparty-digest-audit.py`](../../../../scripts/cd-thirdparty-digest-audit.py): weekly it re-resolves each pinned tag with `crane` (already pinned in `mise.toml` for [ADR-PC-007 §A2/§A3](../../product_concepts/adrs/ADR-PC-007-signed-yaml-oci-pack.md)) and opens a deliberate, human-reviewed bump PR on drift — the same "propose an update PR, don't auto-merge" posture Dependabot uses, keeping the standing hand-rolled-over-frameworks preference and adding no new external bot.

**This does not change the chosen tools or supersede this ADR.** Dependabot remains the SCA leg for the ecosystems it covers (NuGet, Go modules, GitHub Actions, and the Dockerfile `FROM` tags); CodeQL and secret scanning + push protection are untouched; the §A1 blocking-SCA layer and the §A2 report-only grype scan are unaffected; the §F1 public-repo cost premise is unchanged. This records one additional, narrowly-scoped maintenance leg for a class of artefact — third-party image digests in Kubernetes manifests — that Dependabot cannot reach, and confirms Renovate stays rejected. The reproducibility pin those digests realise is owned by [ADR-PC-007 §A3](../../product_concepts/adrs/ADR-PC-007-signed-yaml-oci-pack.md); this ADR owns only the maintenance mechanism.
