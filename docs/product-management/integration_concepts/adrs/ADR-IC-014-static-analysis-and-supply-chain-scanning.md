...
## Amendment — 2026-06-06: the SCA leg's PR-blocking gate is `dependency-review-action`; the grype SBOM scan stays report-only

The original Decision named **Dependabot** as the SCA / dependency-audit leg and rejected extra in-CI SCA scanners (Trivy / OSV-Scanner) as duplicating what Dependabot surfaces natively (§"Trivy / OSV-Scanner — rejected"). It deferred making the security legs **blocking** required checks to Q.7 (§Decision; §Consequences "Report-only is not yet a gate"; §Verifiable commitments). Wiring that gating (Q.7c, bd `archie-2t16.11`, discovered from the Q.7b triage `archie-2t16.10`) surfaced two facts this amendment records as a decision:

1. **Dependabot alerts cannot block a merge.** Dependabot opens update PRs and raises Security-tab alerts, but no Dependabot mechanism fails a PR check when that PR *introduces* a vulnerable dependency. The blocking realization of the SCA leg is therefore the GitHub-native **`actions/dependency-review-action`** ([`.github/workflows/dependency-review.yml`](../../../../.github/workflows/dependency-review.yml)): on every PR it diffs the dependency **graph** between base and head and fails when an introduced dependency carries an advisory at/above HIGH. It is GitHub-native, reads the same GitHub Advisory Database as Dependabot, and is free on a public repo — so it sits inside the §S1/§S2 GitHub-native posture rationale (one home, one trust boundary, one advisory DB) rather than being the kind of extra third-party scanner the Decision rejected. It is the **companion** to Dependabot (it blocks the PR diff; Dependabot keeps the graph fresh and alerted), not a second SCA surface. It joins the [ADR-PC-019](../../product_concepts/adrs/ADR-PC-019-repository-strategy-monorepo.md) §P1 required-check set as `dependency-review`.

2. **The Q.4 grype SBOM scan stays REPORT-ONLY.** Q.4 (bd `archie-2t16.9`) added a CycloneDX SBOM (syft) + grype scan over the *published .NET assembly tree* — a tool the original Decision did not name (it named **Trivy** for the *container-image* SBOM, and that container leg remains out of scope here). grype CPE-matches framework assembly version strings, which are pinned at `X.0.0.0` across every patch, so it false-positives against advisory *package* ranges: the Q.7b triage dismissed all 35 such findings as false positives, while the authoritative `dotnet list package --vulnerable --include-transitive` reported the solution clean. Making grype blocking would demand perpetual CPE-suppression upkeep for no signal Dependabot + dependency-review do not already provide more precisely. So the grype SBOM scan is retained for its genuine value — the **CycloneDX SBOM as DORA supply-chain evidence** plus a Security-tab defense-in-depth cross-check — and is explicitly **not** a gate. This also closes the previously-unrecorded grype-vs-Trivy/Dependabot tool gap (the Q.4 acceptance criterion: "format/tool is consistent with ADR-IC-014, or the gap is recorded via amendment").

### A1 · This amends the Decision; it does not supersede this ADR

The chosen tools are unchanged: CodeQL (SAST), Dependabot (SCA / dependency audit), GitHub secret scanning + push protection. This amendment (a) names `dependency-review-action` as the PR-blocking realization of the SCA leg, and (b) records that the Q.4 grype SBOM scan is SBOM/DORA evidence + defense-in-depth, report-only by decision, not a blocking gate. The §F1 public-repo cost premise and the report-only-then-blocking staging are unchanged — this is the blocking step the Decision deferred to Q.7.
