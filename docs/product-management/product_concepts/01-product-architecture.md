...
- **The projection is the temporal signal, not a clock-driven event.** A clock advance is not a fact about an instance — nothing happened *to it* when a date arrived. So the engine emits no clock-driven event and runs no internal scheduler: *"this deposit matures in 7 days"* is a downstream read over the maturity-calendar projection, never an emitted `DepositMaturityApproaching`. This keeps handler purity on the *emit path* as well as the fold. Full treatment: [ADR-PC-023](./adrs/ADR-PC-023-temporal-signals-projection-derived.md).
...
Two consequences of "declarative configuration, deployed via API" ([ADR-PC-008](./adrs/ADR-PC-008-rate-sheet-storage-and-deploy-api.md)) are worth naming. First, **product management *is* that deploy pipeline** — authoring, validation-at-commit, versioning, and commercialisation windows — not a runtime imperative product-admin surface; a product-admin UI is downstream tooling on the deploy API ([00 §4](./00-product-vision.md)). Second, a product's **constitution preconditions** — which eligibility verdicts it requires (new-money, salary-domiciliation, mortgage-linked) — are part of that config: the engine *declares* them and *refuses* without them, while upstream *evaluates* them ([ADR-PC-024](./adrs/ADR-PC-024-constitution-precondition-contract.md)).
...
- **Event backbone.** The engine emits and consumes on the bank's event backbone (broker chosen in [ADR-IC-001](../integration_concepts/adrs/ADR-IC-001-event-backbone-message-broker.md)). The engine has a contract with the broker's interface, not an opinion about the broker.
- **Schema format and registry.** Event payloads use the format and registry from [ADR-IC-002](../integration_concepts/adrs/ADR-IC-002-schema-format-and-registry.md). Evolution follows [integration_concepts §09](../integration_concepts/09-long-term-schema-evolution.md).
- **Saga participation.** The constitution flow of a new product instance touches Core Banking + Compliance + CRM + Workflow + Notifications — a saga, not a request. The orchestrator is from [ADR-IC-003](../integration_concepts/adrs/ADR-IC-003-saga-orchestrator.md); the walkthrough is in [integration_concepts §05](../integration_concepts/05-constitution-saga-walkthrough.md). The engine participates as a saga step (commands + compensations); it does not run the saga. Constitution **preconditions** follow the same division of labour: the engine *declares* which verdicts a product requires and the decider *refuses* without them, while the saga *gathers* them from the owning upstream systems ([ADR-PC-024](./adrs/ADR-PC-024-constitution-precondition-contract.md)). (Financial-crime AML/KYC is not an engine concern — it is adjudicated upstream, out of scope per [00 §4](./00-product-vision.md).)
- **Outbox emission.** Every state-changing operation produces a domain event; events leave via the outbox pattern (per [ADR-IC-004](../integration_concepts/adrs/ADR-IC-004-outbox-pattern-mechanism.md), [integration_concepts §04](../integration_concepts/04-plumbing-patterns.md)). Exactly-once-effectively semantics. Event store and outbox are co-located so event-append and outbox-write commit atomically.
- **Anti-corruption layer.** The engine talks to Core Banking through the ACL (per [integration_concepts §02](../integration_concepts/02-anti-corruption-layer.md)). The ACL handles translation, idempotency, indeterminate state, and the rest of its eight responsibilities; the engine sees clean domain primitives.
- **Observability.** OpenTelemetry tracing, structured logs, metrics (per [ADR-IC-007](../integration_concepts/adrs/ADR-IC-007-observability-stack.md), [integration_concepts §06](../integration_concepts/06-observability-and-tracing.md)). The engine instruments product semantics (*accrual computed*, *withholding applied*); the integration layer instruments transport semantics.
- **MCP server exposure.** Commands and queries are exposed to LLM agents via the MCP server (per [ADR-IC-010](../integration_concepts/adrs/ADR-IC-010-mcp-server-runtime-and-sdk.md), [integration_concepts §11](../integration_concepts/11-chat-agent-channel-strategy.md)). Agent-channel access is the same surface as the rest of the bank, gated by the same authorisation.
