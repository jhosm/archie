...
- **Saga participation.** The constitution flow of a new product instance touches Core Banking + Compliance + CRM + Workflow + Notifications — a saga, not a request. The orchestrator is from [ADR-IC-003](../integration_concepts/adrs/ADR-IC-003-saga-orchestrator.md); the walkthrough is in [integration_concepts §05](../integration_concepts/05-constitution-saga-walkthrough.md). The engine participates as a saga step (commands + compensations); it does not run the saga. Constitution **preconditions** follow the same division of labour: the engine *declares* which verdicts a product requires and *refuses* without them, while the saga *gathers* them from the owning upstream systems — financial-crime clearance edge-enforced and engine-oblivious ([ADR-PC-013](./adrs/ADR-PC-013-aml-kyc-upstream-precondition.md)), commercial eligibility decider-enforced from in-command verdicts ([ADR-PC-024](./adrs/ADR-PC-024-constitution-precondition-contract.md)).
