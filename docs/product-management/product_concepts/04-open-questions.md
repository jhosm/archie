...
### 8. Pack-Effective-Date Semantics for In-Flight Contracts — **v1 RESOLVED (pin-at-constitution); PER-PRIMITIVE FORWARD POLICY CONFIRMED, v2+ DEFERRED**

**Resolution.** v1 is **pin-at-constitution**, already decided and built. An instance pins its `pack_version` at `DepositConstituted`; every later flow — accrual, coupon, maturity, withholding — resolves against that pinned pack ([ADR-PC-009 §P5](./adrs/ADR-PC-009-per-instance-version-pinning.md)), and `pack_effective_from` is informational metadata in v1. Within a single pack the fiscal rate is constant, so the flow-by-flow withholding rule ([financial_concepts §5.4](../financial_concepts/banking_products_financial_mathematics.md)) stays exact; the only escape hatch for a mid-life statutory rate change is the explicit, audited, per-instance `PackVersionMigrated` event ([ADR-PC-009 §P3](./adrs/ADR-PC-009-per-instance-version-pinning.md)) — never automatic float. The **forward policy is per-primitive** — instrument-defining primitives (cash-flow shape, day-count, the contracted TAN) pin; regulation-tracking primitives (withholding rate, FGD ceiling, disclosure templates) float by accrual date — already named as the deferred direction in [ADR-PC-009 §P5](./adrs/ADR-PC-009-per-instance-version-pinning.md) and **confirmed here**. Its v2+ deliverable (the per-primitive pin-or-float annotation in the manifest, plus the [03 §Pack Maintenance](./03-roadmap.md) addendum) stays deferred — exactly as §3 defers the bitemporal *mechanism* while the commitment is firm. **F.7** (the full `pt.2026.1` pack) needs no new effective-date machinery. The three options below are retained as the rationale; the worked budget-law example is the mid-life case the `PackVersionMigrated` escape hatch covers in v1 and the per-primitive float will cover automatically in v2+.
...
- **Q-AK. Synthetic v4-scale load test specification — SPECIFIED; tooling RESOLVED; pending operator calibration.** Workload pattern (event mix, daily/monthly/annual peak structure, sync/async projection classification), pass/fail thresholds (latency p50/p95/p99 per projection class, sustained 250 TPS for 24h, burst 1000 TPS for 15min, replay budgets, reliability invariants, no projection-rebuild divergence), test infrastructure (engine-team-owned rig on production-shaped hardware, harness through production APIs, standard observability, every-RC cadence), determinism requirements (seeded RNG, injected clock), and three explicit non-goals (Q-AL sharding, Q-AM backpressure, Q-AN cross-mode invariant) — fully laid out in [two-modes §8](./feature-design-two-modes-asymmetry.md). The **tooling** is decided in [ADR-PC-011](./adrs/ADR-PC-011-in-house-load-test-harness.md): an in-house .NET harness that drives Redpanda + Kong with the engine's own Avro envelope code and measures the §8.3 async latency from OpenTelemetry rather than a generic load tool's send clock (k6/Gatling/Locust/JMeter rejected). Residual is the operator calibration in §8.1 (active accounts `N_acct`, active cards `N_card`, annual event volume `E_year`); spec shape is independent of absolute size. The test is v1 acceptance, not a future deliverable.
...
- **Q-AU. AML / KYC signal contract.** [00 §4](./00-product-vision.md) puts AML and KYC out of scope as products but in-scope as integration shapes. The engine emits a constitution event; an AML system may need to gate or post-flag it. Two credible shapes: (a) constitution completes and AML reviews asynchronously, with a compensation flow if AML rejects; (b) AML pre-gates constitution as a synchronous check on the saga. The two have materially different cancellation semantics and different requirements on the legacy seam ([coexistence](./feature-design-strangler-fig-coexistence.md)). Peer in shape to Q-AB (out-of-scope system, in-scope integration contract).
  - **→ Resolved as out-of-scope (2026-06-03).** AML/KYC is **not an engine concern**: adjudication is upstream, and the integration shape (if AML clearance is enforced at the edge, alongside the PSD2 SCA precondition) belongs to the **integration estate / edge gateway** ([ADR-IC-006](../integration_concepts/adrs/ADR-IC-006-edge-api-gateway.md)), not the product engine — per [00 §4](./00-product-vision.md). [ADR-PC-013](./adrs/retired/ADR-PC-013-aml-kyc-upstream-precondition.md), which had modelled AML as an engine-recorded edge precondition (shape (c)), is **`Withdrawn`**; the product engine carries **no AML surface** (no `aml_clearance_ref` envelope field, no AML gate or compensation). Ongoing AML transaction monitoring remains a post-flag downstream consumer of the engine's existing business-event catalogue ([ADR-PC-012](./adrs/ADR-PC-012-gl-posting-signal-contract.md)) — that consumption needs no engine-side AML contract.
- **Q-AV. Customer-communications emit contract.** Pack-shipped disclosure templates (FIN, SECCI, FINE, maturity notices, annual IRS withholding statements) are specified in [surface §3.4](./feature-design-configuration-surface.md); the *trigger* and *delivery* contract is not. Candidates: engine emits `NotificationDue` events per template-ref and a separate system renders and delivers; pack-side renderer with channel-side delivery; channel-side both. Choice determines whether the engine carries a notification-state projection (sent/acked/bounced) or treats notifications as fire-and-forget.
  - **→ Resolved** by [ADR-PC-014](./adrs/retired/ADR-PC-014-customer-notification-emit-contract.md) (delivery: `NotificationDue` events, no engine-side delivery state; FIN carved out as a synchronous saga gate).
  - **→ Temporal-trigger half refined (2026-06-03)** by [ADR-PC-023](./adrs/ADR-PC-023-temporal-signals-projection-derived.md): the engine emits **no clock-driven `SCHEDULED` notification and runs no internal scheduler**; temporal triggers (the 14-day opt-out reminder, the annual statement) are **downstream reads over the maturity-calendar / accrual-schedule / withholding-ledger projections**. [ADR-PC-014 Amendment A1](./adrs/retired/ADR-PC-014-customer-notification-emit-contract.md) records the narrowing (engine emits `EVENT_DRIVEN` + `PRE_CONTRACTUAL` only).
...
- **Q-AX. Regulatory-reporting inventory per phase — v1 SUBSET RESOLVED.** Q-AE asks who owns *a* reporting application; the prior question — the complete named-report inventory v1–v4 must produce — is unanswered. Candidates so far: modelo 39 (annual IRS withholding statements), BdP Aviso 8/2009 (deposit-rate statistics), BdP central de responsabilidades (credit-bureau reporting, from v2), DGSD 2014/49/EU reporting, FATCA / CRS reporting, BdP estatísticas de taxas de juro. Each needs a per-contract design (signal shape, cadence, completeness guarantees, schema-drift protocol).
  - **→ v1 resolved (2026-06-10, Epic 0.4, bd babelstone-gjyl).** The v1 named-report set is **three returns**, all already declared in `pt.2026.1`: BdP retail-deposit interest-rate statistics (`reporting.yaml: bdp_estatisticas_taxas_juro`, monthly), IRS Modelo 39 (`withholding.yaml: reporting.modelo_39`, annual), and FGD deposit-guarantee coverage (`reporting.yaml: fgd_cobertura_depositos`, annual + on-demand; ceiling in `fgd.yaml`). `bdp_centralizacao_responsabilidades` and `ifrs9_staging` stay `active: false` (v2 credit / DEF-3). Per [coexistence §8](./feature-design-strangler-fig-coexistence.md) the engine emits signals only — a downstream reporting application assembles each return (ownership = Q-AE). Every signal carries **subject references, never PII** — the project-wide no-PII-on-the-durable-bus rule; the reporting application resolves identity internally. The engine-side signal field contract feeding F.7 (bd babelstone-f765): **BdP rate stats** ← `DepositConstituted` (instrument type, agreed rate TANB, principal, maturity/term band, currency, counterparty-sector flag, value-date; monthly volume-weighted new-business rates); **FGD coverage** ← `DepositConstituted` + `DepositMatured` + `DepositTerminatedEarly` running totals (subject reference, eligible balance — principal plus accrued interest, coverage-eligibility flag, scheme `fgd_pt`, value-date; per-depositor balance vs ceiling); **Modelo 39** ← `WithholdingApplied` (beneficiary reference, withholding tax `tax_cents`, net interest `net_cents` — gross = net + tax; value-date; exemption code if any; the 2800 bps rate is the `withholding.yaml` pack constant the assembler applies, not carried on the event; per-beneficiary annual gross income + tax withheld). Still open within v1: the exact Modelo 39 beneficiary field set vs the AT spec, the precise BdP "new-business rate" definition (Aviso 8/2009 / Instrução), and reporting ingestion cutoffs. The v2–v4 reports (central de responsabilidades, DGSD 2014/49/EU, FATCA/CRS) remain open above.
