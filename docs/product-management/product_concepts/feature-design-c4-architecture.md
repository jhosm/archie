...
## Level 2 — Container *(to follow)*

Will decompose the engine box into its runnable pieces and the inherited integration-estate runtimes it plugs into. Expected containers (grounded in the ADRs, for review before drafting):

- **Engine process** — single C# (.NET 9) deployable: command/query API, aggregates, hand-rolled event-sourcing core, the in-process saga state machines, and the projection runtime ([ADR-PC-010](./adrs/ADR-PC-010-dotnet-hand-rolled-engine.md)).
- **PostgreSQL** — event store + outbox + read-model projections + saga state, one database ([ADR-PC-001](./adrs/ADR-PC-001-event-store-technology.md), [ADR-IC-005](../integration_concepts/adrs/ADR-IC-005-cqrs-read-model-storage.md)).
- **Outbox polling publisher** — relays committed events to the broker ([ADR-IC-004](../integration_concepts/adrs/ADR-IC-004-outbox-pattern-mechanism.md)).
- Inherited estate: **Redpanda CE** + built-in SR ([ADR-IC-001](../integration_concepts/adrs/ADR-IC-001-event-backbone-message-broker.md)/[002](../integration_concepts/adrs/ADR-IC-002-schema-format-and-registry.md)), **Kong CE** gateway ([ADR-IC-006](../integration_concepts/adrs/ADR-IC-006-edge-api-gateway.md)), **ACL service** ([ADR-IC-012](../integration_concepts/adrs/ADR-IC-012-anti-corruption-layer-implementation.md)), **Python MCP server** ([ADR-IC-010](../integration_concepts/adrs/ADR-IC-010-mcp-server-runtime-and-sdk.md)), **Notification service** ([ADR-IC-011](../integration_concepts/adrs/ADR-IC-011-async-saga-completion-notification.md)), **OTel Collector → Grafana LGTM** ([ADR-IC-007](../integration_concepts/adrs/ADR-IC-007-observability-stack.md)), and the out-of-process **CUE validator** binary ([ADR-PC-006](./adrs/ADR-PC-006-cue-schema-language.md)).
