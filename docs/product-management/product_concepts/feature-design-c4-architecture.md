...
The **notification service** is a pure choreography consumer of saga terminal events ([ADR-IC-011 D5](../integration_concepts/adrs/ADR-IC-011-async-saga-completion-notification.md)): it owns a PostgreSQL store (subscriptions, delivery log, dead-letter) and delivers HMAC-SHA256-signed, at-least-once callbacks to *pre-registered* subscriber endpoints with exponential backoff. Event contracts are governed offline as Git-native AsyncAPI from source control, rendered by Backstage ([ADR-IC-015](../integration_concepts/adrs/ADR-IC-015-event-catalog-governance-tooling-backstage.md)) — not drawn, because it is not on the runtime path.
