...
**`event_type` shape.** A family event's `event_type` is `<aggregate_type>.<EventName>` (the example above, `term_deposit.DepositConstituted`). The five engine-declared cross-cutting events ([§4.1](#41-cross-cutting-generic-events-engine-declared)) are **family-agnostic** — they belong to no family aggregate — so they take a reserved synthetic aggregate_type **`operations`**, giving the stored `event_type` `operations.<EventName>` (e.g. `operations.PackVersionMigrated`, `operations.AccountFrozen`). The synthetic prefix keeps these events inside the `<aggregate_type>.<EventName>` parsing shape the engine relies on (the relay's reverse-DNS transform and prefix-strip, the handler-registry keying) rather than leaving them bare and unprefixed. Most of these events are **store-only** (they have no governed `.avsc` and never reach the durable bus — [ADR-IC-017 §P1](../integration_concepts/adrs/ADR-IC-017-integration-event-promotion-criterion.md): no named external consumer reacts; the only downstream reaction is engine-internal projection rebuild, [ADR-PC-009 §P3](./adrs/ADR-PC-009-per-instance-version-pinning.md)), so for them this `event_type` is the **stored** contract, not a bus/registry one. A cross-cutting event is **promoted** to the bus when it gains named external consumers — the same [ADR-IC-017 §P4](../integration_concepts/adrs/ADR-IC-017-integration-event-promotion-criterion.md) criterion family events face: `operations.PersonalDataErasureRequested` (GDPR Article 17 erasure, [ADR-PC-004 A4](./adrs/ADR-PC-004-pii-crypto-shredding.md)) is the first, with `acl` and `notification` as consumers, so it carries a governed `.avsc` and registry subject `operations.PersonalDataErasureRequested-value` ([ADR-IC-002 §A3](../integration_concepts/adrs/ADR-IC-002-schema-format-and-registry.md) reserves the single-segment `operations` namespace for exactly this).
