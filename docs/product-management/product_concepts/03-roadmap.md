...
- **Payment moratoria and forbearance.** Government-mandated moratoria (Portuguese *moratória*, English *payment moratorium* — the canonical recent case is *Decreto-Lei* 10-J/2020 during COVID) and bank-initiated forbearance (EBA *forborne exposures*) are a first-class lifecycle event on credit instances. The engine ships the `IN_MORATORIUM` lifecycle state, the bulk-command pattern for operator-initiated application, and the bitemporal retroactivity story for moratoria declared after their operative date. Mechanics: [financial_concepts §7.6](../financial_concepts/banking_products_financial_mathematics.md); engine treatment: [moratoria](./feature-design-moratoria-and-forbearance.md).
