...
| [feature-design-money-movement-settlement](./feature-design-money-movement-settlement.md) | `movement` | Implementation design for [ADR-PC-032](./adrs/ADR-PC-032-money-movement-primitive.md): the `Movement` money-movement primitive — the spine value object, the substrate-owned `Movement`-triggered settlement saga (replacing per-family eager settlement), append-first finality, the eager `ISettlementPort` retired, and the migration of the 8 cash legs. Realises bd `babelstone-5r9n.2`; governs bd `babelstone-5r9n.1` and bd `babelstone-t7o3.13`. |
| [feature-design-c4-architecture](./feature-design-c4-architecture.md) | `c4` | Architecture views of the engine described in [01](./01-product-architecture.md): a C4 model (System Context / Container / Component, PlantUML) for structure, plus behaviour & data views (sequence / state-machine / ER / choreography / data-flow, Mermaid) for what structure cannot show — anchored by the family-vs-engine+platform separation. A view onto decisions already made, not a new decision: every box and line traces to a concept doc, feature-design note, or ADR. Cited by [ADR-IC-013](../integration_concepts/adrs/ADR-IC-013-in-house-estate-build-and-repository-placement.md). |
