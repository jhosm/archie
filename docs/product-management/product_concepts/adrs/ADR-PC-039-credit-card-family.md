...
- [ADR-PC-030](./ADR-PC-030-product-scope-and-boundary.md) — the product scope that names `credit_card` as the open-end revolving asset (roadmap item 3), fixes the boundary (the four-party scheme outside; the engine consumes already-cleared postings), and reversed the build order so the card follows the `conta à ordem` (§P2, amended 2026-06-28).
