...
*Revised 2026-06-20 (bd babelstone-up7t / F.8): the deferred store-side byte corpus has now LANDED. `PackSimulationDepth5Tests` generates `expected-events.yaml` from the engine substrate (the per-event financial cents captured off the store JSON, flow-by-flow withholding), commits it, and asserts the replayed sequences against it field-for-field as a HARD CI gate (`pack.sh` fails loud on a missing/empty/unparseable corpus). The logged-skip placeholder is therefore RETIRED and §P4's "asserts against `expected-events.yaml`" clause is back in literal force — exactly as A2 anticipated ("store-side work that does not need to widen the bus"; the bus-Avro array-of-record limit, [bd babelstone-vcxq](../04-open-questions.md), constrains only the bus payload, never this store-side corpus).*
