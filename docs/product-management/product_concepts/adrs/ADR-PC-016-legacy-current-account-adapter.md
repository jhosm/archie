...
- **Constitution debits are gated, not post-flagged.** The constitution saga blocks on settlement confirmation; `InsufficientBalance` from the Core compensates and emits `DepositConstitutionFailed` with `failure_reason: INSUFFICIENT_FUNDS`, and no credit is gated on funds but every credit is gated on confirmation (slot 5 · Error model). This is the **opposite** stance from the GL/IFRS-9 post-flag family — it must **not** reuse `GL_POST_FLAG_NEVER_GATES`; the falsifiable claim is that the producing flow *is* gated. No Test ID is wired yet.
  - *Resolved 2026-06-14 (bd `babelstone-t7o3.4`): the constitution path is now genuinely gated in code.* The engine previously did the constitution debit ITSELF, eagerly, inside `TermDepositConstitutionService.ConstituteAsync` (the walking-skeleton shortcut acknowledged in [ADR-PC-029 §Residual-risks](./ADR-PC-029-engine-command-ingress.md)) — which contradicted §68/§127 above. That eager in-engine settle is removed: the engine now appends `DepositConstituted` only, and the principal debit is the saga's gated `ReserveAccountBalance`→`ConfirmDebit` step, delivered by the dispatcher (bd `babelstone-t7o3.3`) to the v1 WireMock-stubbed Core ACL (the real ACL is DEF-1, bd `babelstone-ub9s`). Money stays integer cents and the relocated amount/direction is unchanged (the saga's reserve/confirm principal equals the cents the engine used to debit). The §68/§127 claim is now true for the constitution flow; the Test ID is still the pending wiring (the gate is exercised by the gated-settlement integration test against the WireMock ACL — `GatedSettlementWireMockIntegrationTests` — pending its catalogue Test ID under the growth provision). The other lifecycle credits (maturity/coupon/early-termination/renewal) remain eager until their own sagas land.
    - *Revised 2026-06-26 (bd `babelstone-t7o3.13`): the remaining-eager-settle is now CLOSED for term_deposit.* Maturity, the PERIODIC coupon, early termination, and the renewal rollover-debit + ADVANCE-interest legs are relocated off the eager `settlement.SettleAsync` (the `LoggingSettlementPort`) onto the **confirmation-gated substrate-owned settlement saga** (ADR-PC-032 slot 5): each records an Originated `Movement` APPEND-FIRST on its event (maturity/coupon/early-termination a `Credit`, the renewal rollover a `Debit` + the ADVANCE interest a `Credit`), and the substrate saga auto-starts off the promoted `ce_movementorigin`/`ce_movementdirections` headers to effect the cash leg. These credits are CONFIRMATION-gated, not funds-gated (this slot 5); on a clearance failure the leg PARKS in `HUMAN_INTERVENTION_REQUIRED` — no compensation (ADR-IC-003 §P6). `TermDepositConstitutionService` no longer takes an `ISettlementPort` at all. `DepositTerminatedEarly` is promoted to a bus event (its own `.avsc` + AsyncAPI entry) so its payout Movement reaches the saga off the bus; the early-termination HTTP money-mover endpoint is bd `babelstone-t7o3.13.1`. The §68/§127 claim now holds for the WHOLE term_deposit lifecycle, not just constitution.
