...
| 15 | An `engine-ca` leg's `AccountRef` equals the promoted `Movement.AccountRef`, never the `ACCT-{processId}` placeholder (§Payload amendment 2026-07-11) | unit | `SETTLEMENT_LEG_ACCOUNT_REF_PROMOTED` | Catalogued (CA-17, Live) |
| 16 | Two payouts on ONE account carry the SAME `AccountRef` while their intent refs differ — the account-identity and exactly-once axes are orthogonal (§Payload amendment 2026-07-11) | unit | `SETTLEMENT_LEG_ACCOUNT_REF_STABLE` | Catalogued (CA-18, Planned) |
