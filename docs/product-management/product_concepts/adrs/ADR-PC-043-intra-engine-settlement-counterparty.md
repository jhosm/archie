...
*Amended 2026-07-11 (bd `babelstone-u79p.13`, the keystone account-identity design in [feature-design-money-movement-settlement.md](../feature-design-money-movement-settlement.md) §2A): the settlement-command body may now carry a **THIRD** additive field beyond the two §Payload-shape already names (`ce_settlementtarget` header + `amount`) — the **promoted persistent `Movement.AccountRef`** as the credit/debit **destination selector** for an `engine-ca`-targeted leg. This sanctions, on the ADR itself, the account-identity axis §Payload-shape's standing constraint did not yet allow ("only opaque process-id-derived refs … the substrate MUST NOT read `Movement.AccountRef` from the body"), without weakening the exactly-once-key discipline. It is **additive**: the §Payload-shape, §Idempotency, §Semantics, §Ordering, §Error-model, and §Ownership decisions above all remain binding as written; this appends a bounded carve-out to §Payload-shape, it does not revise it. Three rules pin it:*

- *(a) **Destination selector, NOT the exactly-once key.** The promoted `AccountRef` answers "which account does value move against?" and is **orthogonal to** the intent-derived `CoreHoldRef`/`CreditRef` that answers "which economic payout is this?" (the §Idempotency exactly-once dedup key). The two sit **side by side** on the body and are never conflated: the CA-apply `command_id` still derives **solely** from the `IntentId`-derived settlement reference (`DeriveFromIntent`), never from `AccountRef`. The promoted `AccountRef` is the in-band guard against `WRONG-ACCOUNT` — the sibling of the `amount`-field guard against `WRONG-AMOUNT` (§Payload-shape) that every identity-keyed dedup likewise misses. The persistent `AccountRef` is **stable across many payouts** on one customer account (one per account), whereas the intent key is **one per payout occurrence** — the cardinalities differ, which is why one cannot stand in for the other.*
- *(b) **The exact, bounded exception to the substrate-blindness constraint.** The substrate router still selects the counterparty from the `ce_settlementtarget` header **alone** and stays payload-blind for ROUTING ([ADR-IC-018](../../integration_concepts/adrs/ADR-IC-018-family-owned-saga-modules.md) §D5): it does **not** read `Movement.AccountRef` to decide where a leg goes. The carve-out is narrow — on an `engine-ca`-targeted leg the CA `/capture` and `/credit` **writers MAY read** the promoted `AccountRef` from the body as the credit/debit **destination** the source `Movement.Amount` lands on. The value is family-promoted and substrate-**forwarded untouched** (never re-derived), threaded the same way `amount` and the intent reference already are. On the legacy-DDA path the `ACCT-{processId}` placeholder is unchanged (the legacy core resolves the account from the process-scoped business reference); the promoted `AccountRef` is mandatory ONLY on the `engine-ca` legs.*
- *(c) **§D5 / §D3 discipline.** This is an appended, dated [ADR-PC-000 §D5](./ADR-PC-000-namespace-and-contract-shape-framework.md) amendment honouring the [ADR-PC-020](./ADR-PC-020-llm-toolchain-and-conformance-governance.md) §D3 explicit-drift gate — the §Payload-shape decision text above is left verbatim, the carve-out rides here. The three new fitness functions the design names (`SETTLEMENT_LEG_ACCOUNT_REF_PROMOTED` — the `AccountRef` on an `engine-ca` leg equals the promoted `Movement.AccountRef`, never the placeholder; `SETTLEMENT_LEG_ACCOUNT_REF_STABLE` — two payouts on one account carry the same `AccountRef` while their intent refs differ, proving the axes are orthogonal; `SETTLEMENT_TARGET_HEADER_ONLY` — routing reads the header alone) are **Planned** and migrate to the [commitment catalogue](./commitment-catalogue.md) when the producer/ingress build (bd `babelstone-u79p.2`/`.3`/`.4`/`.5`) lands them, the same posture this ADR's inline commitments take.*
...
| 14 | Settlement routing reads the `ce_settlementtarget` header ALONE; the substrate router never reads `Movement.AccountRef` to route (§Payload amendment 2026-07-11) | unit | `SETTLEMENT_TARGET_HEADER_ONLY` | Catalogued (CA-16, Live) |
| 15 | An `engine-ca` leg's `AccountRef` equals the promoted `Movement.AccountRef`, never the `ACCT-{processId}` placeholder (§Payload amendment 2026-07-11) | unit + integration | `SETTLEMENT_LEG_ACCOUNT_REF_PROMOTED` | Catalogued (CA-17, Live) |
| 16 | Two payouts on ONE account carry the SAME `AccountRef` while their intent refs differ — the account-identity and exactly-once axes are orthogonal (§Payload amendment 2026-07-11) | unit | `SETTLEMENT_LEG_ACCOUNT_REF_STABLE` | Catalogued (CA-18, Live) |
