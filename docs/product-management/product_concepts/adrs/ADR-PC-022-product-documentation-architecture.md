...
>
> *Revised 2026-06-19 (bd `babelstone-sfnt.26`): the "no second `reference/`" clause above stands as written — there is still exactly **one** generated reference and the product-docs tree restates nothing. What changed is its **location**: that single generated reference was **moved into** `docs/product-docs/reference/`, so product-docs now hosts (not duplicates) the reference quadrant. See §P1's 2026-06-19 amendment for the path/link details.*
>
> *Revised 2026-06-20 (bd `babelstone-i42s`): product-docs gains a fifth top-level slot, `catalogue/` — a hand-authored "product menu" organised by **business decision**, one readable page per product family (term deposit is the first), for the product owner / config author at the moment of **choosing the shape of an offering**, before authoring a pack or variant. It sits **alongside** the four Diátaxis quadrants, not inside them: a "what can I offer?" menu is none of learn / do / understand / look-up cleanly, and its organising axis (business decision) exists nowhere else in the set. This is **additive and explicitly not the rejected 2026-06-01 overlay**: it adds no persona `reading-paths/` (it serves the same single audience as the rest of the set, at an earlier moment — not a role tree); it is **not a second `reference/`** (it restates no normative content — it links to the generated family-schema, `02`-scope §2, financial-concepts §5, and ADR-PC-008 / ADR-PC-024 for every field and rule, the same **link-don't-restate** discipline §P1 requires, and it declares itself non-authoritative and lagging the schema by a deliberate margin); and it is **not a navigation layer over an illegible base** (it is audience-facing content on a now-legible base, the same ground on which the 2026-06-04 Update note resumed the tutorials / how-to / explanation tree). The §P1 "all four Diátaxis quadrants" enumeration stands as written for the four quadrants; this slot is a sanctioned **companion** to them, not a fifth quadrant. Future families get **more pages in `catalogue/`**, never a per-persona tree — the same growth rule the Update note sets for the quadrants.*
...
*Revised 2026-06-19 (bd `babelstone-sfnt.26`): the generated `reference/` tree **moved** from `docs/product-management/reference/` to `docs/product-docs/reference/`, becoming the generated **reference** quadrant of the Diátaxis product-docs set the Update note (above) resumed — so product-docs now carries all four Diátaxis quadrants (tutorials / how-to / explanation hand-authored, reference generated). This is a **relocation of the single generated reference, not a second reference**: nothing is restated or duplicated, the link-don't-restate discipline is intact, and the tree is still rendered by `make docs-gen` and gated byte-identical by `make docs-verify` (commitments #1–#2 unchanged). Only the path changed, so the renderer now emits a `../../../product-management/` hop (`../../product-management/` from the shallower `reference/README.md` / `glossary.md`) to reach the concern-axis spine, which itself stays physically untouched under `docs/product-management/`. The move makes product-docs the single docs front door; [ADR-PC-026](./ADR-PC-026-csharp-api-reference-docfx.md) §P3 (revised the same day) makes the product-docs README the DocFX home page.*

### P2 — Reference is generated, banner-marked, and CI-gated (the un-driftable quadrant)

Two `make` targets mirror the [contracts-check](../../../../Makefile) idiom: `make docs-gen` regenerates `reference/`; `make docs-verify` regenerates into a scratch tree and diffs against the committed tree — **a non-empty diff fails**. A path-scoped CI lane ([ADR-IC-009](../../integration_concepts/adrs/ADR-IC-009-testing-infrastructure.md), [ADR-PC-019 §P1](./ADR-PC-019-repository-strategy-monorepo.md)) runs `docs-verify` whenever a reference source *or* the generator changes — the same structural guarantee [ADR-IC-008](../../integration_concepts/adrs/retired/ADR-IC-008-event-catalog-governance-tooling.md) gives the EventCatalog. Every generated page opens with a `<!-- DO NOT EDIT — generated by make docs-gen from <source> -->` banner.
...
- [ADR-IC-008](../../integration_concepts/adrs/retired/ADR-IC-008-event-catalog-governance-tooling.md) — EventCatalog-as-generated-governance: the precedent that governed reference is generated and source-controlled.
