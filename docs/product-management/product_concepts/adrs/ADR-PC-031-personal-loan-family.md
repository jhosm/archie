...
3. **The second decider informs the pipeline decision.** With two deciders now in hand, the `ConstitutionPipeline` extraction (bd `babelstone-osv6`) can be judged on evidence rather than pre-built on one example. That judgement (recorded in `PersonalLoanConstitutionService`'s remarks) is a **documented defer**: the *full* choreography's rule-of-three is **not** cleanly met — the two constitution paths diverge on exactly the steps a one-example pipeline would freeze (term-deposit constitution is **de-settled** — it appends only, settlement is the saga's gated step, bd `babelstone-t7o3.4` — whereas this disbursement settles eagerly with a lump-sum **debit** at `t=0`; the pack-primitive reads and the variant tails differ too). The one genuinely-shared piece (the rate-resolution-and-fail-loud two-step) cannot be extracted without either widening spine coupling (`Babelstone.RateSheets` → `Babelstone.Engine`, for a six-line helper) or creating the shared cross-family application project [ADR-PC-021 §D1](./ADR-PC-021-application-layer-family-owned-deciders.md) **rejects**. So the deciders stay written **for-lift** (§P4) and the pipeline is taken when a path's full settle/primitive/tail shape genuinely matches — a documented defer is sounder than a leaky abstraction ([ADR-PC-021 §P5](./ADR-PC-021-application-layer-family-owned-deciders.md) rule-of-three discipline).
