# ADR-PC-031: personal_loan Family — a Closed-End Amortizing Personal Loan on the One-Engine-Many-Families Spine
...
| Depends on | [ADR-PC-030](./ADR-PC-030-product-scope-and-boundary.md) (the product scope — names `personal_loan` as roadmap item 2, the closed-end asset; fixes the boundary this family honours), [ADR-PC-021](./ADR-PC-021-application-layer-family-owned-deciders.md) (the family-as-plugin spine this rides — pure folds + a family-owned decider over a family-agnostic engine), [ADR-PC-024](./ADR-PC-024-constitution-precondition-contract.md) (the precondition contract — keeps origination upstream; the engine records verdicts, never makes the decision), [ADR-PC-010](./ADR-PC-010-dotnet-hand-rolled-engine.md) (the deterministic kernel + the `Money`/decimal rounding discipline the amortization math obeys), [ADR-PC-008](./ADR-PC-008-rate-sheet-storage-and-deploy-api.md) (rate resolution at disbursement — the decider's stamp), [ADR-PC-032](./ADR-PC-032-money-movement-primitive.md) (the `Movement` primitive — every loan cash leg is an `Originated` `Movement` recorded **append-first**, its cash effected by the one generic gated settlement leg; this family's disbursement / installment / early-repayment / settlement legs are *implementations* of that contract, never eager pre-append settlements) |
| Resolves | bd `babelstone-g6ar` (Author personal_loan family); realises [ADR-PC-030 §Open Action 2](./ADR-PC-030-product-scope-and-boundary.md) |

---

## Context

[ADR-PC-030](./ADR-PC-030-product-scope-and-boundary.md) fixed what babelstone is *for* — a core product & account ledger — and drew a **family roadmap** spanning the retail product topology: a liability (term deposit, *built*), a **closed-end asset** (`personal_loan`, *next*), an open-end revolving asset (credit card), and the transactional account (conta à ordem). [ADR-PC-021](./ADR-PC-021-application-layer-family-owned-deciders.md) fixed *how a family is added*: own event records, pure fold handlers, a lifecycle legality table, projections, an `IFamilyModule`, and a family-owned decider — all reaching `main` through a one-way **family → engine** dependency arrow, with **zero generic-engine diff**. A single reference family — `term_deposit` — proved both.
...
| 2 | **Lump-sum disbursement** | The loan pays out the whole principal at `t=0`, recorded as an `Originated` `Movement` (a **`Debit`**, `reason` `disbursement`) against the funding account, where a deposit takes the principal *in*. | The `DisburseAsync` path: resolve → decide → **append (carrying the `Movement`)** → the generic gated settlement leg ([ADR-PC-032](./ADR-PC-032-money-movement-primitive.md)) — **append-first**, the money leg a downstream saga step, never a pre-append `settle`. Mirrors the (already de-settled) deposit constitution path, money-direction inverted. |
| 3 | **Capped early repayment** (*reembolso antecipado*) | A partial or full prepayment of the outstanding capital plus a **legally-capped** commission: `min(charged, statutory_cap) × capital_repaid`, never exceeding the interest the borrower would still have paid (fin-math §7.5). PT consumer-credit caps: 0.50% with >1y remaining, 0.25% with ≤1y. | The decider's `DecideEarlyRepayment`, off `Amortization.EarlyRepaymentCommission`; a full repayment carries a closing `Originated` `Movement`, effected by the same gated settlement leg ([ADR-PC-032](./ADR-PC-032-money-movement-primitive.md)). |
...
**S3 · Exit cost.** Low. The decider's disbursement choreography (resolve → stamp → decide → **append (carrying the `Movement`) → gated settlement leg**, [ADR-PC-032](./ADR-PC-032-money-movement-primitive.md)) is written as the *same separable steps* the term-deposit constitution path uses, so the generic `ConstitutionPipeline` extraction ([ADR-PC-021 §P5](./ADR-PC-021-application-layer-family-owned-deciders.md), bd `babelstone-osv6`) — triggered by *this* being the second decider — is a lift, not a rewrite.
...
### `personal_loan` is a closed-end amortizing-loan family on the [ADR-PC-021](./ADR-PC-021-application-layer-family-owned-deciders.md) spine; its amortization math is a generic `FinancialMath` kernel; origination stays upstream.

- **D1 — A family on the proven spine.** `personal_loan` is authored exactly as the term-deposit reference family: pure fold handlers + event records (`Babelstone.Families.PersonalLoan`), a lifecycle legality table, projections, an `IFamilyModule`, and a family-owned decider (`Babelstone.Families.PersonalLoan.Application`). The dependency arrow is **family → engine**; adding it is **zero generic-engine diff** (the `Amortization` kernel is the only generic addition, and it names no family — gated by `ENGINE_FAMILY_AGNOSTIC`).
- **D2 — The closed-end-asset lifecycle.** The loan lifecycle is `disburse → amortize → (settle | write off)`, plus the disbursement-refusal and GDPR-erasure terminals. The family-owned events are `LoanDisbursed`, `LoanDisbursementFailed`, `LoanInstallmentPaid`, `LoanRepaidEarly`, `LoanSettled`, `LoanWrittenOff`; the states are `Pending → Active → (Failed | Settled | WrittenOff) ; Erased`. GDPR erasure is **not** a family-owned event: the terminal `Erased` transition is driven by the engine-declared cross-cutting `operations.PersonalDataErasureRequested` ([ADR-PC-004 §P3/A4](./ADR-PC-004-pii-crypto-shredding.md)), folded into `LoanPosition` via the `IErasable` seam — the same one fact every PII-holding family shares, declared once in the spine rather than re-owned here. The single `LifecycleTransitions` table is the auditable legality source the decider consults before appending — every business-terminal state is closed to every business transition; GDPR erasure is the one cross-cutting transition legal from any PII-holding state.
- **D3 — Three new dimensions, all command-side.** The **amortization schedule** (French / constant-installment), **lump-sum disbursement** (an `Originated` `Movement` — a `Debit` — recorded **append-first** at `t=0`, [ADR-PC-032](./ADR-PC-032-money-movement-primitive.md)), and **capped early repayment** are the only genuinely-new modelling vs. the deposit. All three run in the **decider** (the impure command layer); the **folds stay pure** and only record the already-computed facts the events carry (the installment split, the capped commission, the carried `Movement`). The amortization math is the generic `Amortization` kernel in `Babelstone.FinancialMath`, obeying the [ADR-PC-010 §P1–§P2](./ADR-PC-010-dotnet-hand-rolled-engine.md) one-boundary `Money` rounding (each installment's interest rounded once, capital the integer residual, the schedule conserving to the cent with a balancing final row). **Every loan cash leg is a `Movement` recorded before its append** — the decider never settles before it appends ([ADR-PC-032 §Decision slot 5](./ADR-PC-032-money-movement-primitive.md)); the cash is effected by the one generic confirmation-gated settlement leg, closing the disbursement-atomicity orphan window (bd `babelstone-5r9n.1`) by construction.
...
```
families/personal-loan/src/
  Babelstone.Families.PersonalLoan/              pure folds + events + projections + lifecycle table
      refs: Babelstone.Engine, Babelstone.FinancialTypes        (cannot reach a DB or the kernel)
  Babelstone.Families.PersonalLoan.Application/  the decider (commands → events)
      refs: Babelstone.Engine, Babelstone.FinancialMath, Babelstone.FinancialTypes,
            Babelstone.RateSheets, Babelstone.Packs, Babelstone.Families.PersonalLoan
engine/src/Babelstone.FinancialMath/
  + Amortization                                   the one new generic, family-agnostic kernel
contracts/
  avro/loans/personal_loan/*.avsc                the bus-published event payload schemas
  cue/families/personal-loan.cue                 the product-config family schema
```
...
### P4 — The disbursement choreography is append-first and mirrors the deposit constitution for the pipeline lift

The decider's `DisburseAsync` is written as the same separable steps the term-deposit constitution path uses — **resolve rate sheet → stamp tan + version → decide (compute schedule) → append (carrying the `Originated` `Movement`, a `Debit`) → gated settlement leg**, with the money leg a *debit* (loan pays out) where the deposit's is the principal *in*. The settlement step is the one generic confirmation-gated leg ([ADR-PC-032 §Decision slot 5](./ADR-PC-032-money-movement-primitive.md)), **downstream of the append** — an eager `settle`-before-`append` is illegal ([ADR-PC-032 §Consequences 3](./ADR-PC-032-money-movement-primitive.md)). Under [ADR-PC-032](./ADR-PC-032-money-movement-primitive.md) the loan's money leg now **converges** with the (already de-settled) deposit constitution rather than diverging from it — both append first and effect cash through the same gated leg (this *is* bd `babelstone-t7o3.13`'s deposit/loan settlement convergence). This deliberate shape-match is what lets the second-decider `ConstitutionPipeline` extraction ([ADR-PC-021 §P5](./ADR-PC-021-application-layer-family-owned-deciders.md), bd `babelstone-osv6`) be a lift rather than a rewrite.
...
3. **The second decider informs the pipeline decision.** With two deciders now in hand, the `ConstitutionPipeline` extraction (bd `babelstone-osv6`) can be judged on evidence rather than pre-built on one example. That judgement (recorded in `PersonalLoanConstitutionService`'s remarks) is a **documented defer**: the *full* choreography's rule-of-three is **not** cleanly met — but note [ADR-PC-032](./ADR-PC-032-money-movement-primitive.md) **removes the settlement step as a point of divergence**. Both paths are now **de-settled and append-first**: the term-deposit constitution appends only and settles on the saga's gated step (bd `babelstone-t7o3.4`), and this disbursement does the *same* — records an `Originated` `Movement` and effects cash through the **one generic gated settlement leg** ([ADR-PC-032 §Decision slot 5](./ADR-PC-032-money-movement-primitive.md)), which lives in the saga substrate, **not** duplicated in either decider. What still diverges is narrower — the pack-primitive reads and the variant tails. The one decider-resident shared piece (the rate-resolution-and-fail-loud two-step) cannot be extracted without either widening spine coupling (`Babelstone.RateSheets` → `Babelstone.Engine`, for a six-line helper) or creating the shared cross-family application project [ADR-PC-021 §D1](./ADR-PC-021-application-layer-family-owned-deciders.md) **rejects**. So the deciders stay written **for-lift** (§P4) and the pipeline is taken when a path's full primitive/tail shape genuinely matches — a documented defer is sounder than a leaky abstraction ([ADR-PC-021 §P5](./ADR-PC-021-application-layer-family-owned-deciders.md) rule-of-three discipline).
...
3. **The disbursement (and every loan cash leg) depends on the generic gated settlement leg [ADR-PC-032](./ADR-PC-032-money-movement-primitive.md) mandates.** That parameterised saga module is platform work owned by PC-032 (§Consequences 2 there); until it lands, this family's money legs cannot be effected append-first. The family declares its legs *as data* (`reason` + direction); it never hand-codes an eager settlement.
...
*Amendment 2026-06-24 — the v1 loan lifecycle is the good/closed **binary**; arrears/grace/restructure are FUTURE additive states (additive scope cut, does not change the §D2 Decision — bd `babelstone-5r9n.9`): §D2 gives the v1 `personal_loan` lifecycle as `Pending → Active → (Failed | Settled | WrittenOff) ; Erased`, and every operating transition (installment paid, early repayment, settlement) runs **only from `Active`** — the single non-`Active` exit from a troubled loan is the terminal `WrittenOff`. This records, explicitly, that v1 models **no distressed/servicing state** between healthy and written-off: there is no `Arrears` / `InGrace` / `Restructured` state, no missed-installment event, and no partial-/late-payment lifecycle. The loan is, by deliberate scope, a **good-or-closed binary** — performing while `Active`, otherwise terminated — consistent with the "What this choice makes harder" item 2 above (variable-rate, grace-period (*carência*), balloon-installment loans out of scope here) coarse-start posture. This is a **scope cut, not a contradiction**: arrears, grace (*carência*), and restructuring are **future additive states + transition rows** in the same `LifecycleTransitions` table when a richer loan family lands (the open/closed-property discipline — every business-terminal state closed to every business transition — already accommodates new non-terminal states additively, exactly the coarse-start fine-drift the term-deposit schema took, authoring §3.1). Collections themselves stay **upstream and recorded-not-executed**: per §D4 and [ADR-PC-030 §P1 item 4](./ADR-PC-030-product-scope-and-boundary.md) (the "no collections *enforcement*" stance — the engine may *record* PARI/PERSI transitions as events but does not *run* the legal procedure), the PT consumer-credit distress procedures (PARI / PERSI, DL 227/2012) are an **upstream collections concern**; even when the richer family lands, the engine's role is to **record** a distress/restructure transition as an event, never to execute the regulated procedure — the [ADR-PC-030 §P1 items 3–6](./ADR-PC-030-product-scope-and-boundary.md) recorded-not-executed pattern extended from origination to collections (the engine recording a PARI/PERSI transition is the **recorded-not-executed option**, available but not implemented in v1). No code changes accompany this amendment (no distressed state is brought into scope); it makes the v1 binary-lifecycle scope cut an explicit, audited decision rather than a silent omission.*

*Amendment 2026-06-26 — the fin-math doc **references, not restates**, the statutory early-repayment caps; a doc-to-constant cross-check is DEFERRED (additive, does not change the §D5 Decision — bd `babelstone-5r9n.7`): §D5 fixes the PT consumer-credit statutory early-repayment caps (0.50% with >1y remaining, 0.25% with ≤1y) as the ceiling the decider enforces, and the kernel records them as the constants `PersonalLoanDecider.StatutoryCapBpsOverOneYear` (50 bps) / `PersonalLoanDecider.StatutoryCapBpsUnderOneYear` (25 bps). The same two numbers were also being **restated as bare prose** in [financial_concepts §7.5](../../financial_concepts/banking_products_financial_mathematics.md) (the early-repayment section), with nothing keeping the two copies in sync — a latent **drift** risk, not a present disagreement (the doc and the constants currently agree). This amendment records the v1 decision: **§7.5 now cites the kernel constants (and this §D5) as the single source of truth** for the cap values — reference-not-restate — keeping the statute citation (DL 133/2009 art. 19) for the legal reader but pointing the authoritative values at the symbols, so a future change to a constant has **one** authoritative place and the doc cites it rather than silently diverging. The values themselves are unchanged (0.50% / 0.25% stay correct). Correctness of the constants remains gated by the `CREDITO_PESSOAL_AMORTIZATION_MATH` commitment test (catalogue row CP-1, `Live` — see the 2026-06-24 §Verifiable-commitments amendment); that gate asserts the **runtime** clamp, not the doc's wording. An **automated doc-to-constant cross-check** (a fitness function asserting the prose value tracks the constant) is **DEFERRED**: the durable path is the **pack-sourced regulatory-constant** route (the kernel reading the cap from a `pt.YYYY.N` pack field rather than a hard-coded `const`), which has **no pack field today** and is out of scope here ([ADR-PC-007](./ADR-PC-007-signed-yaml-oci-pack.md) / the pack-author path). Until that lands, the reference-not-restate prose + the `CREDITO_PESSOAL_AMORTIZATION_MATH` math gate + reviewer attention are the composed guard — the [ADR-PC-020 §D3](./ADR-PC-020-llm-toolchain-and-conformance-governance.md) explicit-drift posture, mechanical gate plus human review, exactly as the for-lift ordering discipline is handled in the 2026-06-24 §Verifiable-commitments amendment. No code or constant changes accompany this amendment; it is a docs-sourcing decision (reference-not-restate) plus a recorded defer.*
...
| 3 | The French-system amortization schedule **conserves capital to the principal to the cent** and zeroes the balance, and the early-repayment commission **never exceeds `min(charged, statutory_cap) × capital_repaid` nor the lost-interest ceiling** (§D3, §D5, §P2). Pinned by `AmortizationMathTests` / `PersonalLoanDeciderTests` (the worked fin-math §4.1 example matched to the cent, the conservation invariants, the statutory-cap clamp). | unit (Docker-free) | `CREDITO_PESSOAL_AMORTIZATION_MATH` | Live |

Related: this family's family-agnosticism rides the engine-level [`ENGINE_FAMILY_AGNOSTIC`](./commitment-catalogue.md) (row 12) and the variant-level [`ZERO_ENGINE_DIFF_PER_VARIANT`](./commitment-catalogue.md) (row 9) — adding a *family* is zero *generic*-engine diff, the same property [ADR-PC-021](./ADR-PC-021-application-layer-family-owned-deciders.md) gates. Row 3 above (`CREDITO_PESSOAL_AMORTIZATION_MATH`) is the family-specific financial-math correctness commitment; it is `Live` (the tests exist and pass Docker-free in CI's default unit lane, and with this ADR Accepted the catalogue row CP-1 is registered as a §P6 coverage-checked gate — see the 2026-06-24 amendment below). The loan's cash legs being recorded **append-first** is *not* a new commitment here — it rides [ADR-PC-032](./ADR-PC-032-money-movement-primitive.md)'s `MOVEMENT_APPEND_FIRST` / `MOVEMENT_CASH_LEG_IDEMPOTENT` (the loan disburse/installment/early-repay call sites are exactly the ones that gate covers, bd `babelstone-5r9n.1`).

*Amendment 2026-06-24 — `CREDITO_PESSOAL_AMORTIZATION_MATH` (catalogue row CP-1) flips Planned → Live; the for-lift step ordering is reviewer-enforced, not mechanical (additive, does not change the Decision — bd `babelstone-5r9n.4`): with this ADR Accepted, the row-3 condition above ("the catalogue row is registered as the §P6 coverage checker's target when this ADR is Accepted") is met. The Test ID `CREDITO_PESSOAL_AMORTIZATION_MATH` is anchored in `AmortizationMathTests` (`families/personal-loan/tests/Babelstone.Families.PersonalLoan.Application.Tests/AmortizationMathTests.cs`) — the worked fin-math §4.1 example matched to the cent, the capital-conservation invariants, and the statutory-cap clamp — and runs Docker-free in CI's **default unit lane** (no Testcontainers, no `packs`/`contracts` trigger required), so the §P6 coverage checker registers it `Live`. The row-3 `Status` in the table above therefore reads **Live**, and the [commitment catalogue](./commitment-catalogue.md) row CP-1 flips Planned → Live to match (the catalogue is the authoritative status source per [ADR-PC-020 §P5–§P7](./ADR-PC-020-llm-toolchain-and-conformance-governance.md); this amendment records the ADR-side reconciliation, while the catalogue's own CP-1 status edit is applied centrally — see the PR's reported catalogue flip). **One discipline stays human, not mechanical:** the §P4 / §Consequences "for-lift" property — that the disbursement choreography's steps stay written in the **same separable, lift-ready ordering** the term-deposit constitution path uses, so the `ConstitutionPipeline` extraction (bd `babelstone-osv6`) is a lift rather than a rewrite — is a **reviewer-enforced** design constraint, **not** a CI gate. No analyser asserts step ordering; a reviewer of any change to `DisburseAsync` (or the deposit constitution path) must confirm the for-lift shape is preserved. `CREDITO_PESSOAL_AMORTIZATION_MATH` gates the math's correctness; it does **not** gate the choreography's for-lift ordering — that remains a judgement layer, exactly the [ADR-PC-020 §D3](./ADR-PC-020-llm-toolchain-and-conformance-governance.md) explicit-drift-gate posture that mechanical gates and human review compose.*

---

## Cross-references

- [ADR-PC-030](./ADR-PC-030-product-scope-and-boundary.md) — the product scope that names `personal_loan` as the closed-end asset (roadmap item 2) and fixes the boundary (origination upstream; the engine records, never decides).
...
- [ADR-PC-032](./ADR-PC-032-money-movement-primitive.md) — the `Movement` primitive every loan cash leg implements: disbursement, installment, early-repayment, and settlement are `Originated` `Movement`s recorded **append-first** and effected by the one generic gated settlement leg (closing the disbursement-atomicity orphan window, bd `babelstone-5r9n.1`; converging deposit/loan settlement, bd `babelstone-t7o3.13`).
...
*Proposed 2026-06-21 by jhosm. Accepted 2026-06-23 (reconciled to [ADR-PC-032](./ADR-PC-032-money-movement-primitive.md) — cash legs append-first via the `Movement` primitive).*
