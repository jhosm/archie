...
| LCD-2 | **The lifecycle driver never outruns de-settled settlement** — for a recurring schedule the driver fires occurrence N+1 only when occurrence N's `Originated` cash leg ([ADR-PC-032](./ADR-PC-032-money-movement-primitive.md)) is not parked in `HUMAN_INTERVENTION_REQUIRED`; since the engine advances a loan's paid-count on the `LoanInstallmentPaid` **event** (not on settled cash), an automated catch-up after an outage cannot advance the paid-count past collected cash. Maturity (one-shot) needs no such gate. *(Live for the driver-side mechanics — scoped: `SettlementHealthGateTests` proves hold-while-parked / resume-after-settlement / fail-closed probe + the bounded catch-up on the Docker-free rig; `SettlementHealthGateIntegrationTests` proves the probe's indexed `EXISTS` over the orchestrator's real migrated `saga_state` schema on PostgreSQL; and `The_probe_wire_literals_are_in_lock_step_with_the_settlement_saga` pins `SettlementProcess`/`HUMAN_INTERVENTION_REQUIRED` byte-for-byte. A held pass emits `lifecycle_schedule_held_total` (the alert rule on it is bd `babelstone-1nkm.4`). Scope: this delivers the gate's READ side. The end-to-end headline — no catch-up past uncollected cash in production — waits on two upstream gaps tracked as follow-ups: the personal-loan family ships no orchestration module, so loan Movement-bearing events do not yet reach the settlement saga (bd `babelstone-9z9w`), and the settlement substrate keeps one terminal saga per `ce_subject`, so a recurring schedule's occurrence N ≥ 2 has no per-occurrence settlement instance to park (bd `babelstone-3o6m`, open question Q-BH). Until both close, the gate is armed but its trigger condition is unreachable for loans.)* | [ADR-PC-036 §Decision 4](./ADR-PC-036-lifecycle-command-driver.md) | integration (Testcontainers) | `LIFECYCLE_DRIVER_SETTLEMENT_HEALTH_GATE` | Live |
