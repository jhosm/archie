...
| CP-1 | The **credito_pessoal French-system amortization schedule conserves capital to the principal to the cent and zeroes the balance**, and the **early-repayment commission never exceeds `min(charged, statutory_cap) × capital_repaid` nor the lost-interest ceiling** (the PT consumer-credit caps: 0.50% with >1y remaining, 0.25% with ≤1y, fin-math §7.5). The amortization math runs command-side in the decider, never in a fold; the folds record the already-computed installment split. *(Test exists and is green Docker-free — `AmortizationMathTests` (`Babelstone.Families.CreditoPessoal.Application.Tests`) pins the fin-math §4.1 worked example to the cent (C=€10,000 / TAN 6% / 12m ⇒ €860.66 installment), the capital-conservation + zero-balance invariants, the interest-decreasing/capital-increasing shape, the zero-rate degenerate case, and the statutory-cap + lost-interest clamps; `CreditoPessoalDeciderTests` pins the per-installment split, the full-vs-partial repayment + settlement pairing, and the [ADR-PC-024](./ADR-PC-024-constitution-precondition-contract.md) precondition refusal. Stays `Planned` until ADR-PC-031 is Accepted and the §P6 coverage checker registers this row's Test ID against the running tests in the required CI lane.)* | [ADR-PC-031 §D3/§D5](./ADR-PC-031-credito-pessoal-family.md) | unit (Docker-free) | `CREDITO_PESSOAL_AMORTIZATION_MATH` | Planned |
