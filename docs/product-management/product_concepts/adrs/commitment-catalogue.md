...
| LCD-4 | **A multi-replica lifecycle driver fires each due occurrence exactly once** — with N≥2 replicas all running the `LifecycleSchedulePass`, each due occurrence is claimed once on the durable `lifecycle_dispatch_ledger` under `FOR UPDATE SKIP LOCKED` (+ a per-instance `pg_try_advisory_xact_lock`), the winner POSTs the command and the others skip it; the **durable claim — not an elected leader — is the single-firing guard**, the same competing-consumers pattern as the saga dispatcher ([ADR-PC-029 slot 3](./ADR-PC-029-engine-command-ingress.md), `SagaCommandDispatchDrainer`) and the outbox relay ([ADR-IC-004 §P2](../../integration_concepts/adrs/ADR-IC-004-outbox-pattern-mechanism.md), `OutboxDrainer`). A claim that crashes mid-POST is re-claimable, and the engine `command_dedup` (`ENGINE_COMMAND_IDEMPOTENT`, row 19) makes the re-POST effectively-once — the authoritative correctness floor this optimization sits in front of, never replaces. *(Planned: the gate is named and the Test ID reserved; the multi-claimant integration test is written with the durable-ledger claim implementation, bd `babelstone-1nkm.2`, against a real PostgreSQL.)* | [ADR-PC-038 §Decision 2](./ADR-PC-038-lifecycle-driver-leader-election-and-durable-ledger.md) | integration (Testcontainers) | `LIFECYCLE_DRIVER_SINGLE_FIRING` | Planned |
