...
| 12b | The **Engine API host's composition code** (`Babelstone.Engine.Api/Program.cs`) names **no concrete family type in code** — neither a family aggregate STATE type (`DepositPosition`), nor a family store/endpoint type (`PostgresDepositReadModelStore`, `DepositsEndpoints`), nor a `Babelstone.Families.*` identifier. The host is the §D4 composition root and KEEPS its `families/**` `ProjectReference` as the `HostModuleLoader` scan anchor (§A14), so this is NOT the `.csproj` gate row 12 is (that would contradict §A14) — it is a SOURCE gate: with family host modules discovered by assembly-scan (bd `babelstone-9w2k.2`) and all per-family wiring relocated into the family's `IFamilyHostModule` (bd `babelstone-9w2k.1/.5`), the host's composition file names no family in code (a family named only in a COMMENT is fine). The host-side capstone of the family-count-invariant epic, the cousin of row 12 / ORCH-1. *(Live via bd `babelstone-9w2k.5`: `EngineApiHostFamilyAgnosticTests.Host_Program_cs_names_no_concrete_family_type_in_code` scans `Program.cs` (comments + string literals stripped); runs in CI's engine default lane.)* | [ADR-PC-021 §P2/§D4 + §A2/§A14](./ADR-PC-021-application-layer-family-owned-deciders.md) | architecture / source assertion (CI) | `ENGINE_API_HOST_FAMILY_AGNOSTIC` | Live |
