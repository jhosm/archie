...
4. **Seed the load-bearing commitment catalogue** — *done: the ~8 §P7 invariants (append+outbox atomicity, Money boundary, determinism, post-flag-never-gates, pin-per-event, AML-edge, batch-ingest idempotency, replay budgets, zero-engine-code-per-variant) are seeded as the first fitness functions in [`conformance/commitments.yaml`](../../../../conformance/commitments.yaml), with a per-ADR `Verifiable commitments` table on each source ADR (PC-001/009/010/012/013/014/015/017); the two non-ADR commitments are catalogue-only per [ADR-PC-000 §A2](./ADR-PC-000-namespace-and-contract-shape-framework.md). All `Planned` until their gates land (the §P10 spec-first loop); the #5 coverage checker that consumes the catalogue is still pending.*
...
- [`conformance/commitments.yaml`](../../../../conformance/commitments.yaml) + [`conformance/README.md`](../../../../conformance/README.md) — the seeded commitment catalogue (Open Action #4), the machine-readable registry the §P6 coverage checker consumes.
