...
*Amendment 2026-06-22 — operating-bank sign-off granted (additive, does not change the Decision — bd `babelstone-xydd` / Epic `babelstone-pqwc`): the operating bank **signed off the §Named-targets v1 POC defaults unchanged**, with no deviation required — RPO ≈ 0 for committed events, ≤ 60 s base-backup floor, RTO ≤ 15 min failover, ≤ 4 h full restore, and full-book recovery cold-replay within the published window. The "pending operating-bank sign-off" qualifier on the §Named-targets heading and Residual Risk #1 is hereby satisfied; the production-cutover gate is cleared. Recorded at [04 Q-AY](../04-open-questions.md).*
...
- **Known gap, now wired (`SYNC_REPL_APPEND_COST`):** the synchronous-replication append-latency must be included in the Q-AK load test (§P1) — a falsifiable RPO-vs-write-latency claim. *Filled 2026-06-20 (additive, does not change the Decision — bd `babelstone-2e6q.5` / L.3e): the catalogue's growth provision now carries the row [`SYNC_REPL_APPEND_COST`](./commitment-catalogue.md) (Status `Planned`). The load harness (ADR-PC-011) measures append p50/p99 with `synchronous_commit` on vs off and the §P1 delta, toggling the GUC via connection-string composition (no engine-core change). It is GATING against a real warm standby (the HA overlay) and ADVISORY on the single-node CI stack — the live-cluster verification of the production cost is the residual budget per Residual Risk 2 below. The deliberate, visible hole per [ADR-PC-020 §P5](./ADR-PC-020-llm-toolchain-and-conformance-governance.md) is closed.*
...
*Revised 2026-06-20: Amendment A1 — the OpenBao key-store DR budget scales with KEY CARDINALITY at v4 scale (bd `c14p.2` / M.6).*

In plain English: this ADR already says the OpenBao key store has to be backed up and recoverable at least as strongly as the event store. What it did not account for is that at a real bank's scale there are **millions** of per-customer keys (one per customer, so "forget me" destroys exactly one person's key — [ADR-PC-004](./ADR-PC-004-pii-crypto-shredding.md)). With Integrated Storage (Raft) the whole keyspace is memory-resident, so the **Raft snapshot size, the unseal time, the replica join time, and the steady-state RAM** of the key-store cluster all grow with the key count — which directly sizes its DR posture (how big a snapshot to ship off-site, how long a cold unseal/join takes against the §Decision RTO). This amendment names that dependency and points at the load-harness probe that measures it. It is **additive** — §P4 (key-store DR as critical as event-store DR) and the §Decision RTO/RPO targets all stand verbatim.

- **A1 · Key-store DR is cardinality-sensitive (a refinement of §P4).** §P4 establishes that the OpenBao key store is HA + backed up with guarantees at least as strong as the event store. At v4 scale the key store holds **millions of per-subject transit keys** ([ADR-PC-004 §P2/§P3](./ADR-PC-004-pii-crypto-shredding.md), Amendment A3). Under Raft Integrated Storage the keyspace is memory-resident, so the snapshot size, the unseal/join time the §Decision RTO must absorb on a key-store recovery, the replication lag, and the steady-state RAM **all scale with the key count**. The §Decision RTO/RPO numbers and the §P5 recovery drill must therefore be sized **for the v4 key cardinality**, not just the event-store volume — a dependency §P4 left implicit.

- **A1 · The measurement seam (owned jointly with PC-004).** The load harness (ADR-PC-011) `KeyCardinalityProbe` seeds a growing per-subject key population and measures the per-key encrypt/decrypt/destroy slope; the full **v4-cardinality sizing pass on a Raft-backed cluster** (not the single-node dev-mode container the CI lane uses) derives the snapshot-size / unseal-time / RAM budget that feeds this ADR's RTO/RPO sizing and §P5 drill. The CI integration lane proves the per-key-op slope is flat at a bounded N; the Raft snapshot/unseal dimensions are the residual budget this amendment registers. This is the [ADR-PC-004 A3](./ADR-PC-004-pii-crypto-shredding.md) residual risk seen from the DR side.

- **A1 · This refines §P4; it does not supersede this ADR.** §P4 (key-store DR as critical as event-store DR, the crypto-shred-vs-backup horizon), the §Decision RTO/RPO targets, and §P5 (recovery drills as DORA evidence) all remain binding **as written**. This amendment only *names key cardinality as a DR-sizing input* and points at the measurement seam — no target or guarantee is reversed.

**Residual Risk 6 (added by A1): The key-store DR budget is unsized until the v4-cardinality Raft sizing pass.** At millions of resident per-subject keys, the OpenBao Raft snapshot size, unseal/join time, replication lag, and steady-state RAM scale with key count and bound the §Decision RTO on a key-store recovery. *Mitigation:* the `KeyCardinalityProbe` (ADR-PC-011) measures the per-key-op slope (flat at the CI N); the full v4-cardinality sizing pass on a Raft cluster derives the snapshot/unseal/RAM budget that sizes the RTO/RPO targets and the §P5 drill; carried jointly with [ADR-PC-004 §A3](./ADR-PC-004-pii-crypto-shredding.md).
