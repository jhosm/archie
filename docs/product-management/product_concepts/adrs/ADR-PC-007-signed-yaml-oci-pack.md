...
### A4 · First-party promotion pins to the build of the promoted git commit, not "current latest" (2026-07-09, bd `babelstone-2t16.31.2`)

**In plain English:** §A2 made promotion resolve each first-party image's `:latest` to a digest, verify it, and pin it — atomic and verified, but it deploys "whatever `:latest` is right now", not the build of the specific commit being promoted. This ties promotion to the exact git commit instead: it pins each image to the immutable `sha-<commit>` build `image-build.yml` stamps, so "promote commit X" provably deploys the images built from X. (§A3 above — the third-party *reproducibility* pins, bd `babelstone-2t16.31.1` — is the sibling amendment from the same batch of work; this §A4 covers the *first-party commit-pin* and is independent of it.)

§A2 established the resolve → cosign-verify → pin flow for first-party images but sourced the digest from the movable `:latest` tag. This amendment changes only the *source* of that digest, leaving the verify-and-pin machinery and its fail-closed posture untouched: `scripts/cd-pin-images.sh` now honours a `PROMOTE_GIT_REF` env var (`.github/workflows/cd.yml` sets it to `${{ github.sha }}`, the promoted commit), resolving each `ghcr.io/jhosm/babelstone-*` image to its `sha-<PROMOTE_GIT_REF>` build rather than `:latest`. Both tags point at the same digest for a given build, so the cosign signature (bound to the digest, §P2) verifies identically — this is a provenance refinement, not a new trust boundary.

**The path-scoped-build problem, and the fallback (the decided design).** Image builds are path-scoped ([ADR-PC-019 §P1](./ADR-PC-019-repository-strategy-monorepo.md)): a commit that touched only one service rebuilds only that image, so `sha-<promoted-commit>` legitimately **404s** for every image that commit did not rebuild. Failing the whole promotion on any such miss would make promotion nearly impossible (few commits rebuild all images), and silently resolving `:latest` on a miss would reintroduce exactly the untied-to-a-commit drift this amendment removes. The decided design is a **deliberate ancestor walk**: for each image lacking a build at the promoted commit, walk that commit's git ancestors newest-first and pin the newest ancestor that *does* have a `sha-` build — i.e. "this image exactly as it stood as of the promoted commit" — loudly logged as a fallback. It is deterministic (git ancestry is totally ordered by `git rev-list`), always a real cosign-signed build, and **never** silently `:latest`. `PROMOTE_STRICT=1` overrides the walk to fail-closed (require an exact-commit build for every rendered image) for a caller wanting the stricter guarantee; and if *no* ancestor has a build at all, the promotion fails rather than falling back to `:latest`. The walk needs the commit history, so the promote job checks out at `fetch-depth: 0`.

This is **additive**: it reverses no part of the Decision or of §A2 — the resolve → cosign-verify → pin flow, the `ghcr.io/jhosm/babelstone-*` first-party scope, and the fail-closed discipline are all unchanged; only the digest's *source* moves from `:latest` to the promoted commit's build (with the deterministic ancestor fallback). Recorded in Verifiable commitments below.
...
The §A4 commit-pin refines §A2's *source* without adding a new gate: the same promote-time `cosign verify` fail-closed check now runs over the promoted commit's `sha-<commit>` build (or, on a path-scoped miss, its newest ancestor build), and the hermetic `--contract` lane still proves the resolve → verify → pin path stays wired. The never-`:latest` rule and the deterministic ancestor fallback are enforced by `cd-pin-images.sh` itself — it fails closed if no ancestor build exists, and `PROMOTE_STRICT=1` fails on any miss. No commitment-catalogue Test ID is wired: like §A2 this is a delivery-pipeline invariant, not an engine replay/fold property.
