...
- **The downstream scheduler is deferred (DEF-2), so v1 ships the signal without the consumer.** v1 emits the maturity-calendar projection; the scheduler that reads it and notifies is post-v1. This is the same accept-now/build-downstream posture as [ADR-PC-014](./ADR-PC-014-customer-notification-emit-contract.md)'s delivery side — named here so the gap is a tracked decision, not a surprise.
