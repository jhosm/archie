...
No executable engine commitments are wired to this ADR. The engine's two hard commitments here are *negative* and realised structurally, not by a fitness function — it exposes `sor` as a first-class read-surface column (§1, §6) and it does **not** staple a command endpoint onto its read projection, rejecting [§6.4](../feature-design-strangler-fig-coexistence.md) option (c) for the engine (§6 · slot 6). The routing logic itself is explicitly the bank's channel/gateway tier, out of engine-fitness-function scope (§Decision Binding authority). This ADR composes with one separately-owned seam rather than claiming it — the read-model freshness the router depends on rides the [ADR-PC-017](./ADR-PC-017-legacy-batch-ingest-contract.md) `BATCH_INGEST_IDEMPOTENT` gate (governed by ADR-PC-017 · slot 4), so an observed legacy instance is never duplicated underneath a routing lookup.

- **Known gap, no Test ID yet wired:** the fail-closed routing guard (§5) — when `sor` is unresolvable the router *refuses* the operation and never guesses a backend — is architecturally testable but has no catalogue row today (a deliberate, visible hole per [ADR-PC-020 §P5](./ADR-PC-020-llm-toolchain-and-conformance-governance.md)), to be added under the catalogue's growth provision if and when the engine ever hosts the routing decision; while routing lives in the bank's gateway tier the guard is the channel team's review gate, not engine code.
