...
| Related | [ADR-PC-012](./ADR-PC-012-gl-posting-signal-contract.md) (GL posting signal contract — the sibling outbound-signal contract that shares the raw-facts-out philosophy), [ADR-PC-014](./retired/ADR-PC-014-customer-notification-emit-contract.md) (notification emit contract — same outbound shape + the PII-by-reference rule this ADR inherits), under the [ADR-PC-000 signal-contract principle](./ADR-PC-000-namespace-and-contract-shape-framework.md) — an **outbound-fact** instance that already embodies projection-as-signal ([ADR-PC-023](./ADR-PC-023-temporal-signals-projection-derived.md)): *absolute* days-past-due + as-of read, not a clock-driven "DPD is now N" event |
...
These signals are **structural-only**; `exposure_id` and any obligor identifier are **references**, never PII. No name, NIF, address, or contact data — cleartext or ciphertext — rides them. If a downstream regulatory-reporting consumer ([Q-AX](../04-open-questions.md)) needs the obligor's identity on an IFRS 9-derived report, it resolves PII at point of use against the **engine-internal PII-resolve surface** ([ADR-PC-014 §"PII by reference"](./retired/ADR-PC-014-customer-notification-emit-contract.md), [ADR-PC-004](./ADR-PC-004-pii-crypto-shredding.md)) — which returns null for a crypto-shredded subject and keeps the OpenBao boundary inside the engine. [ADR-PC-014](./retired/ADR-PC-014-customer-notification-emit-contract.md) already named this ADR as a consumer of that cross-cutting rule; this contract is the second instance and adds nothing new — which is the point. IFRS 9 provisioning is largely exposure/portfolio-level and rarely needs obligor PII at all.
...
This contract's load-bearing commitments are fitness functions in the [commitment catalogue](./commitment-catalogue.md) — the single source of truth for each commitment's exact claim, gate (pyramid level), and `Live`/`Planned`/`Gap` status ([ADR-PC-020 §P5–§P7](./ADR-PC-020-llm-toolchain-and-conformance-governance.md)). No signal is emitted in v1 (the v1 product is a liability, not an exposure), so the gate is built before the v2 credit scope implements the contract:

- `IFRS9_POST_FLAG_NEVER_GATES` — IFRS 9 is downstream and never gates or compensates a credit operation (slot 5 · Error model).
