...
*Revised 2026-06-11: the **runtime** base tags named above are hardened to the chiseled variants `mcr.microsoft.com/dotnet/{aspnet,runtime}:10.0-noble-chiseled-extra` to shrink the base-OS CVE surface the report-only grype image scan reports ([ADR-IC-014](../../integration_concepts/adrs/ADR-IC-014-static-analysis-and-supply-chain-scanning.md) §S2). The `-extra` flavour retains ICU + tzdata, which the engine's culture- and timezone-sensitive accrual math (§P1) requires; the images run non-root (UID 1654). Version-and-flavour only — the `sdk:10.0` build stage and the `mise.toml` `dotnet = "10.0.300"` pin are unchanged, and the Language and Framework decisions are untouched.*
