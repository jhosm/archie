...
The load-bearing invariant this ADR owns is a **correctness guard, not a performance SLA**: a snapshot must never be trusted blindly. Concretely (§P4) — a rebuild *from* a snapshot must reproduce state whose hash at `last_event_id` matches the stored `snapshot_hash`, and the monthly discard-and-rebuild drill must reproduce identical state from the log alone. *Filled 2026-06-20 (additive, does not change the Decision — bd `babelstone-0uau.1` / L.5 + bd `babelstone-0uau.2` / L.6): the catalogue's growth provision now carries the row [`SNAPSHOT_REPLAY_EQUIVALENCE`](./commitment-catalogue.md) (Status `Planned`). The load harness (ADR-PC-011) drives `AggregateRuntime.LoadAsync` snapshot-then-tail vs a snapshot-free cold fold over a deep stream and asserts byte-identity + a speedup (a fast-but-divergent snapshot FAILS regardless of speed); the L.6 discard-rebuild drill exercises the discard-of-populated-snapshots correctness path, and the §P6 operational surface (lag/hash-mismatch alarms, advisory→trusted promotion) is documented in [`infra/runbooks/snapshot-operations.md`](../../../../infra/runbooks/snapshot-operations.md). The deliberate, visible gap per [ADR-PC-020 §P5](./ADR-PC-020-llm-toolchain-and-conformance-governance.md) is closed.*
