...
| Reporting hooks (`reporting.*`) | `bdp_estatisticas_taxas_juro`, `fgd_cobertura_depositos`, `bdp_centralizacao_responsabilidades`, `ifrs9_staging` | v1 activates `bdp_estatisticas_taxas_juro` (monthly aggregates) and `fgd_cobertura_depositos` (annual + on-demand; FGD deposit-guarantee coverage). `bdp_centralizacao_responsabilidades` and `ifrs9_staging` are reserved for credit / IFRS 9 (see [Open Question 2 — IFRS 9 Signal Boundary](./04-open-questions.md)). |
