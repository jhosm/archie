...
> motivating research for babelstone's product scope (crédito pessoal is the *next* family
> on the roadmap; origination/underwriting stays upstream of the engine). The briefs remain
> **descriptive market research** — they perform no engine/ADR/contract design themselves;
> that lives in ADR-PC-030. Sibling of the credit-card research in
...
| **Output format** | Separate markdown briefs in `research/personal-loan/` (this folder) |
