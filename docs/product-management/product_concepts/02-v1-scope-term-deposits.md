...
- **Partial-withdrawal rules.** Minimum withdrawal amount, minimum remaining balance, and lock-up (*carência*) period enforced by the decider; the `DepositPartiallyWithdrawn` event already exists ([§2.4.1](#241-family-specific-events-term-deposit-family-schema)). A product **declares** these gates in an optional `partial_withdrawal` block on its variant config (`min_withdrawal_cents` / `min_remaining_balance_cents` / `carencia_days`, mirroring the engine's `PartialWithdrawalPolicy`), resolved at constitution as an explicit decider input ([ADR-PC-008](./adrs/ADR-PC-008-rate-sheet-storage-and-deploy-api.md); [ADR-PC-021 §D3](./adrs/ADR-PC-021-application-layer-family-owned-deciders.md)). A variant that **omits** the block permits no F.12-gated partial withdrawals — it resolves to `PartialWithdrawalPolicy.Unrestricted`, the zero-gate policy, leaving only the structural rules the decider always applies (a positive amount; the whole balance cannot be withdrawn — that is a termination, F.4).
