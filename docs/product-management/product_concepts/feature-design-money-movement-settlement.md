...
> **Status: design draft for maintainer ratification.** This section is the **keystone** that unblocks the engine-CA settlement build (bd `babelstone-u79p.2/.3/.4/.5`, epic `babelstone-98mj`). It reconciles two things the code already has in tension: the **intent-derived exactly-once reference** ([ADR-PC-043](./adrs/ADR-PC-043-intra-engine-settlement-counterparty.md), Accepted) — which names *which economic payout* a leg effects, but **not which account** — with the **persistent customer account** the money must actually land on. It **honours** ADR-PC-043, [ADR-PC-032](./adrs/ADR-PC-032-money-movement-primitive.md) §A6, and [ADR-PC-037](./adrs/ADR-PC-037-current-account-family.md); it proposes **no ADR change** — see §2A.7 for the one clarification that a follow-up ADR-PC-043 amendment should later pin.
...
### 2A.7 What this design does *not* change — and the one ADR clarification recommended for follow-up

This section **honours** ADR-PC-043, ADR-PC-032 §A6, and ADR-PC-037 and introduces **no** contradiction: it fills in the account-identity half that ADR-PC-043 §Idempotency deliberately left to the `Movement.AccountRef` promotion seam, and ADR-PC-032 §A6 already frames the promotion as staged. The exactly-once key, the header-only routing, the loop-breaker (`Origin=Observed`), and the credit-admission gate are all **unchanged**.

**Recommended follow-up (not applied in this lane):** ADR-PC-043's slot-1 payload list names the promoted `ce_settlementtarget` header and the `amount` field, but does **not** yet explicitly name the promoted **`AccountRef`** as a third settlement-command-body field distinct from the intent-derived `CoreHoldRef`/`CreditRef`. A dated **[ADR-PC-043](./adrs/ADR-PC-043-intra-engine-settlement-counterparty.md) §D5 amendment** should pin, on the ADR itself, that the CA-apply body carries the promoted persistent `AccountRef` as the destination (the `WRONG-ACCOUNT` in-band guard, sibling to the `amount`/`WRONG-AMOUNT` guard slot 1 already names) — so the account-identity axis is recorded on the Accepted ADR, not only in this companion. **A sibling lane owns ADR-PC-043 this run, so this lane deliberately does not touch it; the amendment is left as an explicit hand-off.**
