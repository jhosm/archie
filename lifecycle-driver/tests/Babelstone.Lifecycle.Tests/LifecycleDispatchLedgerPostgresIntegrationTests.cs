...
/// babelstone-1nkm.2): the driver's own migration series applies, and the per-occurrence atomic claim on
/// <c>lifecycle_dispatch_ledger</c> IS the multi-replica single-firing guard — the
/// <c>LIFECYCLE_DRIVER_SINGLE_FIRING</c> commitment (catalogue row LCD-4). In plain terms: N replicas all
/// find the same due occurrence; exactly ONE wins the claim and POSTs it, a claim that dies mid-POST is
/// re-claimable, and the engine's <c>command_dedup</c> (never this ledger) remains the correctness floor
/// for the re-POST. Same Testcontainers convention as the saga dispatcher's and outbox relay's suites —
/// the two precedents whose <c>FOR UPDATE SKIP LOCKED</c> pattern the ledger reuses.
/// </summary>
