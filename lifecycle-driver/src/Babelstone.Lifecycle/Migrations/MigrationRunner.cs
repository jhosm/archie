...
/// lifecycle-driver host's OWN migration series (ADR-PC-038 §Decision 1; lifted from the engine's
/// <c>Babelstone.EventStore.Migrations.MigrationRunner</c> via the orchestrator's copy). Thin and
/// hand-rolled (ADR-PC-010): no migration framework owns the schema. It connects as the migration role
/// (ADR-PC-001 §P3) — the role that holds the DDL privileges the driver's runtime role
/// (<c>babelstone_lifecycle</c>) is denied.
/// </summary>
public sealed class MigrationRunner(string connectionString)
{
    ...
    // constant from the engine's, the orchestrator's, and the family read-model runners', so a
    // lifecycle-driver deploy against a shared cluster does not block on another component's unrelated
    // migration set.
    private const long MigrationLockKey = 4831279065123847291;
