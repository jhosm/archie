    ...
    : ILifecycleCommandSink
{
    ...
    /// assembly, ADR-PC-021 §A9). Named locally here (not referenced) so the generic driver core stays free of
    /// any engine/hosting dependency; the engine authorises the route from the gateway-attested value, never
    /// the caller's word.</summary>
