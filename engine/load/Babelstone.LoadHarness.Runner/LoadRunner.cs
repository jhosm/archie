...
internal sealed class LoadRunner
{
    ...
    // never from the Docker-free unit lane (`Category!=Integration`). The [Category("Integration")]
    // Testcontainers suite (LoadRunnerIntegrationTests, bd babelstone-2e6q.7) drives RunAsync end to end
    // (smoke + replay, with and without the live-Redpanda bus path) against a real PostgreSQL + Redpanda
    // in the CI integration lane, so these members are MEASURED rather than excluded. The PURE,
    // Docker-free-testable surface — the static PlanPhases(RunnerOptions) phase planner (pinned by
    // BurstProfileTests) and the DrivePhase struct — stays a measured, covered branch as before.
