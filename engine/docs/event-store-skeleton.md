...
> **Not realized as designed (bd `babelstone-e6fr.6`).** This §5.2 premise assumed a CUE-declared *event taxonomy* — but the CUE schemas as built are variant-config, not an event taxonomy (ADR-PC-006), so there is nothing to cross-check against. The full bidirectional check was therefore **dropped, not deferred**: the aggregate fold is already fail-closed (an unhandled event type throws at append AND replay; duplicates throw at registry build), so the check would only shift-left an already-loud failure, and for the store-only events its taxonomy would be a hand-copy of the handler list (weak independence). The genuinely-independent half ships instead as a catalogue-driven completeness fitness test (`FamilyHandlerCatalogueCompletenessTests`): every event type with an integration *contract* (an `.avsc` / EventCatalog entry) must have a registered handler.
...
- **Family module (§4.4).** `IFamilyModule` drops the separate `EventPayloadTypes` list — each `HandlerRegistration(EventType, PayloadType, Handler, EventSchemaVersion=1)` already carries its payload type, and `HandlerRegistry` builds both the event-type→handler and payload-type→registration maps from it. The `FamilyModuleLoader` CUE cross-check was **dropped, not deferred** (bd `babelstone-e6fr.6`; see the §5.2 note above): the fold is already fail-closed and no CUE event taxonomy exists, so the catalogue→handler completeness fitness test (`FamilyHandlerCatalogueCompletenessTests`) ships the independent half instead.
...
| **A.6** handler dispatch | `archie-n0nq` | `IEventHandler<,>`, `HandlerResult<>`, `IHandlerRegistry`/`HandlerRegistry`, `AggregateRuntime<>` (read via `IEventStore`, write via `IEventSink`; `IPiiProtector` + `IEventSerializer` seams), `IFamilyModule`, `FamilyModuleLoader`, typed `Snapshot<>`/`SnapshotStore<>`/`ISnapshotPolicy`. **Dropped, not deferred (`babelstone-e6fr.6`):** the full `FamilyModuleLoader` CUE cross-check (no CUE event taxonomy exists + the fold is already fail-closed); the catalogue→handler completeness fitness test (`FamilyHandlerCatalogueCompletenessTests`) ships the independent half |
