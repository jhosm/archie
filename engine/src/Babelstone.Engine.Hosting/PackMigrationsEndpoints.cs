...
// right family's registered IPackMigrationService / IPackMigrationInstanceResolver (ADR-PC-021 §P2).

/// <summary>
/// An operator pack migration: re-pin a target instance set from one pack version to a newer one
/// (ADR-PC-009 §P3). The target set is named ONE of two ways (exactly one — they are mutually
...
/// ADR-PC-009 §P4): there are no silent upgrades, and this endpoint never touches the currently-active
/// version for new constitutions.
/// </summary>
/// <remarks>
/// Family-agnostic (it lives in the hosting spine, ADR-PC-021 §P2): the HTTP contract, the validation,
