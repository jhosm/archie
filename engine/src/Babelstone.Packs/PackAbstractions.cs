...
/// <summary>
/// Pulls a pack's files by OCI digest (ADR-PC-007 §P2 — by digest, never by tag). The real
...
/// <summary>
/// Verifies a pack's cosign signature (ADR-PC-007 §P2). The real implementation
...
/// CI (ADR-PC-006 §P3), so the loader does not re-run <c>cue vet</c>.
/// </summary>
...
/// <summary>A resolved pack pin: the OCI reference, the image digest, and the signature digest (ADR-PC-007 §P3).</summary>
...
/// digest — the <c>pack_versions</c> registry of ADR-PC-007 §P3. A missing row is a fail-loud
/// startup error (the store turns a null into a <see cref="PackLoadException"/>).
/// </summary>
/// <remarks>
/// The Postgres-backed registry (the <c>pack_versions</c> table / migration) is a separate
/// follow-up; <see cref="InMemoryPackVersionRegistry"/> is the configuration-backed
/// implementation used by the walking skeleton and tests.
/// </remarks>
...
/// <summary>
/// The in-engine pack loader/verifier (C.5, ADR-PC-007 §P4). <see cref="GetAsync"/> does the
...
/// pinned <c>pt.YYYY.N → (OCI ref, digest, signature digest)</c> mapping. Immutable once
/// constructed. The Postgres-backed <c>pack_versions</c> table is a separate follow-up.
/// </summary>
