...
/// <summary>
/// Persistence for rate sheets (ADR-PC-008 §P1/§P3): append-only writes, idempotent
...
public interface IRateSheetStore
{
    ...
    /// already taken — the deploy endpoint turns that into the §P2 idempotency outcome.
    /// </summary>
    ...
    /// <summary>
    /// Resolves the sheet active at <paramref name="asOf"/> for a family (ADR-PC-008 §P3):
...
/// unique key. The deploy endpoint re-reads and applies the §P2 rule: identical body
