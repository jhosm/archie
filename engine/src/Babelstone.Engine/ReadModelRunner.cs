...
/// folded <typeparamref name="TState"/> to a <see cref="ReadModelRow"/> via a family-supplied
/// <paramref name="map"/> function and UPSERTing it through <see cref="IReadModelStore"/>.
/// </summary>
/// <remarks>
/// <para>
...
/// <typeparamref name="TState"/> and family-agnostic by construction (the family closes the type and
/// supplies the <paramref name="map"/>); the engine spine never names a deposit, so it stays under
/// ENGINE_FAMILY_AGNOSTIC (ADR-PC-021 §P2). The mirror of <see cref="ProjectionRunner{TState}"/>,
/// but writing the flat read-model row instead of a bitemporal belief row.
/// </para>
/// <para>
/// Idempotency under at-least-once delivery is the ADR-IC-005 §P2 monotonicity guard inside
/// <see cref="IReadModelStore.UpsertAsync"/>: a re-delivered event whose sequence is at or below the
/// stored row's is dropped by the UPSERT's WHERE, so re-applying an already-folded event is a no-op.
/// The mapper MUST be pure (no clock, no I/O, no randomness) — it receives the event's
/// transaction_time as data, so the rebuild stays deterministic.
/// </para>
/// </remarks>
public sealed class ReadModelRunner<TState>(
    string kind,
    string family,
    ProjectionMode mode,
    ...
    Func<TState> seed,
    Func<ReadModelFold<TState>, ReadModelRow> map,
    IReadModelStore store) : IProjectionRunner
    where TState : class
{
    ...
    private TState DeserializeDetail(ReadOnlyMemory<byte> detail) =>
