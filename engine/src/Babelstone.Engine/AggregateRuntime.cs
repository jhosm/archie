...
/// <c>last_updated</c> a read served from the denormalized read-model row would (ADR-IC-005) —
...
/// stay pure, the runtime owns the clock (ADR-PC-010).
/// </summary>
...
/// registered Schema-Registry <c>schema_id</c> (ADR-IC-002 / ADR-IC-004). When non-null the
/// append DUAL-ENCODES inside the one sink transaction (ADR-PC-028): JSON → store, Avro →
...
/// is what flips snapshots ON for v1 (ADR-PC-003). Has no effect without a <paramref name="snapshots"/>
/// store (there is nowhere to write).
/// </param>
/// <param name="onSnapshotError">
/// Fail-soft sink for a post-commit snapshot-write failure. Per ADR-PC-003 / event-store §8.1 the
    ...
    ICalendarBoundaryPolicy? calendarBoundaryPolicy = null)
{
    ...
    public async Task<Hydrated<TState>> LoadAsync(Guid streamId, CancellationToken ct = default)
    {
        ...
        // Event-derived, never the wall clock (ADR-PC-010): the transaction_time of the last folded
    ...
    /// Honours snapshots (ADR-PC-003): it seeds from the latest VALID snapshot at or below
    ...
    /// <see cref="LoadAsync"/> (no clock, no randomness, ADR-PC-010), so a repeated as-of read at a
    ...
    {
        // Snapshot-then-tail for the as-of read (ADR-PC-003): seed from the latest VALID snapshot
        ...
        // Event-derived transaction_time of the last folded event (ADR-PC-010). A snapshot seed
    ...
    /// (ADR-PC-010 / ADR-IC-007): the pure decider/fold never touches telemetry. When
    ...
    /// to it compares like-for-like (ADR-IC-005) and folds the stream only while the projector lags.
    /// </returns>
        ...
        IReadOnlyList<KeyValuePair<string, object?>>? spanAttributes = null)
    {
        ...
        for (var i = 0; i < events.Count; i++)
        {
            ...
            // Catalog-gated relay (ADR-IC-017): the envelope above is ALWAYS written, but the outbox
            ...
            {
                // BUS encode (ADR-PC-028 dual-encode / STORE_BUS_ENCODING_EQUIVALENCE): the
                // outbox row carries the BUS codec's bytes + its registered schema_id (real Avro +
                // schema_id from the host's Avro+SR codec; ADR-IC-002 / ADR-IC-004), NOT the
                ...
                outboxRows.Add(new OutboxRow(
                    ...
                    // ADR-IC-018). Null for events that declare none — the common case. Carried onto
        ...
        // Snapshot write (ADR-PC-003 / event-store §8.1): AFTER the commit succeeds, evaluate the
    ...
    /// <summary>
    /// Post-commit, eventually-consistent snapshot write (ADR-PC-003 / event-store §8.1). Evaluates
    ...
    /// </summary>
    /// <remarks>
    /// Composes ALL THREE triggers of ADR-PC-003 (event-store §8.1): the per-N count
        ...
        bool isLifecycleBoundary, AppendContext context, CancellationToken ct)
    {
        try
        {
            ...
            // Snapshot-lag SLI (ADR-PC-003): report how far behind the
            ...
            // change replayed/folded state (ADR-PC-010).
            ...
            // head? The runtime owns the transaction-time clock (ADR-PC-010), so it compares the
            ...
            // The COMPOSING context (ADR-PC-003): per-N OR lifecycle OR calendar. CountBasedSnapshotPolicy
            ...
            // (ADR-PC-010). transactionTime is also carried as the snapshot's transaction_time (the
