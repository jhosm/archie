...
// lifecycle (the Hold* records above) and PersonalDataErasureRequested. In plain English: when a
...
//
// Declared HERE in the spine cross-cutting source (not a separate file) so the §P3 AsyncAPI reverse-
// orphan gate (scripts/asyncapi-catalog-validate.sh, ADR-IC-017 NO_UNCATALOGUED_EVENT_ON_BUS), which
// scans this file for the spine's promoted DomainEvent records, resolves the catalogued
// contracts/avro/operations/CreditUnapplied.avsc / CreditReapplied.avsc to a real, relay-capable event.
//
...
public static class CrossCuttingEventRegistrations
{
    ...
    [
        ...
        // The ADR-PC-043 slot-5 undeliverable-credit pair (declared above in this file): the two
