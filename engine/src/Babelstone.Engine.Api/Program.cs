...
builder.Services.AddOpenTelemetry()
    ...
    .WithTracing(tracing => tracing
        ...
        // The runtime no-PII guard (OBS_NO_PII_ATTRS / ADR-IC-007): strips any span tag
        ...
        .AddOtlpExporter())
    // Metrics (ADR-IC-007 Layer 1 / ADR-IC-004): listen to the engine's meter and export over
    // OTLP to the Collector → Prometheus, where the ADR-IC-004 warning/critical thresholds alert. The
    ...
    .WithMetrics(metrics => metrics
        ...
        .AddBabelstonePiiGuard()
        .AddOtlpExporter())
    // Logs (ADR-IC-007 Layer 1 / OBS_NO_PII_ATTRS): wire an OTel LoggerProvider so the
    ...
    // message-template hole) before export, while keeping the ADR-IC-007 operational references. Registered
...
// composition root: never on a saga message (ADR-IC-003) nor the durable bus (ADR-PC-004).
...
catch (SecretProviderException)
{
    ...
    // preserve the exact ADR-PC-001 contract message.
    throw new InvalidOperationException(
        "ConnectionStrings:Engine is required (the PostgreSQL event-store tier, ADR-PC-001 §P1).");
}

// The engine-instance's pinned pack(s) (ADR-PC-007, ADR-PC-009). Two modes,
...
//     logged at Critical, the ADR-PC-007 fatal-on-load discipline).
...
// pack a live instance references was pre-loaded above, ADR-PC-007).
...
// The runtime owns the clock (ADR-PC-010); the host stamps a missing constituted_at/matured_at.
...
// The spine-owned, account_ref-keyed MOVEMENT LEDGER (ADR-PC-032 read side): the read
// half the money-movement ADR named but deferred until the discoverability seam (IMovementBearing)
// existed. It is a family-agnostic spine component (ADR-PC-021) — ONE account-keyed projection,
...
// family-owned ownership) — the deposit-shaped table is one family's domain shape, so its
...
// The dual-encode split (ADR-PC-028 / STORE_BUS_ENCODING_EQUIVALENCE):
...
// The per-subject PII transit-key boundary (IPiiKeyStore, ADR-PC-004) — distinct from the
...
// Catalog-gated relay (ADR-IC-017 / INTEGRATION_EVENT_CATALOG_GATED): the engine publishes an
...
// authoring a schema/AsyncAPI entry is the deliberate promotion (ADR-IC-017), and an uncatalogued event is
...
// Composition at the edge (ADR-PC-021): the host enumerates the families it runs as
...
// (ADR-PC-021, `[new TermDepositHostModule()]`) is now ASSEMBLY-SCAN discovery (ADR-PC-021 Option B,
...
// (ADR-PC-021) stays reproducible across boots. Reflection is confined to this composition root
// (ADR-PC-010), never the dispatch spine. The ConfigureServices / MapEndpoints loops below are unchanged.
...
// MANDATORY fail-closed version-skew cross-check (ADR-PC-007 / ADR-PC-009):
// the pinned pack's family-manifest (families.yaml) is the authoritative per-deployment family
// set. Each discovered module stamps its SchemaVersion onto every EventEnvelope (ADR-PC-009), so a family or
...
// The family-AGNOSTIC async projection runtime (ADR-PC-002, two-modes §5.4): the single
// in-process relay + its registry + drainer, composed ONCE here at the ADR-PC-021 composition root rather
...
//   • AddSingleton<OutboxLagObserver> — the ADR-IC-004 SLI itself: an observable gauge of the oldest
...
// The relay producer's SASL/SCRAM identity (ADR-IC-016 plane ii / KAFKA_SASL_TOPIC_ACL): THIS
...
// (OpenBaoKvSecretProvider when OpenBao:Enabled, configuration-backed otherwise — ADR-PC-004). The resolved
// secret stays here in process memory to open the connection: never logged, never on a span, never on
// the bus (ADR-PC-004). With no Kafka:Sasl:OutboxPublisher:Username configured this is the OFF
...
// command window is load-bearing (ADR-PC-029 — pruning a receipt before the stream's active
...
// metrics-collection cycle — a lazily-resolved singleton would not register the ADR-IC-004 instrument
...
// trace id is an opaque operational identifier, never PII (ADR-IC-007 / ADR-PC-004). Placed
...
// The operator pack-migration command surface (ADR-PC-009 / surface §3.6): POST /v1/pack-migrations,
