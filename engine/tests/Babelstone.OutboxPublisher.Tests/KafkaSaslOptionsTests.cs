...
/// <summary>
/// KAFKA_SASL_TOPIC_ACL (C3 / ADR-IC-016 plane ii §4–§6): the SASL/SCRAM credential applier.
/// These pure tests pin the security-relevant behaviour without a broker — that a configured
/// credential turns SASL on and lands the per-service identity on the client config, that an
/// unconfigured one is a no-op (the additive local-dev posture), and that the cleartext PLAIN
/// mechanism is never the default.
/// </summary>
public sealed class KafkaSaslOptionsTests
{
    ...
    {
        var config = new ProducerConfig { BootstrapServers = "localhost:19092", EnableIdempotence = true };
        var sasl = new KafkaSaslOptions(); // no username ⇒ not configured

        Assert.False(sasl.IsConfigured);
        sasl.ApplyTo(config);

        // Additive no-op: SASL stays off and the load-bearing producer settings are untouched.
    ...
    public void Configured_options_apply_the_scram_identity_to_the_config()
    {
        ...
        Assert.Equal("svc-outbox-publisher", config.SaslUsername);
