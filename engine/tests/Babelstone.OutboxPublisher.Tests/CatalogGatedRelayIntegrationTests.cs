...
/// <c>.avsc</c>) and an UNCATALOGUED event (<c>InterestPaid</c>, which has a registered handler — so the
/// engine can append/fold it — but no <c>.avsc</c>), through the durable
/// <see cref="AggregateRuntime{TState}"/> wired with the REAL <see cref="AvroSchemaCatalog"/> as the
/// §P1 gate. It then asserts the biconditional at the storage seam: BOTH events are in the event store
/// (appended), but ONLY the catalogued one produced an outbox row (will reach the bus). The append+
/// outbox atomicity (ES_ATOMIC_APPEND_OUTBOX) is exercised by construction — both halves commit in the
/// runtime's single sink transaction.
/// </summary>
/// <remarks>
...
public sealed class CatalogGatedRelayIntegrationTests : IAsyncLifetime
{
    ...
    public async Task Only_catalogued_events_get_an_outbox_row_while_both_are_appended()
    {
        ...
        // The §P1 gate is the REAL embedded-schema catalogue: DepositConstituted has an .avsc
        // (catalogued → publishable), InterestPaid does not (uncatalogued → store-only).
        var catalog = new AvroSchemaCatalog();
        Assert.True(catalog.IsCataloguedIntegrationEvent("term_deposit.DepositConstituted"));
        Assert.False(catalog.IsCataloguedIntegrationEvent("term_deposit.InterestPaid"));
        ...
        var interestPaid = new InterestPaid(
            depositId, new Money(30_417), new Money(8_517), new Money(21_900), MaturityDate);
        await runtime.AppendAsync(depositId, expectedVersion: 0, [interestPaid], Ctx(), CancellationToken.None);
        ...
        // --- Assert: ONLY the catalogued event produced an outbox row (the bus surface). ---
    ...
    public async Task A_store_only_batch_appends_with_no_outbox_row()
    {
        ...
        var paid = new InterestPaid(depositId, new Money(100), new Money(20), new Money(80), MaturityDate);
        ...
        // Both InterestPaid and DepositRenewed are schemaless → the whole batch is store-only.
        var head = await runtime.AppendAsync(
            depositId, expectedVersion: 0, [paid, renewed], Ctx(), CancellationToken.None);
