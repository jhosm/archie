...
| backstage (catalogue portal) | Deployment + DB-init Job | 7007 | [ADR-IC-015](../../docs/product-management/integration_concepts/adrs/ADR-IC-015-event-catalog-governance-tooling-backstage.md) (supersedes the retired ADR-IC-008) — renders `catalog-info.yaml` from the baked image; uses a dedicated `backstage` database + role in the shared Postgres, provisioned by `backstage-db-init.yaml` (bd babelstone-zla1.6.6) |
