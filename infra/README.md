...
- **The Backstage catalogue portal renders the AsyncAPI catalogue ([ADR-IC-015](../docs/product-management/integration_concepts/adrs/ADR-IC-015-event-catalog-governance-tooling-backstage.md)).** The `backstage` service (Node.js + **in-memory SQLite — no Postgres**, bd babelstone-zla1.6.6) serves the governed `app-config.yaml` and the read-only `contracts/catalog/` descriptor tree (mounted in Compose, baked into the K8s image); its **only** catalogue source is the registered `catalog-info.yaml`, which `$text`-references the AsyncAPI files under `contracts/catalog/events/` — Backstage restates **no** schema (the no-drift invariant, ADR-IC-015 Decision §1–§2). Its state is fully reconstructible from `/catalog` on boot, so nothing is persisted. This retired the EventCatalog nginx placeholder (`eventcatalog/site/`, ADR-IC-008) that the supersession left behind.
  - **Human-handoff tail (this is what is NOT done headless).** Backstage ships **no turnkey image** — a real portal additionally needs: (1) an app scaffolded with `npx @backstage/create-app`, the AsyncAPI catalog-provider plugins added, and an image built + pushed. **The K8s path is now wired** (bd babelstone-zla1.6.6): `k8s/base/backstage.yaml` runs the real `ghcr.io/jhosm/babelstone-backstage:latest` (built by `backstage/Dockerfile`) with in-memory SQLite — no `backstage-db` pod. The `compose.yaml` service still carries the `:placeholder` image, profile-gated below. (2) The whole `contracts/catalog/` tree available to the portal (a ConfigMap is flat and can't carry the `events/`+`reconciliation/` subdirs the `$text` refs resolve against). **DONE for the K8s image**, which BAKES the tree in via a build-time COPY; the Compose stack still mounts it read-only. (3) The Backstage **GDPR surface** entered in the data inventory once user/team identity data exists (ADR-IC-015 Residual Risk). In Compose, the `backstage` service is gated behind the `catalog` Compose profile so the default `make up` **skips** it — the committed placeholder `image:` would otherwise fail to pull (`denied`) and abort the whole bring-up. Once a real image is wired there too, start the portal with `docker compose --profile catalog up`. Meanwhile the estate operates Git-native (AsyncAPI files + the [`asyncapi-catalog-validate`](../scripts/asyncapi-catalog-validate.sh) CI gate + GitHub's renderer) — the documented fallback posture [ADR-IC-015 §9](../docs/product-management/integration_concepts/adrs/ADR-IC-015-event-catalog-governance-tooling-backstage.md) records. `make verify` treats the portal as a non-fatal, informational check for exactly this reason.
