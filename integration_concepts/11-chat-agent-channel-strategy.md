...
How an authenticated MCP session binds to a banking customer (`sub` claim → `client_id`), how the OAuth flow integrates with PSD2 SCA at enrolment, how step-up authentication is requested mid-session, how refresh and revocation work, and what happens when the customer revokes access from an agent that has cached resource handles — these are not addressed in detail here. They are the substance of an update to [Document 10](./10-security-and-threat-model.md): a new trust boundary (agent → MCP server) and a section on customer-identity binding lifecycle. That update is filed as a follow-up issue.
...
| [Document 10 — Security](./10-security-and-threat-model.md) | A ninth trust boundary (agent → MCP server) and a new section on customer-identity binding are deferred to a follow-up update of doc 10. The six security principles apply as-is to the MCP boundary. |
