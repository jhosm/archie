...
**S4 · Community and longevity:** Kafka is a top-level Apache project with foundation governance, a very large contributor base, and a multi-vendor commercial ecosystem (Confluent, Redpanda, AWS MSK, Azure Event Hubs, Aiven). The probability of Kafka becoming unmaintained in the next decade is effectively zero. Redpanda (founded 2020) is younger and single-vendor controlled, but its Apache 2.0 licence and Kafka compatibility mean the community edition is not a lock-in risk — the application can switch to Apache Kafka if Redpanda's commercial direction diverges.
