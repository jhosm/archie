...
Document 03 defines CQRS as the mechanism for meeting the 500ms requirement on the client-facing deposits screen. Read models — denormalized, pre-computed projections fed by the Redpanda event backbone — decouple read performance from write-side complexity. The Anti-Corruption Layer (document 02) feeds Core Banking data into these projections, producing a cross-system combination of deposit conditions, client labels from CRM, and KYC signals from Compliance that no single source system holds. What remains to be decided is the concrete storage technology for those projections.
...
**S2 · Ecosystem coherence:** Maximum. Projectors are PostgreSQL writers; the API layer is a PostgreSQL reader. The same ORM, the same connection pool, the same EXPLAIN plan tooling, the same PostgreSQL slow-query log. UPSERT semantics (`INSERT … ON CONFLICT DO UPDATE`) are the natural mechanism for idempotent projectors (Primitive 5 from document 01). The `processed_events` deduplication table described in document 03 is a PostgreSQL table, in the same transaction as the projection update — the idempotency check is atomic with the write by construction. OpenTelemetry instrumentation is uniform with the rest of the application.
